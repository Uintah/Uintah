...
void SurfaceToSurface::setBdryCorticalBC(double *Phi_c, SurfTree *st, int ns, 
				    int nc) {
    for (int i=0; i<nc; i++) {
...
void SurfaceToSurface::getBW(double **a, int n, int *m1, int *m2) {
    *m1=0;
    *m2=0;
    for (int i=1; i<=n; i++) {
...
void SurfaceToSurface::bldInvMap(int *fwdMap, int *invMap, int nv) {
    int i;
    for (i=1; i<=nv; i++) {
	invMap[i]=-123456;
    }
...
void SurfaceToSurface::parallel(int proc) {
...
void SurfaceToSurface::buildAc(double **Ac, int ns, int nc, 
			  int nv, double **Avv, double **Avc, 
			  double **Asv, double **Asc) {
    int kk=0;
    int idx=0;
...
double SurfaceToSurface::rms(double *a, double *b, int n) {
    double total=0;
    double diff=0;
    for(int i=1; i<=n; i++) {
	total+=a[i]*a[i];
...
double **SurfaceToSurface::splitMatrix(double **Ass, double **Asv, double **Asc, 
...
void SurfaceToSurface::jacobi_sci(Matrix* matrix,
...
void SurfaceToSurface::solveSparse(int nr, int ncol, double **AA, double *X, 
			      double *B) {
...
void SurfaceToSurface::solveDense(int nr, int nc, double **A, double *x, 
			     double *b) {
#if 0
...
void SurfaceToSurface::execute() {
    ...
    if (!mh.get_rep()) {
	cerr << "Error: empty matrix in SurfaceToSurface\n";
	return;
    }
    if (!imesh->get(mesh)) return;
    if (!mesh.get_rep()) {
	cerr << "Error: empty mesh in SurfaceToSurface\n";
	return;
    }
...
// Phi_c = M^(-1) * u;
    ...
    int i;
    for (i=0; i<ns; i++) if (mesh->nodes[i]->bc == 0) cerr << "WRONG!\n";
...
//	    cerr << "ERROR - not same node: surf="<<st->points[st->bcIdx[i-nv]]<<"  mesh="<<mesh->nodes[i]->p<<"!\n";
    ...
    omesh->send(mesh);
    cerr << "Done with SurfaceToSurface::execute (timer="<<timer.time()<<")\n";
    timer.stop();
}
