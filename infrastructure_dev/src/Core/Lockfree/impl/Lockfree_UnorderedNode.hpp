#ifndef LOCKFREE_UNORDERED_NODE_HPP
#define LOCKFREE_UNORDERED_NODE_HPP
...
class LOCKFREE_ALIGNAS((get_alignment<T,Alignment>())) UnorderedNode
{
  static_assert( std::is_unsigned<BitsetType>::value, "ERROR: BitsetType must be an unsigned integer type." );

public:
  using node_type = UnorderedNode<T, BitsetType, Alignment>;
  using value_type = T;
  ...
  /// class iterator
  ///
  /// UnorderedNode iterators are moveable but not copyable.  When a valid iterator
  ...
  private:
    friend class UnorderedNode;

    // only UnorderedNode is allowed to construct a valid iterator
...
#endif //LOCKFREE_UNORDERED_NODE_HPP
