...
ZTexRenderer::render( const bool doFPS ) {
  ...
  if( glIsEnabled( GL_TEXTURE_2D ) == GL_TRUE )
    Log::log( DEBUG, "[ZTexRenderer::render] GL_TEXTURE_2D enabled" );
  else
    Log::log( DEBUG, "[ZTexRenderer::render] GL_TEXTURE_2D *not* enabled" );
...
ZTexRenderer::setTestZTex() {
  Log::log( ENTER, "[ZTexRenderer::setTestZTex] entered" );
  ...
  displayLists.push_back( ztexObj( list, texture ) );
#else
#if 0
  // Add a ztex object with the checkered texture!
  addZTex( triangle, 3, (char *)s_checkerTexture, 64, 64, true );
#else
#if 1
...
<setViewingMethod><method method = \"Geometry Transmission\">okay<eyePoint x = \"2.100000\" y = \"1.600000\" z = \"11.500000\"/><lookAtPoint x = \"0.000000\" y = \"0.000000\" z = \"0.000000\"/><upVector x = \"0.000000\" y = \"1.000000\" z = \"0.000000\"/><perspective far = \"116.77768\" fov = \"60.000000\" near = \"0.1553159\"/></method></setViewingMethod>\001\002\003\004\005";
  ...
  Log::log( LEAVE, "End of ZTexRenderer::setTestZTex" );
}
...
ZTexRenderer::readyTexture( int &width, int &height, char * texture,
  ...
  // If not, set newTex to NULL and return false
...
ZTexRenderer::addZTex( float *vertices,
		       ...
		       const bool replace ) {
  Log::log( ENTER, "[ZTexRenderer::addZTex] entered" );
  ...
  if ( readyTexture( texWidth,
    ...
    writePPM("after.ppm", theTex, texWidth, texHeight );
  }
  else {
    w_scale = 1.0;
    h_scale = 1.0;
  ...
  if ( gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, texWidth,
			 ...
			 GL_UNSIGNED_BYTE,
			 theTex/*texture*/) != 0 )
    Log::log( ERROR, "GLU BUILD MIPMAP ERROR!" );
  ...
  if ( list != 0 ) {
	...
	texY[0] << endl;
      ...
      // Normals
      normal = mkNormal( &vertices[ i ], 0, false );
