...
#define FLOP_ADD 1.0
      ...
      struct TeamTagVectorHM {};	// SIMD Type
      ...
      struct Functor {
        ...
        template<typename MemberType>
		KOKKOS_INLINE_FUNCTION
		void operator()(const TeamTagVectorHM &, const MemberType &member) const {
		  //const int kbeg = member.league_rank()*VectorLength;
		  ...
		  //Kokkos::parallel_for
			//(Kokkos::ThreadVectorRange(member, VectorLength),
			 //[&](const int &k) {
			  //const int kk = kbeg + k;
          	  //int kk = threadIdx.x;
                //if (kk < int(_c.extent(0))) {	//this condition is equivalent to threadIdx.x < _c.extent(0). I guess for vectorized code _c.extent(0) should be multiple of vector length or some masking logic should be added
					const int m = _c.extent(1), n = _c.extent(2), q = _a.extent(2);
					Kokkos::parallel_for
					  (Kokkos::TeamThreadRange(member,0,m*n),
					   [&](const int &ij) {
						...
						//using VectorTemp
						VectorTemp<value_type,VectorLength/VectorLength> cval(0.0);
						for (int p=0;p<q;++p)
						  cval += _a(k, i, p) *_b(k, p, j);
						_c(k, i, j) += cval;
...
//#endif
        ...
        if (1) {
                  ///
                  /// Team policy - Vectorized handmade
                  ///
        		  typedef Kokkos::View<value_type***,DeviceSpaceType> view_type;
                  view_type
                    a("a", N*VectorLength, BlkSize, BlkSize),
                  ...
                  //reinterpret_cast to vectorized views
                  typedef Vector<SIMD<value_type>,VectorLength> VectorType;
                  typedef Kokkos::View<VectorType***,DeviceSpaceType> view_type_simd;
                  view_type_simd a_simd, b_simd, c_simd;
                  a_simd = view_type_simd(reinterpret_cast<VectorType*>(a.data()), N, BlkSize, BlkSize);
                  b_simd = view_type_simd(reinterpret_cast<VectorType*>(b.data()), N, BlkSize, BlkSize);
                  c_simd = view_type_simd(reinterpret_cast<VectorType*>(c.data()), N, BlkSize, BlkSize);


                  double tavg = 0, tmin = tmax;
                  {
                    typedef Kokkos::TeamPolicy<DeviceSpaceType,ScheduleType,TeamTagVectorHM> policy_type;

                    typedef Functor<view_type_simd,AlgoTagType,VectorLength> functor_type;
                    typedef Kokkos::Impl::ParallelFor<functor_type,policy_type,DeviceSpaceType> parallel_for_type;
                    ...
                    for (int iter=iter_begin;iter<iter_end;++iter) {
                      ...
                      Kokkos::parallel_for(policy, functor_type(a_simd,b_simd,c_simd));
