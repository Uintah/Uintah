...
#This script generates the mean value over a region of the mean as time progresses. In other words,
# if the mean is computed as:
# \bar{\phi} = 1/T \int_{t_{start}}^{t_{end}} \phi dt
# and the final value as:
# mean(<\phi>)
# over a region, then this script computes the mean such that T is growing from tstart to tend.
...
for i in range(tstart,tend):
    ...
    DefineScalarExpression("mean_conv_ave", "(<"+vname+">"+
    " - conn_cmfe(<[" + str(tstart) + "]i:"+vname+">, <CC_Mesh>))/"+str(dt))

    ChangeActivePlotsVar("mean_conv_ave")
...
print "r=pickle.load(open(\"mean_conv_ave.pkl\",\"rb\"))"
