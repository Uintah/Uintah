...
// line, again with an optional one line header (use -noHexesCount if it's 
...
main(int argc, char **argv) {
  ...
  int nhexes;
  if (!hexesCountHeader) nhexes = getNumNonEmptyLines(hexesName);
  ifstream hexesstream(hexesName);
  if (hexesCountHeader) hexesstream >> nhexes;
