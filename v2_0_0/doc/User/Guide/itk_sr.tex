...
used in \xmlstarttag{parameter} elements which  describe filter
...
\replaceable{Data\_type\_name} is the name of a type that is declared or
...
The \xmlstarttag{inputs} element describe data types accepted by a
...
declared in a \xmlstarttag{template} element (as shown above) or it
can be a parameterized data type whose parameters are names declared
in a \xmlstarttag{template} element---for example, the following
template element associates type \datatype{float} with name
\datatype{ScalarType}: 
...
declared in a \xmlstarttag{template} element (as shown above) or it
can be a parameterized data type whose parameters are names declared
in a \xmlstarttag{template} element.
...
\begin{alltt}
  <parameters>
    <param>
      ...
      <type>\replaceable{c++\_builtin\_type}</type>
      <call>\replaceable{member\_function}</call>
...
\xmlstarttag{include} is an empty element and is closed with
characters \verb|/>|:
...
Elements \xmlstarttag{package} and \xmlstarttag{category} specifies
...
Element \xmlstarttag{value} declares a data type to be associated with
...
Hide a parameter using element \xmlstarttag{const}.  Element
\xmlstarttag{const} must have a \xmlattrname{value} attribute that
