...
itcl_class Packages/Phil_Tbon_TbonUG {
	...
	label $w.title -text "Unstructured TBON Parameters"
	button $w.bmeta -text "Metafile..." -command "$this metaui"

	scale $w.top.s1 -label "Threshold: Tets/Node" \
		-from 0 -to 255 -resolution 1 \
		-variable $this-threshold \
		-length 3c -orient horizontal

	make_labeled_radio $w.top.fa.radio1 "Brick Size (Nodes):" {} top \
		$this-nodebricksize {{128 128} {256 256} {512 512} {1024 1024} {2048 2048} {4096 4096}}
	make_labeled_radio $w.top.fa.radio2 "Brick Size (Data):" {} top \
		$this-databricksize {{256 256} {512 512} {1024 1024} {4096 4096} {8192 8192} {16384 16384}}
...
#	pack $w.top.l1 $w.top.e1 -in $w.top \
#		-side left -fill both
	pack $w.top.fa.radio1 $w.top.fa.radio2 -in $w.top.fa \
		-side left -fill both
	pack $w.top.s1 $w.top.fa -in $w.top -side top -fill both
