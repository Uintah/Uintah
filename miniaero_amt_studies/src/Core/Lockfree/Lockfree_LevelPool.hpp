...
public:
  ...
  iterator insert(size_t level, value_type const & value)
  {
    return emplace(level, value);
  }
  ...
  iterator emplace(size_t level, Args&&... args)
  {
    const size_t curr_size = m_size->fetch_add( one, std::memory_order_relaxed );

    const size_t id = level < m_num_levels ? level : m_num_levels - 1u;
  ...
  iterator find_any( UnaryPredicate const & pred ) const
  {
    ...
    for (size_t i=m_num_levels; !itr && i>0u; --i) {
      itr = m_pools[i-1u].m_pool.find_any(pred);
    }
  ...
  /// If there are no values for which predicate is true return an invalid iterator
  template <typename UnaryPredicate>
  iterator find_any( size_t level, UnaryPredicate const & pred ) const
  {
    return m_pools[level].m_pool.find_any(pred);
  }
  ...
  /// Contruct a pool
  LevelPool( size_t num_levels = 2u )
