...
inline bool VecEq(const dVector &V0, const dVector &V1, const double &DSqr = 0.0)
{
...
struct Edge
{
  ...
  dVector V; // The vertex result after the edge is collapsed.
  dVector Col; // Results of collapse.
  dVector Tex; // Results of collapse.
  double Cost;
  Edge *next, *prev; // For the linked list of all edges.

  int Heap; // Index into the heap dVector.
...
struct Mesh
{
  ...
  inline Vertex *AddVertex(const dVector &Vec, const dVector *Color, const dVector *TexCoord)
  {
    Vertex *V = new Vertex;
    V->V = Vec;
    if(HasColor)
  ...
  // Doesn't make anything point to this vertex.
  // This is faster.
  inline Vertex *AddVertex(const dVector &Vec)
  {
    Vertex *V = new Vertex;
    V->V = Vec;
    V->next = Verts;
  ...
  Vertex *FindVertex(const dVector &V, bool &AddedVert, const dVector *Color = NULL, const dVector *TexCoord = NULL);
  Vertex *FindVertexInEdgeList(const vector<Edge *> &Edg, const dVector &V, Edge * &e) const;
