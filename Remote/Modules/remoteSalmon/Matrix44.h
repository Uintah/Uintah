...
class Matrix44
{
  ...
  void build_rotate(double m[4][4], double, const dVector&) const;
  void build_scale(double m[4][4], const dVector&) const;
  void build_translate(double m[4][4], const dVector&) const;
...
public:
  ...
  inline Matrix44 &operator*=(const Matrix44& right)
  {
    ...
    return *this;
				...
				// homogeneous divide.
  dVector Project(const dVector& p) const;
  ...
  void Project(double &x, double &y, double &z, double &w) const;
				...
				// divide. Returns the dVector.
  dVector Project(const dVector &p, const double w) const;
  
				// Project using the upper
				// 3x3. Doesn't translate.
  dVector ProjectDirection(const dVector& p) const;
  
				// Transform by the inverse.
  dVector UnProject(const dVector& p);
	
				// Returns this * right.
  inline dVector operator*(const dVector& right) const
  {
  ...
  inline void LoadIdentity()
  {
    ...
    is_identity = true;
				...
				// dVectors. Bottom row is 0,0,0,1.
  void LoadFrame(const dVector&, const dVector&, const dVector&);
  void LoadFrame(const dVector&, const dVector&, const dVector&, const dVector&);
	
				// Returns column dVector. You can't
				// get the bottom row this way.
  void GetFrame(dVector&, dVector&, dVector&);
  void GetFrame(dVector&, dVector&, dVector&, dVector&);
  ...
  void Rotate(double, const dVector& axis);
  void Translate(const dVector&);
	
  void Frustum(double left, double right, double bottom, double top, double znear, double zfar);
  void Perspective(double fovy, double aspect, double znear, double zfar);
  void LookAt(const dVector& eye, const dVector& lookat, const dVector& up);
