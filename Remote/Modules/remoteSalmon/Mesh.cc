...
Vertex *Mesh::FindVertex(const dVector &V, bool &AddedVert, const dVector *Color, const dVector *TexCoord)
{
  Vertex TmpV;
  TmpV.V = V;
...
Vertex *Mesh::FindVertexInEdgeList(const vector<Edge *> &Edg, const dVector &V, Edge * &e) const
{
...
void Mesh::ImportObject(const Object &Ob)
{
  ...
  ASSERT1M(Ob.verts.size() % 3 == 0, "Must have a multiple of three vertices in Object.");

  HasColor = Ob.verts.size() == Ob.dcolors.size();
  HasTexCoords = Ob.verts.size() == Ob.texcoords.size();
  ...
  for(int i=0; i<Ob.verts.size(); i+=3)
    {
      ...
      Vertex *v0 = FindVertex(Ob.verts[i], AddedVert, HasColor ? &Ob.dcolors[i] : NULL,
	...
	continue;
      ...
      if(v1 == NULL)
	{
	  v1 = FindVertex(Ob.verts[i+1], AddedVert, HasColor ? &Ob.dcolors[i+1] : NULL,
	...
	v2 = vt;
      if(v2 == NULL)
	v2 = FindVertex(Ob.verts[i+2], AddedVert, HasColor ? &Ob.dcolors[i+2] : NULL,
...
void Mesh::CheckIntegrity(const bool Slow)
{
  ...
  for(Face *F = Faces; F; F = F->next, fc++)
	...
	ASSERT1M(F->prev, "No prev pointer");
      ...
      for(int i=0; i<F->e0->Faces.size(); i++)
	if(F->e0->Faces[i] == F)
	  break;
  ...
  for(Vertex *V = Verts; V; V = V->next, vc++)
      ...
      for(i=0; i<V->Faces.size(); i++)
	{
	  ...
	  // Make sure there is no duplicate face.
	  for(int j=i+1; j<V->Faces.size(); j++)
	    {
	      Face *F1 = V->Faces[j];
