...
public:
	...
	inline dVector Center() const
  	{
	  return MinV + (MaxV - MinV) * 0.5;
	}

	// Returns true if the point is in the box.
	inline bool Inside(const dVector &P) const
	{
	...
	inline bool SphereIntersect(const dVector &P, const double r) const
	{
	  return (P.x+r < MinV.x || P.y+r < MinV.y || P.z+r < MinV.z || 
		  P.x-r > MaxV.x || P.y-r > MaxV.y || P.z-r > MaxV.z);
	}

	inline BBox& operator+=(const dVector &v)
	{
		if(valid)
		{
