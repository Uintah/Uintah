...
struct Quadric3
{
  ...
  inline void DoSym(const dVector &N, const double D)
  {
    DoSym(N.x, N.y, N.z, D);
  }

  // Compute p * Q * p.
  inline double MulPt(const dVector& p) const
  {
  ...
  inline void MulVec(const dVector& p, double res[4]) const
  {
    // Compute Qp
