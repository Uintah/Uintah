...
void SimpMesh::DoCollapse(Edge *Xe)
{
  ...
  if(Kv->Edges.size() <= 1 || Kv->Faces.size() == 0)
      ...
      for(i=0; i<Kv->Faces.size(); i++)
	{
      ...
      for(int k=0; k<Kv->Edges.size(); k++)
	{
	  Edge *E0 = Kv->Edges[k];
	  for(int l=k+1; l<Kv->Edges.size(); l++)
	    {
	      Edge *E1 = Kv->Edges[l];
