  ...
  for(i=0; i<verts.size(); i+=3, F = F->next)
    {
      ...
      dVector P0 = verts[i] - verts[i+1];
      dVector P1 = verts[i+2] - verts[i+1];

      dVector *N = new dVector(Cross(P1, P0));
      N->normalize();
      ...
      for(int j=0; j<3; j++)
	{
	  ...
	  for(int k=0; k<V->Faces.size(); k++)
	      ...
	      if(V->Faces[k] != F)
		{
		  ...
		  dVector &FNT = *((dVector *)(V->Faces[k]->e0));
...
void Object::MakeTriangles(bool KeepBad)
{
  ...
  vector<dVector> _verts, _normals, _texcoords, _dcolors;
...
//----------------------------------------------------------------------
void Model::RemoveTriangles(dVector& pos, double thresh) {
  int i;
...
//----------------------------------------------------------------------
void Object::RemoveTriangles(dVector& pos, double thresh)
{
  int i, n = verts.size();
