...
//#include <Remote/Modules/remoteSalmon/MiscMath.h>
...
void Matrix44::build_scale(double m[4][4], const dVector& v) const
{
	build_identity(m);
	m[0][0]=v.x;
	m[1][1]=v.y;
...
void Matrix44::build_translate(double m[4][4], const dVector& v) const
{
	build_identity(m);
	m[0][3]=v.x;
	m[1][3]=v.y;
...
void Matrix44::build_rotate(double m[4][4], double angle, const dVector& axis) const
{
...
void Matrix44::LoadFrame(const dVector& x, const dVector& y, const dVector& z)
{
...
void Matrix44::LoadFrame(const dVector& x, const dVector& y, 
						 const dVector& z, const dVector& t)
{
	LoadFrame(x, y, z);
	Translate(t);
}

void Matrix44::GetFrame(dVector &c0, dVector &c1, dVector &c2)
{
...
void Matrix44::GetFrame(dVector &c0, dVector &c1, dVector &c2, dVector &c3)
{
...
void Matrix44::Scale(const dVector& v)
{
	double m[4][4];
	build_scale(m, v);
	if(is_identity)
...
void Matrix44::Rotate(double angle, const dVector& axis)
{
	double m[4][4];
...
void Matrix44::Translate(const dVector& v)
{
	double m[4][4];
	build_translate(m, v);
	if(is_identity)
...
void Matrix44::LookAt(const dVector& eye, const dVector& lookat, const dVector& up)
{
	dVector f(lookat - eye);
	f.normalize();
	
	dVector upn(up);
	upn.normalize();
	
	dVector s(Cross(f, upn));
	s.normalize();
...
dVector Matrix44::Project(const dVector& p) const
{
	...
	return dVector(xw * w1, yw * w1, zw * w1);
}

dVector Matrix44::Project(const dVector& p, const double w) const
{
	// XXX Should I put an optimization here for is_identity?
...
dVector Matrix44::ProjectDirection(const dVector& p) const
{
	// XXX Should I put an optimization here for is_identity?
	
	return dVector(mat[0][0]*p.x+mat[0][1]*p.y+mat[0][2]*p.z,
		mat[1][0]*p.x+mat[1][1]*p.y+mat[1][2]*p.z,
		mat[2][0]*p.x+mat[2][1]*p.y+mat[2][2]*p.z);
}

dVector Matrix44::UnProject(const dVector& p)
{
	// XXX Should I put an optimization here for is_identity?
