...
void Worker::fill_frameless_work(Array1<int>& xpos, Array1<int>& ypos,
  ...
  if (scene->get_rtrt_engine()->framelessMode == 0) {
    // do hilbert curve stuff...
    ...
    int chunksize = uxres*uyres/numChunks;
    // num chunk intervals
    int chunki = numChunks/np;
    ...
    if (chunki*np != numChunks) {
      slopstart = chunksize*chunki*np;
      totslop = numChunks*chunksize - np*chunki*chunksize;
      if (!num) {
        io_lock_.lock();
