...
main(int argc, char **argv) {
  ...
  if (argc != 4 && argc != 5) {
    cerr << "Usage: "<<argv[0]<<" pts tetras [channels] fieldout\n";
    return 0;
  }
  ...
  string cond_name, units;
  double cond_val;
  cout << "What would you like to call the volume conducting material (e.g. saline)? ";
  cin >> cond_name;
  cout << "What is the conductivity value (S/m) of "<<cond_name<<"? ";
  cin >> cond_val;
  cout << "What are the units of the geometry (e.g. mm)? ";
  cin >> units;
  ...
  tvm->set_property("units", units, false);
  TetVolMeshHandle tvmH(tvm);
  TetVolField<int> *tv = scinew TetVolField<int>(tvmH, Field::CELL);
  FieldHandle tvH(tv);
  vector<pair<string, Tensor> > tens;
  tens.resize(1);
  tens[0] = pair<string, Tensor>(cond_name, Tensor(cond_val));
  tv->set_property("conductivity_table", tens, false);
