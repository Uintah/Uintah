...
static const struct whoc_IDPort__external *_ior = NULL;
/*
 * Lookup the symbol to get the IOR functions.
 */

static const struct whoc_IDPort__external* _loadIOR(void)
/*
...

{
  ...
  _ior = whoc_IDPort__externals();
#else
  const struct whoc_IDPort__external*(*dll_f)(void) =
    (const struct whoc_IDPort__external*(*)(void)) SIDL_Loader_lookupSymbol(
      "whoc_IDPort__externals");
  _ior = (dll_f ? (*dll_f)() : NULL);
  if (!_ior) {
    fputs("Babel: unable to load the implementation for whoc.IDPort; please set SIDL_DLL_PATH\n", stderr);
    exit(-1);
  }
#endif
  return _ior;
...
whoc_IDPort__cast(
  void* obj)
{
  whoc_IDPort cast = NULL;

  if (obj != NULL) {
    SIDL_BaseInterface base = (SIDL_BaseInterface) obj;
    cast = (whoc_IDPort) (*base->d_epv->f__cast)(
      base->d_object,
      "whoc.IDPort");
  }
...
whoc_IDPort__array_createCol(int32_t        dimen,
                             ...
                             const int32_t upper[])
{
  return (struct whoc_IDPort__array*)SIDL_interface__array_createCol(dimen,
    lower, upper);
}
...
whoc_IDPort__array_createRow(int32_t        dimen,
                             ...
                             const int32_t upper[])
{
  return (struct whoc_IDPort__array*)SIDL_interface__array_createRow(dimen,
    lower, upper);
}
...
{
  return (struct whoc_IDPort__array*)SIDL_interface__array_create1d(len);
}
...
{
  return (struct whoc_IDPort__array*)SIDL_interface__array_create2dCol(m, n);
}
...
{
  return (struct whoc_IDPort__array*)SIDL_interface__array_create2dRow(m, n);
}
...
{
  return (struct whoc_IDPort__array*)SIDL_interface__array_borrow(
...
{
  return (struct whoc_IDPort__array*)
