...
class PSECORESHARE UCharToFloat : public Module {
public:
  // Declare Ports
  ...
  UCharToFloat(GuiContext*);

  virtual ~UCharToFloat();
...
DECLARE_MAKER(UCharToFloat)
UCharToFloat::UCharToFloat(GuiContext* ctx)
  : Module("UCharToFloat", ctx, Source, "Converters", "Insight")
{
}

UCharToFloat::~UCharToFloat(){
}

template<class InputImageType, class OutputImageType>
bool UCharToFloat::run( itk::Object *obj_InputImage) 
{
...
template<class InputImageType, class OutputImageType>
bool UCharToFloat::run2( itk::Object *obj_InputImage) 
{
  InputImageType *data_InputImage = dynamic_cast<  InputImageType * >(obj_InputImage);
...
void UCharToFloat::execute(){
  ...
  if(0) { }
  else if(run< itk::Image<unsigned char, 2>, itk::Image<float, 2> >( data_InputImage )) {} 
  else if(run< itk::Image<unsigned char, 3>, itk::Image<float, 3> >( data_InputImage )) {} 
  else if(run2< itk::Image<itk::Vector<unsigned char>, 2>, itk::Image<itk::Vector<float>, 2> >( data_InputImage )) {} 
  else if(run2< itk::Image<itk::Vector<unsigned char>, 3>, itk::Image<itk::Vector<float>, 3> >( data_InputImage )) {} 
  else {
    // error
...
void UCharToFloat::tcl_command(GuiArgs& args, void* userdata)
{
