...
#ifndef __TBON_TREE_P_H__
#define __TBON_TREE_P_H__
...
public:
  ...
  TbonTreeP( int nx, int ny, int nz, int nb, int db );
  // constructor for execution
  TbonTreeP( const char* filename, int np, 
  ...
  int search1( double iso, const char* treefile, const char* datafile, 
	       int timechanged, int level, int np, int res );
  void search2( int timechanged );
  void resize( int res, int isocells, int np, int rank );
  GeomTriGroup* search3( double iso, int level, int res, int rank, 
			 int isocells );
  void cleanup();
...
private:
  ...
  int searchFirstPass( double iso, int level );
  void searchSecondPass( int myindex, int rank, int d, double iso, 
			 int x, int y, int z, int res, MCubes<T>* mc );
...
// define constants
template <class T>
const int TbonTreeP<T>::TIME_CHANGED = 0;
template <class T>
const int TbonTreeP<T>::TIME_SAME = 1;

// TbonTreeP - constructor for preprocessing
template <class T>
TbonTreeP<T>::TbonTreeP( int nx, int ny, int nz, int nb, int db ) {
  int idx;
...
// TbonTreeP - constructor for execution
template <class T>
TbonTreeP<T>::TbonTreeP( const char* filename, int np, 
...
// ~TbonTreeP - Destructor
template <class T>
TbonTreeP<T>::~TbonTreeP() {
...
template <class T>
void 
TbonTreeP<T>::readData( const char* filename ) {
...
template <class T>
void 
TbonTreeP<T>::writeTree( const char* meta, const char* base, 
			const char* newdata, int num ) {
  char filename[128];
  int i;
  FILE* out;
...
// search1 - first phase of search - read in nodes and set up work queue
template <class T>
int 
TbonTreeP<T>::search1( double iso, const char* treefile, const char* datafile, 
		       int timechanged, int level, int np, int res ) {
  numProcessors = np;
  ...
  int n = searchFirstPass( iso, depth-level );
  //  cout << "n = " << n  << endl;

  // initialize array of mcube's (1 for each thread)
  static int firsttime = 1;
  int i;
  if( firsttime ) {
    for( i = 0; i < np; i++ ) 
      mcube[i] = new MCubes<T>( data, Xarray, Yarray, Zarray, dx, dy, dz );
    firsttime = 0;
  }

  int currjob = 0;
  assignPoints( 0, depth, 0, 0, 0, depth-level-1, &currjob );

  return n;
} // search1

// search2 - second phase of search - read in data
template <class T>
void 
TbonTreeP<T>::search2( int timechanged ) {
  ...
  if( timechanged == TIME_CHANGED ) {
    while( i < numDataBricks ) {
      ...
      for(; i < numDataBricks && brickstoread[i] == gen; 
	  datainmem[i]=1, i++ );
    ...
    while( i < numDataBricks ) {
      for(; i < numDataBricks && (datainmem[i] || brickstoread[i] != gen); 
	  i++);
      last = i*databricksize;
      fseek( currdata, last, SEEK_SET );
      for(; i < numDataBricks && brickstoread[i] == gen; 
...
template <class T>
void
TbonTreeP<T>::resize( int res, int isocells, int np, int rank ) {
  mcube[rank]->setResolution( res );
  mcube[rank]->reset( isocells/np );
}

// search3 - last phase of search - get jobs from work queue and
//   construct isosurface
template <class T>
GeomTriGroup*
TbonTreeP<T>::search3( double iso, int level, int res, int rank, 
		       int isocells ) {
  int start, end;
  while( workqueue->getWork( start, end ) ) {
    for( int i = start; i < end; i++ ) {
      searchSecondPass( workqueue->jobs[i].index, rank, depth-level-1, iso,
			...
			workqueue->jobs[i].z, res, mcube[rank] ); 
    }
  }
...
template <class T>
void
TbonTreeP<T>::reorderTree( char* branchArray, int* sibArray ) {
  int currdepth;
...
template <class T>
void
TbonTreeP<T>::fill( int myindex, int d, int x, int y, int z, T* min, T* max ) {
  int j;
  T mins[8], maxs[8];
...
template <class T>
void
TbonTreeP<T>::getMinMax( int x, int y, int z, T* min, T* max ) {
  T v;
  int index = Xarray[x] + Yarray[y] + Zarray[z];
...
template <class T>
int
TbonTreeP<T>::searchFirstPass( double iso, int level ) {
  ...
  for( int currdepth = depth; currdepth > 0; currdepth-- ) {
    ...
    if( currdepth == level ) {
      workqueue = new WorkQueue( numProcessors, newsize );
    }
	...
	for( j = 0; j < BRANCHTABLE[ (int)tree[oldqueue[i]].branching ]; j++ ){
	  ...
	  idx++;
	  if( currdepth == level ) 
	    workqueue->jobs[numjobs++].index = tree[oldqueue[i]].child + j; 
	}
...
//   construct isosurface triangles
template <class T> 
void 
TbonTreeP<T>::searchSecondPass( int myindex, int rank, int d, double iso, 
				int x, int y, int z, int res,
				MCubes<T>* mc ) {
  if( tree[ myindex ].min <= iso && tree[ myindex ].max >= iso ) {
    if( d <= res ) {
      mc->interp( x, y, z, (int)tree[ myindex ].branching, iso );
      return;
    }
    int step = 1 << d;
    d--;
    searchSecondPass( tree[myindex].child, rank, d, iso, x, y, z, res, mc );
    switch( (int)tree[ myindex ].branching ) {
    case 0:
      break;
    case 1: {
      searchSecondPass( tree[myindex].child+1, rank, d, iso, 
			x+step, y, z, res, mc );
    } break;
    case 2: {
      searchSecondPass( tree[myindex].child+1, rank, d, iso, 
    ...
    case 3: {
      searchSecondPass( tree[myindex].child+1, rank, d, iso, 
			x+step, y, z, res, mc );
      searchSecondPass( tree[myindex].child+2, rank, d, iso, 
			x, y+step, z, res, mc );
      searchSecondPass( tree[myindex].child+3, rank, d, iso, 
			x+step, y+step, z, res, mc );
    } break;
    case 4: {
      searchSecondPass( tree[myindex].child+1, rank, d, iso, 
			x, y, z+step, res, mc );
    ...
    case 7: {
      ...
      searchSecondPass( tree[myindex].child+3, rank, d, iso, 
			x+step, y+step, z, res, mc );
      searchSecondPass( tree[myindex].child+4, rank, d, iso, 
			x, y, z+step, res, mc );
      searchSecondPass( tree[myindex].child+5, rank, d, iso, 
			x+step, y, z+step, res, mc );
      searchSecondPass( tree[myindex].child+6, rank, d, iso, 
			x, y+step, z+step, res, mc );
      searchSecondPass( tree[myindex].child+7, rank, d, iso, 
			x+step, y+step, z+step, res, mc );
    } break;
    }; // switch
  } // if node spans iso
} // searchSecondPass
...
template <class T> 
void 
TbonTreeP<T>::assignPoints( int myindex, int d, int x, int y, int z, 
			    int level, int* currjob ) {
  if( d == level ) {
    if( workqueue->jobs[*currjob].index == myindex ) {
      workqueue->jobs[*currjob].x = x;
      workqueue->jobs[*currjob].y = y;
      workqueue->jobs[*currjob].z = z;
      *currjob = *currjob + 1;
    }
    return;
  }
  ...
  int step = 1 << d;
  d--;
  assignPoints( tree[myindex].child, d, x, y, z, level, currjob );
  switch( (int)tree[myindex].branching ) {
  case 0:
    break;
  case 1: {
    assignPoints( tree[myindex].child+1, d, x+step, y, z, level, currjob );
  } break;
  case 2: {
    assignPoints( tree[myindex].child+1, d, x, y+step, z, level, currjob );
  } break;
  case 3: {
    assignPoints( tree[myindex].child+1, d, x+step, y, z, level, currjob );
    assignPoints( tree[myindex].child+2, d, x, y+step, z, level, currjob );
    assignPoints( tree[myindex].child+3, d, x+step, y+step, z, level, currjob);
  } break;
  case 4: {
    assignPoints( tree[myindex].child+1, d, x, y, z+step, level, currjob );
  } break;
  case 5: {
    assignPoints( tree[myindex].child+1, d, x+step, y, z, level, currjob );
    assignPoints( tree[myindex].child+2, d, x, y, z+step, level, currjob );
    assignPoints( tree[myindex].child+3, d, x+step, y, z+step, level, currjob);
  } break;
  case 6: {
    assignPoints( tree[myindex].child+1, d, x, y+step, z, level, currjob );
    assignPoints( tree[myindex].child+2, d, x, y, z+step, level, currjob );
    assignPoints( tree[myindex].child+3, d, x, y+step, z+step, level, currjob);
  } break;
  case 7: {
    assignPoints( tree[myindex].child+1, d, x+step, y, z, level, currjob );
    assignPoints( tree[myindex].child+2, d, x, y+step, z, level, currjob );
    assignPoints( tree[myindex].child+3, d, x+step, y+step, z, level, currjob);
    assignPoints( tree[myindex].child+4, d, x, y, z+step, level, currjob );
    assignPoints( tree[myindex].child+5, d, x+step, y, z+step, level, currjob);
    assignPoints( tree[myindex].child+6, d, x, y+step, z+step, level, currjob);
    assignPoints( tree[myindex].child+7, d, x+step, y+step, z+step, level, 
		  currjob );
  } break;
  }; // switch
