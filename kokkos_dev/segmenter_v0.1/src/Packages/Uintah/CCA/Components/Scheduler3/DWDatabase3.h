#ifndef UINTAH_HOMEBREW_DWDatabase3_H
#define UINTAH_HOMEBREW_DWDatabase3_H
...
using std::ostringstream;
 ...
 template<class DomainType>
   ...
   void initializeScrubs(int dwid, const map<VarLabelMatlDW<DomainType>, int>& scrubcounts);
...
private:
   ...
   public:
      ...
      void putVar(int matlIndex, Variable* var, bool replace);

      inline DataItem* getDataItem(int matlIndex);
      inline Variable* getVar(int matlIndex) const;
   ...
   DWDatabase3(const DWDatabase3&);
   DWDatabase3& operator=(const DWDatabase3&);      
};

template<class DomainType>
DWDatabase3<DomainType>::DWDatabase3()
{
}
...
template<class DomainType>
void
DWDatabase3<DomainType>::cleanForeign()
{
  for(typename nameDBtype::iterator iter = names.begin();
      iter != names.end(); iter++){
	...
	const Variable* var = pr->getVars()[m].var;
...
template<class DomainType>
void
DWDatabase3<DomainType>::scrub(const VarLabel* var, int matlIndex,
				       const DomainType* dom)
{
  ASSERT(matlIndex >= -1);
...
template<class DomainType>
void
DWDatabase3<DomainType>::initializeScrubs(int dwid,
...
template<class DomainType>
DWDatabase3<DomainType>::NameRecord::NameRecord(const VarLabel* label)
   : label(label)
{
}

template<class DomainType>
DWDatabase3<DomainType>::NameRecord::~NameRecord()
{
...
template<class DomainType>
DWDatabase3<DomainType>::DomainRecord::DomainRecord(const DomainType* dom)
  : dom(dom), count(0)
{
}
...
template <class DomainType>
void
DWDatabase3<DomainType>::DomainRecord::putVar( int matlIndex,
						       Variable* var,
						       bool replace )
{
  ...
  Variable* oldVar = vars[matlIndex+1].var;
...
template <class DomainType>
inline
typename DWDatabase3<DomainType>::DataItem*
DWDatabase3<DomainType>::DomainRecord::getDataItem(int matlIndex)
{
  if (matlIndex+1 < (int)vars.size())
...
inline
Variable*
DWDatabase3<DomainType>::DomainRecord::getVar(int matlIndex) const
{
  if (matlIndex+1 < (int)vars.size())
...
inline
void
DWDatabase3<DomainType>::DomainRecord::removeVar(int matlIndex)
{
...
template<class DomainType>
DWDatabase3<DomainType>::DomainRecord::~DomainRecord()
{
...
template<class DomainType>
bool DWDatabase3<DomainType>::exists(const VarLabel* label, int matlIndex,
				 const DomainType* dom) const
{
  int domainid = getDB_ID(dom);
...
/*
template<class DomainType>
bool DWDatabase3<DomainType>::exists(const VarLabel* label, const DomainType* dom) const
{
  int domainid = getDB_ID(dom);
  typename nameDBtype::const_iterator nameiter = names.find(label);
  if(nameiter != names.end()) {
...
template<class DomainType>
void
DWDatabase3<DomainType>::put( const VarLabel* label, 
				      int matlIndex,
				      const DomainType* dom,
				      Variable* var,
				      bool replace )
...
template<class DomainType>
const typename DWDatabase3<DomainType>::DataItem&
DWDatabase3<DomainType>::getDataItem( const VarLabel* label,
					      int matlIndex,
					      const DomainType* dom ) const
{
  ASSERT(matlIndex+1 >= 0);
...
inline
Variable*
DWDatabase3<DomainType>::get( const VarLabel* label,
				      int matlIndex,
				      const DomainType* dom ) const
{
...
template<class DomainType>
void DWDatabase3<DomainType>::print(std::ostream& out) const
{
...
template<class DomainType>
void
DWDatabase3<DomainType>::logMemoryUse(ostream& out, unsigned long& total,
				  const std::string& tag, int dwid)
{
  for(typename nameDBtype::iterator iter = names.begin();
      iter != names.end(); iter++){
...
template<class DomainType>
void
DWDatabase3<DomainType>::getVarLabelMatlTriples( vector<VarLabelMatl<DomainType> >& vars ) const
{
  for(typename nameDBtype::const_iterator nameiter = names.begin();
      nameiter != names.end(); nameiter++){
	...
	if(domainRecord->getVars()[i].var){
	  vars.push_back(VarLabelMatl<Level>(nameRecord->label, i-1, domainRecord->getDomain() ));
	}
      }
    }
  }
