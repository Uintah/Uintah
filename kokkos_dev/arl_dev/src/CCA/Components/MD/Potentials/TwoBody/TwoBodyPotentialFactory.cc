...
NonbondedTwoBodyPotential* NonbondedTwoBodyFactory::create(forcefieldType forcefieldSource,
    ...
    if (forcefieldSource == Lucretius) {
      if (potentialType == "exp-6") {
        double A = Parse::stringToDouble(potentialArguments[0]);
        double B = Parse::stringToDouble(potentialArguments[1]);
        double C = Parse::stringToDouble(potentialArguments[2]);
    ...
    else if (forcefieldSource == Generic ) { // Potential types defined for a generic, unspecified forcefield
