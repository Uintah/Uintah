...
! 3   call NVE_VV(stage=2)
 ...
 use ALL_atoms_data, only : xxx,yyy, zzz, vxx,vyy,vzz,fxx,fyy,fzz,Natoms,all_atoms_mass,l_proceed_kin_atom 
...
!   EXTERNAL FIELD CORRECTIONS HERE 
    ...
    enddo
    if (Nconstrains>0) call shake_vv_2
    call get_kinetic_energy_stress(T_eval)
    call add_kinetic_pressure

    T_imposed = temperature
    csi=dsqrt(1.d0+(time_step/thermo_coupling)*(T_imposed/T_eval-1.d0))
    do i = 1, Natoms
     if(l_proceed_kin_atom(i)) then
         vxx(i)=vxx(i)*csi
         vyy(i)=vyy(i)*csi
 ...
 CASE default
   print*, 'ERRROR in NVT_B_VV; the variable stage can only be 1 or 2 and cannot be its actual value',stage
   STOP
 end SELECT
