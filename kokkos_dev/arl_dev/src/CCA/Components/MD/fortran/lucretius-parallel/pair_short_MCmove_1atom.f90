...
module pair_short_MCmove_1atom


contains 

!------------------
subroutine pair_short_Q_MCmove_1atom(iwhich)
 use sys_data
 ...
 use profiles_data
 use d_energies_data, only : d_en_Qreal,d_en_vdw,d_En_Q,d_en_induced_dip
 use stresses_data
 use atom_type_data
 ...
 use compute_14_module, only : compute_14_interactions_driver_MCmove_1atom
...
!  call nonbonded_vdw_2_forces(i,iStyle,size_list_nonbonded(i)) ! put it back
  ...
  qi = all_charges(iwhich) ! electricity = charge no matter of what kind.
...
!    call Q_2_forces(i,iStyle,neightot)
  ...
  call compute_14_interactions_driver_MCmove_1atom(iwhich)
!  call REAL_part_intra_correction_sfc_sfc_driver_MCmove_1atom(iwhich) !see it latter
local_energy = 0.0d0
do i = 1, Natoms
...
enddo
d_en_Qreal = d_en_Qreal + local_energy
d_En_Q = d_En_Q + d_en_Qreal
d_en_induced_dip = local_energy

 deallocate(dr_sq)
 deallocate(dx,dy,dz)
 deallocate(in_list_Q)

end subroutine pair_short_Q_MCmove_1atom
...
!------------
 ...
 subroutine pair_short_vdw_MCmove_1atom(iwhich) ! do only vdw
  use sys_data
  use paralel_env_data
  use math_constants
  use boundaries
  ...
  use non_bonded_lists_data, only : list_nonbonded, size_list_nonbonded
  use profiles_data
  use d_energies_data, only : d_en_vdw
  use stresses_data
  use atom_type_data
  use interpolate_data
  use variables_short_pairs
 ...
 use compute_14_module, only : compute_14_interactions_vdw_MCmove_1atom
...
!    call nonbonded_vdw_2_forces(i,istyle,neightot)
  ...
  if (N_pairs_14>0.and.l_red_14_vdw_CTRL) call compute_14_interactions_vdw_MCmove_1atom(iwhich)


  deallocate(dr_sq)
  deallocate(dx,dy,dz)
  deallocate(in_list_Q)
end  subroutine pair_short_vdw_MCmove_1atom
...
end module pair_short_MCmove_1atom
