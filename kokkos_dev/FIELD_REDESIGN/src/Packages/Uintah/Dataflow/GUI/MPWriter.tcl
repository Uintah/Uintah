...
itcl_class Uintah_Writers_MPWriter { 
    inherit Module 
	...
	frame $w.ff -relief flat
	pack $w.ff -side top -expand yes -fill both -pady 2
	...
	button $w.ff.b -text Save -command "$this SaveFile"
	pack $w.ff.b -expand yes -fill x 
    }
	...
	if { $savefile != "" } {
	    set isBin [tk_dialog $w.dialog "bin check" \
			   "Should this file be saved as a binary file?" \
			   "" 1 no yes]
	    $this-c save $savefile $isBin
	}
    }
    method Visible {} {
