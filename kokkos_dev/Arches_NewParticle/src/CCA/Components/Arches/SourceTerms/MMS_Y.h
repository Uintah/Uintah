#ifndef Uintah_Component_Arches_MMS_Y_h
#define Uintah_Component_Arches_MMS_Y_h
...
class MMS_Y: public SourceTermBase {
public: 

  MMS_Y( std::string srcName, SimulationStateP& shared_state, 
  ...
  static double evaluate_MMS_source( double x,
    ...
    double source;
    double pi = MMS_Y::pi;
    source = -4.0*pi*MMS_Y::d_vvel*(1/domain_size.y());
    source *= sin( 4.0*pi*(x/domain_size.x()) );
    source *= sin( 4.0*pi*(y/domain_size.y()) );
    return source;
  }
  ...
  class Builder : public SourceTermBase::Builder {
    public:
      ...
      ~Builder(){};
      MMS_Y* build() { 
        return scinew MMS_Y( _name, _shared_state, _required_label_names ); 
      }
    private:
      std::string _name;
