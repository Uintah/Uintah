...
/*
 *  ScalarFieldRGchar.cc: char Scalar Field defined on a Regular grid
 *
 *  Written by:
...
{
    return scinew ScalarFieldRGchar;
}

PersistentTypeID ScalarFieldRGchar::type_id("ScalarFieldRGchar", "ScalarField", maker);

ScalarFieldRGchar::ScalarFieldRGchar()
: ScalarFieldRGBase("char")
{
}

ScalarFieldRGchar::ScalarFieldRGchar(const ScalarFieldRGchar& copy)
...
void ScalarFieldRGchar::resize(int x, int y, int z) {
    nx=x; ny=y; nz=z;
    grid.newsize(x,y,z);
}

#define ScalarFieldRGchar_VERSION 3

void ScalarFieldRGchar::io(Piostream& stream)
{
    using SCICore::PersistentSpace::Pio;
    using SCICore::Containers::Pio;
    using SCICore::Geometry::Pio;

    int version=stream.begin_class("ScalarFieldRGchar", ScalarFieldRGchar_VERSION);
    if(version == 1){
...
void ScalarFieldRGchar::compute_minmax()
{
...
Vector ScalarFieldRGchar::gradient(const Point& p)
{
...
int ScalarFieldRGchar::interpolate(const Point& p, double& value, int&,
				   double epsilon1, double epsilon2,
				   int) {
    return interpolate(p, value, epsilon1, epsilon2);
}

int ScalarFieldRGchar::interpolate(const Point& p, double& value, double eps,
				   double)
{
    using SCICore::Math::Interpolate;
...
ScalarField* ScalarFieldRGchar::clone()
{
    return scinew ScalarFieldRGchar(*this);
}

Vector
ScalarFieldRGchar::get_normal( const Point& ivoxel )
{
  return get_normal( ivoxel.x(), ivoxel.y(), ivoxel.z() );
}

Vector
ScalarFieldRGchar::get_normal( int x, int y, int z )
{
  Vector normal;
...
Vector 
ScalarFieldRGchar::gradient(int x, int y, int z)
{
...
void ScalarFieldRGchar::fill_gradmags() // these guys ignor the vf
{
...
//
// replaced ScalarFieldRGxxx with ScalarFieldRGchar. it also replaces
