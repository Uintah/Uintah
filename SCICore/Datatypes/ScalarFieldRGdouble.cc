...
/*
 *  ScalarFieldRGdouble.cc: double Scalar Field defined on a Regular grid
 *
 *  Written by:
...
{
    return scinew ScalarFieldRGdouble;
}

PersistentTypeID ScalarFieldRGdouble::type_id("ScalarFieldRGdouble", "ScalarField", maker);

ScalarFieldRGdouble::ScalarFieldRGdouble()
: ScalarFieldRGBase("double")
{
}

ScalarFieldRGdouble::ScalarFieldRGdouble(const ScalarFieldRGdouble& copy)
...
void ScalarFieldRGdouble::resize(int x, int y, int z) {
    nx=x; ny=y; nz=z;
    grid.newsize(x,y,z);
}

#define ScalarFieldRGdouble_VERSION 3

void ScalarFieldRGdouble::io(Piostream& stream)
{
    using SCICore::PersistentSpace::Pio;
    using SCICore::Containers::Pio;
    using SCICore::Geometry::Pio;

    int version=stream.begin_class("ScalarFieldRGdouble", ScalarFieldRGdouble_VERSION);
    if(version == 1){
...
void ScalarFieldRGdouble::compute_minmax()
{
...
Vector ScalarFieldRGdouble::gradient(const Point& p)
{
...
int ScalarFieldRGdouble::interpolate(const Point& p, double& value, int&,
				   double epsilon1, double epsilon2,
				   int) {
    return interpolate(p, value, epsilon1, epsilon2);
}

int ScalarFieldRGdouble::interpolate(const Point& p, double& value, double eps,
				   double)
{
    using SCICore::Math::Interpolate;
...
ScalarField* ScalarFieldRGdouble::clone()
{
    return scinew ScalarFieldRGdouble(*this);
}

Vector
ScalarFieldRGdouble::get_normal( const Point& ivoxel )
{
  return get_normal( ivoxel.x(), ivoxel.y(), ivoxel.z() );
}

Vector
ScalarFieldRGdouble::get_normal( int x, int y, int z )
{
  Vector normal;
...
Vector 
ScalarFieldRGdouble::gradient(int x, int y, int z)
{
...
void ScalarFieldRGdouble::fill_gradmags() // these guys ignor the vf
{
...
//
// replaced ScalarFieldRGxxx with ScalarFieldRGdouble. it also replaces
