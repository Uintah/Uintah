...
/*
 *  ScalarFieldRGushort.cc: ushort Scalar Field defined on a Regular grid
 *
 *  Written by:
...
{
    return scinew ScalarFieldRGushort;
}

PersistentTypeID ScalarFieldRGushort::type_id("ScalarFieldRGushort", "ScalarField", maker);

ScalarFieldRGushort::ScalarFieldRGushort()
: ScalarFieldRGBase("ushort")
{
}

ScalarFieldRGushort::ScalarFieldRGushort(const ScalarFieldRGushort& copy)
...
void ScalarFieldRGushort::resize(int x, int y, int z) {
    nx=x; ny=y; nz=z;
    grid.newsize(x,y,z);
}

#define ScalarFieldRGushort_VERSION 3

void ScalarFieldRGushort::io(Piostream& stream)
{
    using SCICore::PersistentSpace::Pio;
    using SCICore::Containers::Pio;
    using SCICore::Geometry::Pio;

    int version=stream.begin_class("ScalarFieldRGushort", ScalarFieldRGushort_VERSION);
    if(version == 1){
...
void ScalarFieldRGushort::compute_minmax()
{
...
Vector ScalarFieldRGushort::gradient(const Point& p)
{
...
int ScalarFieldRGushort::interpolate(const Point& p, double& value, int&,
				   double epsilon1, double epsilon2,
				   int) {
    return interpolate(p, value, epsilon1, epsilon2);
}

int ScalarFieldRGushort::interpolate(const Point& p, double& value, double eps,
				   double)
{
    using SCICore::Math::Interpolate;
...
ScalarField* ScalarFieldRGushort::clone()
{
    return scinew ScalarFieldRGushort(*this);
}

Vector
ScalarFieldRGushort::get_normal( const Point& ivoxel )
{
  return get_normal( ivoxel.x(), ivoxel.y(), ivoxel.z() );
}

Vector
ScalarFieldRGushort::get_normal( int x, int y, int z )
{
  Vector normal;
...
Vector 
ScalarFieldRGushort::gradient(int x, int y, int z)
{
...
void ScalarFieldRGushort::fill_gradmags() // these guys ignor the vf
{
...
//
// replaced ScalarFieldRGxxx with ScalarFieldRGushort. it also replaces
