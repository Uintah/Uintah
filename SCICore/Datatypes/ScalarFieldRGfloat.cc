...
/*
 *  ScalarFieldRGfloat.cc: float Scalar Field defined on a Regular grid
 *
 *  Written by:
...
{
    return scinew ScalarFieldRGfloat;
}

PersistentTypeID ScalarFieldRGfloat::type_id("ScalarFieldRGfloat", "ScalarField", maker);

ScalarFieldRGfloat::ScalarFieldRGfloat()
: ScalarFieldRGBase("float")
{
}

ScalarFieldRGfloat::ScalarFieldRGfloat(const ScalarFieldRGfloat& copy)
...
void ScalarFieldRGfloat::resize(int x, int y, int z) {
    nx=x; ny=y; nz=z;
    grid.newsize(x,y,z);
}

#define ScalarFieldRGfloat_VERSION 2

void ScalarFieldRGfloat::io(Piostream& stream)
{
    using SCICore::PersistentSpace::Pio;
    using SCICore::Containers::Pio;
    using SCICore::Geometry::Pio;

    int version=stream.begin_class("ScalarFieldRGfloat", ScalarFieldRGfloat_VERSION);
    if(version == 1){
...
void ScalarFieldRGfloat::compute_minmax()
{
...
Vector ScalarFieldRGfloat::gradient(const Point& p)
{
...
int ScalarFieldRGfloat::interpolate(const Point& p, double& value, int&,
				   double epsilon1, double epsilon2,
				   int) {
    return interpolate(p, value, epsilon1, epsilon2);
}

int ScalarFieldRGfloat::interpolate(const Point& p, double& value, double,
				   double)
{
    using SCICore::Math::Interpolate;
...
ScalarField* ScalarFieldRGfloat::clone()
{
    return scinew ScalarFieldRGfloat(*this);
}

Vector
ScalarFieldRGfloat::get_normal( const Point& ivoxel )
{
  return get_normal( ivoxel.x(), ivoxel.y(), ivoxel.z() );
}

Vector
ScalarFieldRGfloat::get_normal( int x, int y, int z )
{
  Vector normal;
...
Vector 
ScalarFieldRGfloat::gradient(int x, int y, int z)
{
...
void ScalarFieldRGfloat::fill_gradmags() // these guys ignor the vf
{
...
// Initial commit
//
// Revision 1.3  1999/07/07 21:10:41  dav
