...
static DebugStream dbg("TaskGraph3", false);
...
TaskGraph3::TaskGraph3(Scheduler3Common* sc, const ProcessorGroup* pg)
  : sc(sc), d_myworld(pg)
{
}

TaskGraph3::~TaskGraph3()
{
  initialize(); // Frees all of the memory...
}

void
TaskGraph3::initialize()
{
...
bool
TaskGraph3::overlaps(Task::Dependency* comp, Task::Dependency* req) const
{
...
void
TaskGraph3::setupTaskConnections(GraphSortInfo3Map& sortinfo)
{
  vector<Task*>::iterator iter;
  // Initialize variables on the tasks
  for( iter=d_tasks.begin(); iter != d_tasks.end(); iter++ ) {
    sortinfo[*iter] = GraphSortInfo3();
  }    
  if (edges.size() > 0) {
  ...
  for( iter=d_tasks.begin(); iter != d_tasks.end(); iter++ ) {
	...
	if(it == reductionTasks.end()){
	  ...
	  Task* newtask = scinew Task(taskname.str(), Task::Reduction);
          ...
          sortinfo[newtask] = GraphSortInfo3();
  ...
  // Initialize variables on the tasks
  GraphSortInfo3Map::iterator sort_iter;
...
void TaskGraph3::addDependencyEdges(Task* task, GraphSortInfo3Map& sortinfo,
                                   Task::Dependency* req,
...
void
TaskGraph3::processTask(Task* task, vector<Task*>& sortedTasks,
                       GraphSortInfo3Map& sortinfo) const
{
  if(dbg.active())
    dbg << d_myworld->myrank() << " Looking at task: " << task->getName() << '\n';

  GraphSortInfo3& gsi = sortinfo.find(task)->second;
  if(gsi.visited){
    ostringstream error;
...
void TaskGraph3::processDependencies(Task* task, Task::Dependency* req,
...

{
  for(; req != 0; req=req->next){
	...
	Task* vtask = edge->comp->task;
        GraphSortInfo3& gsi = sortinfo.find(vtask)->second;
	if(!gsi.sorted){
	  if(gsi.visited){
	    ostringstream error;
...
void
TaskGraph3::nullSort( vector<Task*>& tasks )
{
  GraphSortInfo3Map sortinfo;
...
void
TaskGraph3::topologicalSort(vector<Task*>& sortedTasks)
{
  GraphSortInfo3Map sortinfo;
...
void
TaskGraph3::addTask(Task* task, const PatchSet* patchset,
...
void
TaskGraph3::createDetailedTask(DetailedTasks3* tasks, Task* task,
			      const PatchSubset* patches,
			      const MaterialSubset* matls)
{
  DetailedTask3* dt = scinew DetailedTask3(task, patches, matls, tasks);
...
DetailedTasks3*
TaskGraph3::createDetailedTasks( LoadBalancer* lb, bool useInternalDeps )
{
  ...
  DetailedTasks3* dt = scinew DetailedTasks3(sc, d_myworld, this, useInternalDeps );
...
class CompTable3 {
  struct Data {
    ...
    Data(DetailedTask3* task, Task::Dependency* comp,
 ...
 public:
  ...
  bool findcomp(Task::Dependency* req, const Patch* patch, int matlIndex,
		DetailedTask3*& dt, Task::Dependency*& comp,
                const ProcessorGroup* pg);
private:
...
TaskGraph3::createDetailedDependencies(DetailedTasks3* dt, LoadBalancer* lb)
{
  ...
  DetailedTask3* lastReductionTask = 0;
  for(int i=0;i<dt->numTasks();i++){
    DetailedTask3* task = dt->getTask(i);
			      ...
			      CompTable3& ct)
{
  ...
  for(;comp != 0; comp = comp->next){
	...
	DetailedTask3* reductionTask = d_reductionTasks[comp->var];
				      ...
				      bool modifies)
{
  ...
  for( ; req != 0; req = req->next){
	  ...
	  if (neighbor->isVirtual()) {
	    ...
	    for(int m=0;m<matls->size();m++){
	      ...
	      if (modifies) {
	        ...
	        list<DetailedTask3*> requireBeforeModifiedTasks;
	        creator->findRequiringTasks(req->var,
					    requireBeforeModifiedTasks);

	        list<DetailedTask3*>::iterator reqTaskIter;
	...
	if(task->getAssignedResourceIndex() ==
      ...
      ostringstream desc;
      desc << "TaskGraph3::createDetailedDependencies, task dependency not supported without patches and materials"
...
int TaskGraph3::findVariableLocation(LoadBalancer* lb,
				    Task::Dependency* req,
				    const Patch* patch, int matl)
{
...
int TaskGraph3::getNumTasks() const
{
  return (int)d_tasks.size();
}

Task* TaskGraph3::getTask(int idx)
{
  return d_tasks[idx];
}

TaskGraph3::VarLabelMaterialMap* TaskGraph3::makeVarLabelMaterialMap()
{
