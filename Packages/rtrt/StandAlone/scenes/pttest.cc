...
// Returns whether the object was added or not.  This is based on if
// the texture was found.
bool add_sphere(char *tex_name, const Point& center, Group *group) {
...
Group *make_geometry(char* tex_names[NUM_TEXTURES])
{
  ...
  add_sphere(tex_names[0], Point(-1,-1,-1), group);
  add_sphere(tex_names[1], Point(-1, 1,-1), group);
  add_sphere(tex_names[2], Point( 1, 1,-1), group);
  add_sphere(tex_names[3], Point( 1,-1,-1), group);

  add_sphere(tex_names[4], Point(-1,-1, 1), group);
  add_sphere(tex_names[5], Point(-1, 1, 1), group);
  add_sphere(tex_names[6], Point( 1, 1, 1), group);
  add_sphere(tex_names[7], Point( 1,-1, 1), group);
