...
/*
 *  StructHexVolMesh.h: Templated Mesh defined on an 3D Irregular Grid
 *
 *  Written by:
 *   Michael Callahan
...
public:
  ...
  void get_weights(const Point &, Edge::array_type &, vector<double> &) {ASSERTFAIL("StructHexVolMesh::get_weights for edges isn't supported");}
  void get_weights(const Point &, Face::array_type &, vector<double> &) {ASSERTFAIL("StructHexVolMesh::get_weights for faces isn't supported");}
  void get_weights(const Point &p, Cell::array_type &l, vector<double> &w);

  void get_point(Point &p, const Node::index_type &i) const
  { get_center(p, i); }
  void set_point(const Node::index_type &i, const Point &p);
