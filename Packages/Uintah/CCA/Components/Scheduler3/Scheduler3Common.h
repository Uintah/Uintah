...
#ifndef UINTAH_HOMEBREW_SCHEDULERCOMMON3_H
#define UINTAH_HOMEBREW_SCHEDULERCOMMON3_H
...
****************************************/
  ...
  class Scheduler3Common : public Scheduler, public UintahParallelComponent {
  public:
    Scheduler3Common(const ProcessorGroup* myworld, Output* oport);
    virtual ~Scheduler3Common();
  ...
  protected:
    ...
    void makeTaskGraphDoc(const DetailedTasks3* dt,
			  int rank = 0);
    void emitNode(const DetailedTask3* dt, double start, double duration,
  ...
  private:
    ...
    Scheduler3Common(const Scheduler3Common&);
    Scheduler3Common& operator=(const Scheduler3Common&);
