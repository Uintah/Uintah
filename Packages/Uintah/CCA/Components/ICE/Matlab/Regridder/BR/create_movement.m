function d = create_movement(type,varargin)
%CREATE_MOVEMENT Create object handle for a time-stepping test.
%
%   D = CREATE_MOVEMENT('LINE',[x1 y1 x2 y2]) creates a 2D box. Lower left
%       corner is (x1,y1), upper right is (x2,y2).
%   D = CREATE_MOVEMENT('CIRC',[x y],RADIUS,THETA0,DTHETA) creates a movement along a sphere of radius
...
%       (per one time-step).
%   
%   See also CREATE_OBJECT, TEST_CASE, TEST_MOVEMENT.
