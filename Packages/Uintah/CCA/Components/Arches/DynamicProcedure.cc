						...
						const MaterialSet* matls)
{
...
#ifdef PetscFilter
  {
    ...
    Task* tsk = scinew Task("DynamicProcedure::initFilter",
			  this,
			  &DynamicProcedure::initFilter);
  ...
  {
    string taskname =  "DynamicProcedure::computeTurbSubmodel";
  ...
  {
    string taskname =  "DynamicProcedure::computeFilterValues";
    Task* tsk = scinew Task(taskname, this,
			    &DynamicProcedure::computeFilterValues);
  ...
  {
    string taskname =  "DynamicProcedure::computeSmagCoeff";
    Task* tsk = scinew Task(taskname, this,
			    &DynamicProcedure::computeSmagCoeff);
...
//****************************************************************************
void
DynamicProcedure::initFilter(const ProcessorGroup* pg,
...
//****************************************************************************
void 
DynamicProcedure::computeTurbSubmodel(const ProcessorGroup*,
				      const PatchSubset* patches,
				      const MaterialSubset*,
				      DataWarehouse*,
				      DataWarehouse* new_dw)
{
  for (int p = 0; p < patches->size(); p++) {
	...
	for (int colX = startX; colX < endX; colX ++) {
	  ...
	  (SIJ[0])[currCell] = pow(-1.0,colX+colY+colZ);
	}
      }
    }
    (SIJ[0]).print(cout);
...
//****************************************************************************
void 
DynamicProcedure::computeFilterValues(const ProcessorGroup* pc,
				      const PatchSubset* patches,
				      const MaterialSubset*,
				      DataWarehouse*,
				      DataWarehouse* new_dw)
...
//    IshatI.initialize(0.0);
	...
	for (int colX = startX; colX < endX; colX ++) {
	  ...
	  if (Abs((SHATIJ[0])[currCell]) < 1.0e-14)
	  (SHATIJ[0])[currCell] = 0.0;
	}
      }
    }
    (SHATIJ[0]).print(cout);
...
void 
DynamicProcedure::computeSmagCoeff(const ProcessorGroup* pc,
				   const PatchSubset* patches,
				   const MaterialSubset*,
				   DataWarehouse*,
				   DataWarehouse* new_dw)
...
//  double time = d_lab->d_sharedState->getElapsedTime();
  for (int p = 0; p < patches->size(); p++) {
    ...
    IntVector idxLo = patch->getGhostCellLowIndex(Arches::ONEGHOSTCELL);
    IntVector idxHi = patch->getGhostCellHighIndex(Arches::ONEGHOSTCELL);
    Array3<double> MLHatI(idxLo, idxHi); // magnitude of strain rate
    MLHatI.initialize(0.0);
    Array3<double> MMHatI(idxLo, idxHi); // magnitude of test filter strain rate
