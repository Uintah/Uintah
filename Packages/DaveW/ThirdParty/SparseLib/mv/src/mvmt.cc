...
unsigned int MV_ColMat_$TYPE::dim(int i) const 
{
    if (i==0) return dim0_;
    if (i==1) return dim1_;
    else
...
MV_ColMat_$TYPE::MV_ColMat_$TYPE()  : v_(), dim0_(0), dim1_(0) , lda_(0), ref_(0){}
                                                                

MV_ColMat_$TYPE::MV_ColMat_$TYPE(unsigned int m, unsigned int n) : v_(m*n),
        dim0_(m), dim1_(n), lda_(m), ref_(0) {}

MV_ColMat_$TYPE::MV_ColMat_$TYPE(unsigned int m, unsigned int n, const $TYPE &s) : v_(m*n),
        dim0_(m), dim1_(n), lda_(m), ref_(0) 
{
    operator=(s);
}
...
MV_ColMat_$TYPE& MV_ColMat_$TYPE::operator=(const $TYPE & s) 
{
    int M = dim(0);
    int N = dim(1);
...
MV_ColMat_$TYPE& MV_ColMat_$TYPE::newsize(unsigned int M, unsigned int N)
{
    v_.newsize(M*N);
    dim0_ = M;
    dim1_ = N;
...
MV_ColMat_$TYPE& MV_ColMat_$TYPE::operator=(const MV_ColMat_$TYPE & m) 
{
...
MV_ColMat_$TYPE::MV_ColMat_$TYPE(const MV_ColMat_$TYPE & m) : 
...
MV_ColMat_$TYPE::MV_ColMat_$TYPE($TYPE* d, unsigned int m, unsigned int n) :
...
const MV_ColMat_$TYPE MV_ColMat_$TYPE::operator()(const MV_VecIndex &I, 
...
MV_ColMat_$TYPE::~MV_ColMat_$TYPE() {}

ostream&   operator<<(ostream& s, const MV_ColMat_$TYPE& V)
{
    int M = V.dim(0);
    int N = V.dim(1);
