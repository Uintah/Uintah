...
/*
 *  AssignLeadField: Assign a leadfield solution vector to the field fdata
 *
 *  Written by:
 *   David Weinstein
...
class AssignLeadField : public Module
{
  FieldIPort *ifp;
  MatrixIPort *imp;
  FieldOPort *ofp;
  FieldOPort *ofp2;
public:
  AssignLeadField(const string& id);
  virtual ~AssignLeadField();
  virtual void execute();
};


extern "C" Module* make_AssignLeadField(const string& id)
{
  return new AssignLeadField(id);
}

AssignLeadField::AssignLeadField(const string& id)
  : Module("AssignLeadField", id, Filter, "LeadField", "DaveW")
{
...
AssignLeadField::~AssignLeadField()
{
}

void
AssignLeadField::execute()
{
  // Get input field.
  FieldHandle ifield;
