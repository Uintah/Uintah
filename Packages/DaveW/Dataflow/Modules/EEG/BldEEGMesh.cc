...
public:
    ...
    void reorderPts(Mesh *m, Array1<int>& scalpBCMeshNodes, 
...
void BldEEGMesh::genPtsAndTets(SegFldHandle sf, SurfTree *st, int num, 
    ...
    int i;
    for (i=0; i<inum; i++, curr.x(curr.x()+dx)) {
	curr.y(min.y());
...
//    Point v0, v1, v2, v3;
    ...
    int i;
    for (i=0; i<st->surfI[greyMatlIdx].inner.size(); i++) {
...
//		        cerr << "another of type 6, ";
			popularity[6] = popularity[6]+1;
		        ...
		        int k;
			for (k=0; k<st->surfI[greyMatlIdx].inner.size(); k++) {
...
//			    cerr << "grey type 6, ";
			    ...
			    for (k=0; k<st->surfI[whiteMatlIdx].inner.size(); k++) {
...
void BldEEGMesh::reorderPts(Mesh *m, Array1<int>& scalpBCMeshNodes, 
			    Array1<int>& cortexBCMeshNodes) {
