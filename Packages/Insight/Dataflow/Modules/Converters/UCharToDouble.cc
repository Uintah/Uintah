...
class PSECORESHARE UCharToDouble : public Module {
...
public:
  UCharToDouble(GuiContext*);

  virtual ~UCharToDouble();
...
DECLARE_MAKER(UCharToDouble)
UCharToDouble::UCharToDouble(GuiContext* ctx)
  : Module("UCharToDouble", ctx, Source, "Converters", "Insight")
{
}

UCharToDouble::~UCharToDouble(){
}


template<class InputImageType, class OutputImageType>
bool UCharToDouble::run( itk::Object *obj_InputImage) 
{
...
template<class InputImageType, class OutputImageType>
bool UCharToDouble::run2( itk::Object *obj_InputImage) 
{
  InputImageType *data_InputImage = dynamic_cast<  InputImageType * >(obj_InputImage);
...
void UCharToDouble::execute(){
  ...
  if(0) { }
  else if(run< itk::Image<unsigned char, 2>, itk::Image<double, 2> >( data_InputImage )) {} 
  else if(run< itk::Image<unsigned char, 3>, itk::Image<double, 3> >( data_InputImage )) {} 
  else if(run2< itk::Image<itk::Vector<unsigned char>, 2>, itk::Image<itk::Vector<double>, 2> >( data_InputImage )) {} 
  else if(run2< itk::Image<itk::Vector<unsigned char>, 3>, itk::Image<itk::Vector<double>, 3> >( data_InputImage )) {} 
  else {
    // error
...
void UCharToDouble::tcl_command(GuiArgs& args, void* userdata)
{
