...
int
SurfTree::extractTriSurface(TriSurface* ts, Array1<int>& map,
			    Array1<int>& imap, int comp, int remapPoints)
{
  ...
  cerr << "surface " << ts->name << " has " <<
    ts->points.size() << " nodes, " <<
    ts->elements.size() << " elements and " <<
    ts->bcVal.size() << " known vals.\n";
		   ...
		   int &compBest, int &faceBest, int comp)
{
  ...
  for (int ii=0; ii<surfI[comp].faces.size(); ii++)
  {
    ...
    if (surfI[comp].faceOrient[ii])
    {
      p2=nodes[e->i2]; p3=nodes[e->i3];
    }
    else
    {
      p2=nodes[e->i3]; p3=nodes[e->i2];
    }
