			   ...
			   CCVariable<double>& AP)
{
  ...
  */
  ...
  if (plusZ)
    facZ = -1;
  double start_mattime = Time::currentSeconds();
  ...
  int me = d_myworld->myrank();
  if (me == 0)
    cerr << "matrix create local time: " << Time::currentSeconds()-start_mattime << " Seconds\n" ;
#ifdef ARCHES_PETSC_DEBUG
  cerr << "assemblign rhs\n";
#endif
...
RadLinearSolver::radLinearSolve()
{
  ...
  ierr = KSPSetTolerances(ksp, 1.e-3, PETSC_DEFAULT, PETSC_DEFAULT, PETSC_DEFAULT);
  if(ierr)
