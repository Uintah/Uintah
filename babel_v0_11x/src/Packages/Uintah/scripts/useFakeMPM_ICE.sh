...
if test -z "$using_fake_mpmice"; then
    ...
    sed -e "s,#DUMMY_LIB,DUMMY_LIB ,g" \
        -e "s,MPMICE_LIB ,#MPMICE_LIB,g" $filename.tmp > $filename
    rm $filename.tmp
    ...
    mv -f $filename $filename.tmp
    sed -e "s,#FAKE_MPMICE,FAKE_MPMICE ,g" $filename.tmp > $filename
    rm $filename.tmp
    ...
    mv -f $filename $filename.tmp
    sed_line2='-e "s,#MPMICE_LIB,MPMICE_LIB ,g"'
