...
const char * ZTexRenderer::version = "1.0";
const char * ZTexRenderer::name = "ZTex Transmission";
...
ZTexRenderer::render( const bool doFPS ) {
  ...
  Log::log( DEBUG, "[ZTexRenderer::render] modelview:" );
  for ( int i = 0; i < 16; i++ )
  {
    Log::log( DEBUG, "[ZTexRenderer::render]" + mkString(modelview[ i ]) );
  }

  Log::log( DEBUG, "[ZTexRenderer::render] projMatrix:" );
  for ( int i = 0; i < 16; i++ )
  {
    Log::log( DEBUG, "[ZTexRenderer::render] " + mkString(projMatrix[ i ]) );
  }

  Log::log( DEBUG, "[ZTexRenderer::render] viewPort: (" + mkString(viewPort[0]) + ", " + mkString(viewPort[1]) + ", " + mkString(viewPort[2]) + ") " );
...
ZTexRenderer::addZTex( float *vertices,
  ...
  */
  ...
  // Calculate modelview, viewPort, and projMatrix matrices to pass to the mkTexCoords function
  // These matrices may be incorrect
  // Why are we calculating the matrix values rather than just getting them from OpenGL?
  ...
  Log::log( DEBUG, "[ZTexRenderer::addZTex] modelview:" );
  for ( int i = 0; i < 16; i++ )
  {
    Log::log( DEBUG, "[ZTexRenderer::addZTex]" + mkString(modelview[ i ]) );
  }

  Log::log( DEBUG, "[ZTexRenderer::addZTex] projMatrix:" );
  for ( int i = 0; i < 16; i++ )
  {
    Log::log( DEBUG, "[ZTexRenderer::addZTex] " + mkString(projMatrix[ i ]) );
  }

  Log::log( DEBUG, "[ZTexRenderer::addZTex] viewPort: (" + mkString(viewPort[0]) + ", " + mkString(viewPort[1]) + ", " + mkString(viewPort[2]) + ") " );
...

  {
    ...
    Log::log( DEBUG, "[ZTexRenderer::addZTex] Eye: " + eyep.toString() );
    Log::log( DEBUG, "[ZTexRenderer::addZTex] At: " + lookat.toString() );
    Log::log( DEBUG, "[ZTexRenderer::addZTex] Up: " + up.toString() );
  ...
  */
  ...
  if ( list != 0 ) {
    ...
    Log::log( DEBUG, "[ZTexRenderer::addZTex] New geom bounding box: " + b.toString() );
    Log::log( DEBUG, "[ZTexRenderer::addZTex] Global bounding box was " + bounding_box.toString() );
    bounding_box.extend(b);
    Log::log( DEBUG, "[ZTexRenderer::addZTex] Global bounding box is now " + bounding_box.toString() );
...
ZTexRenderer::mkTexCoords( float * vertices,
  ...
  if ( gluProject( vertices[ 0 ],
		   ...
		   &z ) == GL_FALSE ) {
    texX = -1; texY = -1;
    Log::log( ERROR, "[ZTexRenderer::mkTexCoords] gluProject failed, sending bogus tex coords" );
    return;
  }

  Log::log( DEBUG, "[ZTexRenderer::mkTexCoords] x = " + mkString(x) + ", y = " + mkString(y) + ", viewPort[2] = " + mkString(viewPort[2]) + ", viewPort[3] = " + mkString(viewPort[3]) );
  texX = x / viewPort[2];
  texY = y / viewPort[3];

  Log::log( DEBUG, "[ZTexRenderer::mkTexCoords] vertex: (" + mkString(vertices[0])
                    + ", " + mkString(vertices[1]) + ", " + mkString(vertices[2]) + ") texcoords: ("
	            + mkString(texX) + ", " + mkString(texY) + ")" );  
