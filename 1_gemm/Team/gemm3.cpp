					...
					for(int m = mb; m < me; m++)	//matrix without subview
					{
						for(int i = rb; i < re; i=i+TILE) 	//NN loop
						{
							...
							Kokkos::parallel_for(Kokkos::ThreadVectorRange(thread, DEFAULT_VECTOR_LENGTH), KOKKOS_LAMBDA (const int& v){
									...
									value_type c00=0.0, c01=0.0, c02=0.0,
												  c10=0.0, c11=0.0, c12=0.0,
												  c20=0.0, c21=0.0, c22=0.0;

									value_type a00=0.0, a01=0.0, a02=0.0,
												  a10=0.0, a11=0.0, a12=0.0,
												  a20=0.0, a21=0.0, a22=0.0;
								
									value_type b00=0.0, b01=0.0, b02=0.0,
												  b10=0.0, b11=0.0, b12=0.0,
												  b20=0.0, b21=0.0, b22=0.0;

									for(int k = 0; k<N ; k=k+TILE) 	
									{
										a00 = A(m, i, k, v); a01 = A(m, i, k+1, v); a02 = A(m, i, k+2, v); 
										a10 = A(m, i+1, k, v); a11 = A(m, i+1, k+1, v); a12 = A(m, i+1, k+2, v); 
										a20 = A(m, i+2, k, v); a21 = A(m, i+2, k+1, v); a22 = A(m, i+2, k+2, v); 

										b00 = B(m, j, k, v); b01 = B(m, j, k+1, v); b02 = B(m, j, k+2, v);
										b10 = B(m, j+1, k, v); b11 = B(m, j+1, k+1, v); b12 = B(m, j+1, k+2, v); 
										b20 = B(m, j+2, k, v); b21 = B(m, j+2, k+1, v); b22 = B(m, j+2, k+2, v); 

										c_mm(0,0); c_mm(0,1); c_mm(0,2);
										c_mm(1,0); c_mm(1,1); c_mm(1,2);
										c_mm(2,0); c_mm(2,1); c_mm(2,2);
									}
								C(m, i, j, v) = c00; C(m, i, j+1, v) = c01; C(m, i, j+2, v) = c02;
								C(m, i+1, j, v) = c10; C(m, i+1, j+1, v) = c11; C(m, i+1, j+2, v) = c12;
								C(m, i+2, j, v) = c20; C(m, i+2, j+1, v) = c21; C(m, i+2, j+2, v) = c22;
