...
struct Gemm
{
	...
	int mats, N, M, NumTeams, ThreadsPerTeams, NN, tiles_ij;
  	...
  	Gemm(plain_view a, plain_view b, plain_view c, int NumTeams , int ThreadsPerTeams, int N1, int M1): NumTeams(NumTeams), ThreadsPerTeams(ThreadsPerTeams), N(N1), M(M1/DEFAULT_VECTOR_LENGTH)
	{
		...
		tiles_ij = N / TILE;	// number of tiles in i (or j) dimension
		if(N % TILE) tiles_ij++;

		NN = tiles_ij*tiles_ij;	//NN per thread
	}
				...
				if(re < N && ce < N)
				{
					for(int m = mb; m < me; m++)	//matrix without subview
					{
						for(int i = rb; i < re; i=i+TILE) 	//NN loop
						{
							...
							{
								PerVL<double> c00(0.0), c01(0.0),
										    c10(0.0), c11(0.0);

								for(int k = 0; k<N ; k=k+TILE) 	
								{
									PerVL<double> a00(0.0), a01(0.0),
											    a10(0.0), a11(0.0);
								
									PerVL<double> b00(0.0), b01(0.0),
											    b10(0.0), b11(0.0);
								...
								C(m, i, j) = c00; C(m, i, j+1) = c01;
								C(m, i+1, j) = c10; C(m, i+1, j+1) = c11;
							}
						}
					}
				}
				else	//left over elements
				{
					ce = min(ce, N);
					re = min(re, N);
					for(int m = mb; m < me; m++)	//matrix without subview
					{
						for(int i = rb; i < re; i=i+1) 	//NN loop
						{
							for(int j = cb; j<ce ; j=j+1) 	//columns loop
							{
								//PerVL<double> c00(0.0);
