				...
				int ce = cb + TILE;
					...
					for(int m = mb; m < me; m++)	//matrix without subview
					{
						for(int i = rb; i < re; i=i+TILE) 	//NN loop
						{
							...
							{
								PerVL<double> c00(0.0), c01(0.0), c02(0.0), c03(0.0), 
										    c10(0.0), c11(0.0), c12(0.0), c13(0.0), 
										    c20(0.0), c21(0.0), c22(0.0), c23(0.0), 
										    c30(0.0), c31(0.0), c32(0.0), c33(0.0);

								PerVL<double> a00(0.0), a01(0.0), a02(0.0), a03(0.0), 
										    a10(0.0), a11(0.0), a12(0.0), a13(0.0), 
										    a20(0.0), a21(0.0), a22(0.0), a23(0.0), 
										    a30(0.0), a31(0.0), a32(0.0), a33(0.0);
								
								PerVL<double> b00(0.0), b01(0.0), b02(0.0), b03(0.0), 
										    b10(0.0), b11(0.0), b12(0.0), b13(0.0), 
										    b20(0.0), b21(0.0), b22(0.0), b23(0.0), 
										    b30(0.0), b31(0.0), b32(0.0), b33(0.0);

								for(int k = 0; k<N ; k=k+TILE) 	
								{
									a00 = A(m, i, k); a01 = A(m, i, k+1); a02 = A(m, i, k+2); a03 = A(m, i, k+3); 
									a10 = A(m, i+1, k); a11 = A(m, i+1, k+1); a12 = A(m, i+1, k+2); a13 = A(m, i+1, k+3);
									a20 = A(m, i+2, k); a21 = A(m, i+2, k+1); a22 = A(m, i+2, k+2); a23 = A(m, i+2, k+3);
									a30 = A(m, i+3, k); a31 = A(m, i+3, k+1); a32 = A(m, i+3, k+2); a33 = A(m, i+3, k+3);

									b00 = B(m, j, k); b01 = B(m, j, k+1); b02 = B(m, j, k+2); b03 = B(m, j, k+3); 
									b10 = B(m, j+1, k); b11 = B(m, j+1, k+1); b12 = B(m, j+1, k+2); b13 = B(m, j+1, k+3);
									b20 = B(m, j+2, k); b21 = B(m, j+2, k+1); b22 = B(m, j+2, k+2); b23 = B(m, j+2, k+3);
									b30 = B(m, j+3, k); b31 = B(m, j+3, k+1); b32 = B(m, j+3, k+2); b33 = B(m, j+3, k+3);

									c_mm(0,0); c_mm(0,1); c_mm(0,2); c_mm(0,3); 
									c_mm(1,0); c_mm(1,1); c_mm(1,2); c_mm(1,3); 
									c_mm(2,0); c_mm(2,1); c_mm(2,2); c_mm(2,3); 
									c_mm(3,0); c_mm(3,1); c_mm(3,2); c_mm(3,3); 
								}
								C(m, i, j) = c00; C(m, i, j+1) = c01; C(m, i, j+2) = c02; C(m, i, j+3) = c03;
								C(m, i+1, j) = c10; C(m, i+1, j+1) = c11; C(m, i+1, j+2) = c12; C(m, i+1, j+3) = c13;
								C(m, i+2, j) = c20; C(m, i+2, j+1) = c21; C(m, i+2, j+2) = c22; C(m, i+2, j+3) = c23;
								C(m, i+3, j) = c30; C(m, i+3, j+1) = c31; C(m, i+3, j+2) = c32; C(m, i+3, j+3) = c33;
							}
						}
					}
			});
