...
below the \TT{min\_patch\_size}.  The target number of patches per processor is 
specified using the \TT{patches\_per\_level\_per\_proc} tag. To prevent the 
resizing of tiles set the tag \TT{dynamic\_size} to false. 
...
\TT{DLB,} \TT{RoundRobin,} \TT{SimpleLoadBalancer,} or
\TT{SingleProcessorLoadBalancer.}  If no load balancer is specified
...
balancer you must include the \TT{dynamicAlgorithm} tag. This tag can
be \TT{PatchFactor} or \TT{Zoltan.}
...
The \TT{timestepInterval} tag specifies the maximum number of timesteps the simulation
can run without attempting to load balance.  The \TT{gainThreshold} tag specifies the 
percent improvement that a load balance must provide over the old load balance in 
order for the new load balance to be used.  Finally the \TT{outputNthProc} tag causes
...
If the dynamic algorithm is set to \TT{Zoltan} then the user must also specify which
Zoltan algorithm to use with the \TT{zoltanAlgorithm} tag.  

\begin{Verbatim}[fontsize=\footnotesize]
       <zoltanAlgorithm>HSFC</zoltanAlgorithm>
\end{Verbatim}

The Zoltan algorithm can be set to \TT{HSFC,} \TT{RIB,} \TT{RCB.}
