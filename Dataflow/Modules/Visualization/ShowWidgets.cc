...
class ShowWidgets : public Module {
   GeometryOPort* ogeom;
...
extern "C" Module* make_ShowWidgets(const clString& id) {
  return new ShowWidgets(id);
}

static clString module_name("ShowWidgets");

ShowWidgets::ShowWidgets(const clString& id)
: Module("ShowWidgets", id, Source), widget_lock("ShowWidgets widget lock"),
...
void ShowWidgets::execute()
{
   ...
   cerr << "There are currently " << ogeom->getNViewWindows() << " active viewwindows\n";
   GeometryData* data=ogeom->getData(0, GEOM_ALLDATA);
   cerr << "data=" << data << endl;
}

void ShowWidgets::widget_moved(int last)
{
    if(last)
	cerr << "Last callback...\n";
   cerr << "ShowWidgets: begin widget_moved" << endl;
   ...
   //    want_to_execute();
   //}
   cerr << "ShowWidgets: end widget_moved" << endl;
}


void ShowWidgets::tcl_command(TCLArgs& args, void* userdata)
{
#if 0
   if(args.count() < 2){
      args.error("ShowWidgets needs a minor command");
      return;
   }
#endif
   if (args[1] == "nextmode") {
