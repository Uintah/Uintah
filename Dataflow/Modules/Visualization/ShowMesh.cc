...
/*
 *  ShowMesh.cc:  Convert a Mesh into geoemtry
 *
 *  Written by:
 *   David Weinstein
...
class ShowMesh : public Module {
  ...
  TCLint showNodes;
public:
  ShowMesh(const clString& id);
  virtual ~ShowMesh();
  virtual void execute();
};

extern "C" Module* make_ShowMesh(const clString& id)
{
  return scinew ShowMesh(id);
}

ShowMesh::ShowMesh(const clString& id)
  : Module("ShowMesh", id, Filter), showNodes("showNodes", id, this),
