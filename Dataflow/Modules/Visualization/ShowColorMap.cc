/*
 *  ShowColorMap.cc: create a key for colormap
 *
 *  Written by:
 *   Philip Sutton
...
public:
  ShowColorMap(const clString &id);
  virtual ~ShowColorMap();
  virtual void execute();
};
extern "C" Module *make_ShowColorMap(const clString &id) {
  return new ShowColorMap(id);
}

ShowColorMap::ShowColorMap(const clString &id)
  : Module("ShowColorMap", id, Filter)
{
...
void ShowColorMap::execute() {
  ...
  if( isf->get( sf ) ) {
    ...
    if( grid == 0 ) {
      cerr << "ShowColorMap wants ScalarFieldRG, didn't get it\n";
      return;
    }
    double max = -1e30;
    double min = 1e30;
