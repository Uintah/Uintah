...
# Zip the two lists
ZIPPED_LIST = zip(ARENA_PART_SAT_TEST_LIST, SHPB_EXPT_DATA_LIST, SHPB_DENSITY_LIST)

### COMMENT ME OUT!!!!!!! ###
ARENA_PART_SAT_TEST_LIST = [
  ZIPPED_LIST[0],  # SHPB: sample 26 with damage 
  ZIPPED_LIST[1],  # SHPB: Mason sand: 1770 kg/m^3
  ZIPPED_LIST[2],  # SHPB: Mason sand: 1700 kg/m^3
  ZIPPED_LIST[3],  # SHPB: Mason sand: 1580 kg/m^3
  ZIPPED_LIST[4],  # SHPB: Mason sand: 1640 kg/m^3
  ]
### --------------------- ###
...
def computeLogStrain(data):
  R, U = scipy.linalg.polar(np.reshape(data, (3,3)))
  logU = scipy.linalg.logm(U)
  return logU
  
def getLogStrain(data, index1, index2, VARIABLE_TYPE):
  result = {
    'scalar': lambda data: 0.0,
    'vector': lambda data: 0.0,
    'matrix': lambda data: computeLogStrain(data)[index1][index2]
  }[VARIABLE_TYPE](data)
  return result
...
def convertData(data, VARIABLE_TYPE, CONVERSION_TYPE):
  ...
  result = {
    ...
    'logeps11': lambda data: getLogStrain(data, 0, 0, VARIABLE_TYPE)
...
def read_extracted_test_data(root_dir, ups_file):
  ...
  ReadData = collections.namedtuple('ReadData', ['time', 'detF', 'F_11', 'F_22', 'lambda_1', 
    'logeps_11', 'sig_I1', 'sig_J2', 'sig_11', 'sig_12', 'sig_22', 'sigQS_I1', 'sigQS_J2', 
    'capX', 'epsp_I1', 'epse_v', 'epsp_v', 'epsp_eq', 'alpha_I1', 'phi', 'sw',
    'coher', 'tgrow', 'PEAKI1', 'FSLOPE', 'STREN', 'YSLOPE'])

  return ReadData(time, detF, F_11, F_22, lambda_1, logeps_11,
...
def read_expt_data(root_dir, expt_file):
  ...
  with open(file_name_expt, 'rb') as exptfile:
    ...
    for row in reader:
      ...
      eps_a_expt.append(np.log(1.0 - eps_a))
...
def plot_extracted_test_data(ups_file, data, stress_scale_fac):
  ...
  plotExtractedPartVars(test_name, test_name + "_sig_11_eps_11",
                        data.logeps_11, "none", -1.0, "$\\mathbf{\\varepsilon_a}$", 
                        data.sig_11, "none", -fac, "$\\mathbf{\\sigma_{a}}$ "+units)
...
#---------------------------------------------------------------------------
def plotSimVsExptStressTime(test_name, pdf_name, density,
                          x_data, X_OPERATION, x_scale, X_VARIABLE_LABEL, 
                          y_data, Y_OPERATION, y_scale, Y_VARIABLE_LABEL,
  ...
  # Interpolate (downsample) expt data
  #ff = scipy.interpolate.InterpolatedUnivariateSpline(expt_x_data, expt_y_data, k = 1)
  #x_data_interp = np.linspace(expt_x_data[0], expt_x_data[-1], 40)
  #y_data_interp = ff(x_data_interp)
  #expt_x_data = x_data_interp
  #expt_y_data = y_data_interp

  # Interpolate simulation data to downsampled expt data range
  #ff = scipy.interpolate.InterpolatedUnivariateSpline(x_data, y_data, k = 1)
  #y_data_interp = ff(x_data_interp)
  ...
  # Create a dataframe
  d_sim = {X_VARIABLE_LABEL: x_data, Y_VARIABLE_LABEL: y_data, 
           "Label": [pdf_name]*len(x_data), 
           "Density": [density]*len(x_data),
           "Type": ["sim"]*len(x_data)}
  d_expt = {X_VARIABLE_LABEL: expt_x_data, Y_VARIABLE_LABEL: expt_y_data, 
           "Label": [pdf_name]*len(expt_x_data), 
           "Density": [density]*len(expt_x_data),
           "Type": ["expt"]*len(expt_x_data)}
  df_sim = pd.DataFrame(d_sim, 
    columns=(X_VARIABLE_LABEL, Y_VARIABLE_LABEL, "Label", "Density", "Type"))
  df_expt = pd.DataFrame(d_expt,
    columns=(X_VARIABLE_LABEL, Y_VARIABLE_LABEL, "Label", "Density", "Type"))

  df = pd.DataFrame(columns=(X_VARIABLE_LABEL, Y_VARIABLE_LABEL, "Label", "Density", "Type"))
  df = df.append(df_sim)
  df = df.append(df_expt)
  #print df.head()

  return df

def plotSimVsExptStressYieldSurf(test_name, pdf_name, sim_data,
                        I1_data, X_OPERATION, x_scale, X_VARIABLE_LABEL, 
                        J2_data, Y_OPERATION, y_scale, Y_VARIABLE_LABEL,
                        capX_data, alpha_I1_data, yieldParams,
...
def plot_sim_vs_expt_stress(ups_file, sim_data, expt_data, 
                            stress_scale_fac, expt_scale_fac, density): 
  ''' '''
  ...
  # Plot stress vs. strain
  dfSaEa = plotSimVsExptStressTime(test_name, test_name + "_vs_expt_SaEa", density,
                        sim_data.logeps_11, "none", -1.0, "$\\mathbf{\\epsilon_{a}}$", 
                        sim_data.sig_11, "none", -fac, "$\\mathbf{\\sigma_{a}}$ "+units,
                        expt_data.eps_11, -1.0, 
                        expt_data.sig_11, expt_scale_fac)
  #print dfSaEa.head()

  plotSimVsExptStressTime(test_name, test_name + "_vs_expt_S11E11", density,
  ...
  dfSat = plotSimVsExptStressTime(test_name, test_name + "_vs_expt_S11time", density,
  ...
  dfSrt = plotSimVsExptStressTime(test_name, test_name + "_vs_expt_S22time", density,
                        sim_data.time, "none", 1.0e6, "$\\mathbf{t}$ " + "$\\mu$s", 
                        sim_data.sig_22, "none", -fac, "$\\mathbf{\\sigma_{22}}$ "+units,
                        expt_data.time, 1.0e6, 
                        expt_data.sig_22, expt_scale_fac)

  dfpt = plotSimVsExptStressTime(test_name, test_name + "_vs_expt_ptime", density,
                        sim_data.time, "none", 1.0e6, "$\\mathbf{t}$ " + "$\\mu$s", 
                        sim_data.sig_I1, "p", -fac, "$\\mathbf{p = I_1/3}$ "+units,
                        expt_data.time, 1.0e6, 
                        expt_data.sig_I1, expt_scale_fac)

  dfqt = plotSimVsExptStressTime(test_name, test_name + "_vs_expt_qtime", density,
                        sim_data.time, "none", 1.0e6, "$\\mathbf{t}$ " + "$\\mu$s", 
                        sim_data.sig_J2, "q", -fac, "$\\mathbf{q = \\sqrt{3J_2}}$ "+units,
                        expt_data.time, 1.0e6, 
                        expt_data.sig_J2, expt_scale_fac)

  dfzt = plotSimVsExptStressTime(test_name, test_name + "_vs_expt_ztime", density,
                        sim_data.time, "none", 1.0e6, "$\\mathbf{t}$ " + "$\\mu$s", 
                        sim_data.sig_I1, "z", -fac, "$\\mathbf{z = I_1/\\sqrt{3}}$ "+units,
                        expt_data.time, 1.0e6, 
                        expt_data.sig_I1, expt_scale_fac)

  dfrt = plotSimVsExptStressTime(test_name, test_name + "_vs_expt_rtime", density,
                        sim_data.time, "none", 1.0e6, "$\\mathbf{t}$ " + "$\\mu$s", 
  ...
  return dfSaEa, dfSat, dfSrt, dfpt, dfqt, dfzt, dfrt
...
def plot_all_tests():
  ...
  df_zt = []
  df_rt = []
  for ups_file, expt_file, density in ARENA_PART_SAT_TEST_LIST:
    ...
    dfSaEa, dfSat, dfSrt, dfpt, dfqt, dfzt, dfrt = \
      plot_sim_vs_expt_stress(ups_file, data, expt_data, stress_scale_fac, expt_scale_fac, density)
    df_SaEa.append(dfSaEa)
    df_Sat.append(dfSat)
    df_Srt.append(dfSrt)
    df_pt.append(dfpt)
    df_qt.append(dfqt)
  ...
  # Write the data frames t0 csv files
  pd.concat(df_SaEa).to_csv("MasonSandSHPB_SaEa_sim_expt.dat", 
    sep = " ", index = False)
  pd.concat(df_Sat).to_csv("MasonSandSHPB_Sat_sim_expt.dat", 
    sep = " ", index = False)
  pd.concat(df_Srt).to_csv("MasonSandSHPB_Srt_sim_expt.dat", 
    sep = " ", index = False)
  pd.concat(df_pt).to_csv("MasonSandSHPB_pt_sim_expt.dat", 
    sep = " ", index = False)
  pd.concat(df_qt).to_csv("MasonSandSHPB_qt_sim_expt.dat", 
    sep = " ", index = False)
  pd.concat(df_zt).to_csv("MasonSandSHPB_zt_sim_expt.dat", 
    sep = " ", index = False)
  pd.concat(df_rt).to_csv("MasonSandSHPB_rt_sim_expt.dat", 
    sep = " ", index = False)
