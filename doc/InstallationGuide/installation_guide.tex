...
\emph{Note: This may have been fixed... if there is
  '${SHARED_COMPILE_FLAG}' on the compile line, then it should be good
to go.}
...
\emph{Note: However, it looks like another problem may have been
  introduced.  /src/FEI_mv/SuperLU/SRC/Makefile line 209
  $(SHARED_COMPILE_FLAG) needs to be added to the superlu_timer.o
  build line...}
...
The latest official release of VisIt is 2.0.1.  The VisIt build
system has been changed to CMake, so building the UDA reader plugin for
previous versions of VisIt (1.x) will no longer be supported, and CMake is now also
...
automatically download these dependencies, but often fails.  In is
highly recommended to NOT use the SVN version of VisIt, but instead us
one of the official tarball releases.
...
later of CMake is required for VisIt 2.0).  Note, VisIt builds CMake for
...
to manually update
/path/to/installation/2.0.1/linux-x86_64/include/PluginVsInstall.cmake
