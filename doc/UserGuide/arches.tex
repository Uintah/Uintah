...
{\bf Default}: {\it none} \\
{\bf Description}: This identifies which inlets are associated with this scalar balance.  One may have multiple \verb=<inlet>='s specified. 
\end{enumerate}
...
\end{Verbatim}
% 
where each section enclosed by the \verb=<Stream>= tag defines one of the individual streams.  Note that only two streams are allowed. 
