...
Each of the simulation components has many options, these are described
below in each respective component's section of this document.
...
\begin{itemize}
  ...
  \item \textbf{Dynamic MPI Scheduler:} Dynamic scheduling with
      non-deterministic, out-of-order execution of tasks at runtime.
      \emph{One MPI rank per CPU core}.
  \item \textbf{Unified Scheduler:} An advanced, multi-threaded
      scheduler that uses a combination of MPI + Pthreads and offers support
      for GPU tasks (MPI + Pthreads + NVIDIA CUDA). Dynamic scheduling with non-deterministic, out-of-order execution of tasks at runtime. \emph{One MPI
      rank per multi-core node}. Pthreads are pinned to individual CPU cores where these tasks are executed. Uses a decentralized model wherein all threads can access task queues, processes there own MPI send and recvs, with shared
      access to the DataWarehouse. (requires \textbf{MPI\_THREAD\_MULTIPLE}
      support)
\end{itemize}
...
\begin{itemize}
  \item \emph{Scheduler} - specifies the specific Uintah task scheduler to
      use.
      Valid options are: \TT{MPI \; DynamicMPI \; Unified}
  \item \emph{small\_messages} - whether or not to turn on MPI message
      ...
      is no "best" value for it. Sometimes MPI message combination works
  ...
  \item \emph{taskReadyQueueAlg} - (only applicable for Dynamic and Unified
      ...
      options are: \\
      \TT{PatchOrder \; PatchOrderRandom \; MostMessages \; LeastMessages \\*
          Random     \; FCFS             \; Stack}. \\
      Evidence suggests using \TT{MostMessages} algorithm works best in general. This means highest execution priority is given to
      tasks that will generate \emph{the most outgoing MPI messages}.
...
Do note that you may use the MPI or Unified scheduler without ANY of the
...
Unified Scheduler automatically. The input file options simply give
...
uses a pure process model, e.g. one MPI process per core. Though distributed, the task-graph on each compute node will be identical and is executed in the same order, that is static task ordering and deterministic execution of ALL tasks. This normally
...
cells and global meta-data \cite{SCI:Men2011a}. This increase becomes a barrier to scalability
...
MPI-only approach \cite{SCI:Men2011a, SCI:Men2014b}.
...
\subsection{Dynamic MPI Scheduler}
\label{Sec:DynamicMPIScheduler}
In the MPI
...
type of delay can be nearly 80 percent of the total MPI wait time in Uintah \cite{SCI:Men2010b}.
...
\subsection{Unified Scheduler}
\label{Sec:UnifiedScheduler}
...
scalability beyond $\mathcal{O}(100K)$ cores \cite{SCI:Men2011a}. This limitation has been overcome
...
many-core architectures. This hybrid memory approach (a combination of MPI +
Pthreads (std::thread)) has enabled Uintah to demonstrate scalability up to 256K cores on DOE Titan 768k cores on the DOE Mira system \cite{SCI:Men2014b}.

With Uintah's dynamic and static MPI schedulers, based on a pure process model
...
share data. The multi-threaded Unified scheduler (\ref{fig:UnifiedScheduler}) extends  the Uintah runtime system to support multi-threaded execution, allowing all threads to access task queues (assigning work to themselves) during
...
system has been extended to support multi-threaded and GPU execution \cite{SCI:Men2012b}. The scalability of UIntah's GPU support has been shown to scale to 16K GPUs on the DOE Titan system \cite{SCI:Hum2016a}.

Compared to Uintah’s dynamic MPI and Threaded MPI scheduler, the Unified scheduler has
...
To invoke the Unified scheduler, you may specify \texttt{Unified} in
...
compute node. The above command line will launch 1 MPI process per node and create 15 \TT{(nthreads -1)} additional task execution threads. The main thread of execution will also execute tasks itself in the same way spawned threads do.

\vspace{20pt}

\textbf{NOTE: This scheduler requires \textbf{MPI\_THREAD\_MULTIPLE} support.}
...
\section{GPU Support} \label{Sec:GpuSupport}
Uintah provides support for NVIDIA GPUs through simulation variable preparation and task execution.  The high-level goal is to maintain a strong layer of separation between the runtime and the application task developer using the same philosophy that exists for CPU tasks.  Uintah's runtime manages copying simulation variables in and out of GPU memory, ghost cell scattering and gathering, task execution, overlapping GPU kernels, and overlapping computation with communication.  This GPU engine has been demonstrated at scale using 16K GPUs \cite{SCI:Hum2016a} and in a heterogeneous production 1000 MWe coal boiler simulation on Titan using 7.5K GPUs in a mixture of $\sim$1400 GPU and CPU tasks per MPI rank.  GPU support is still under active development, and some features are likely to change, most notably the host-side GPU Data Warehouse, and support for GPUs with Kokkos.

This user guide will describe the features of Uintah's GPU support through a simple Poisson example.  To begin, Uintah must be compiled with CUDA support which requires Uintah's configure step list the path to CUDA using  \texttt{--with-cuda=/path/to/cuda}.  Once configured and then compiled, Uintah can verify if a GPU is active by running \texttt{sus -gpucheck}.  

A task is designated as a GPU enabled task within its declaration, generally alongside when its  \texttt{computes} and  \texttt{requires} are also declared.  This declaration is done like so:  

\begin{Verbatim}[fontsize=\footnotesize]
  task->usesDevice(true);
\end{Verbatim} 

An example of this is found in Uintah's UnifiedSchedulerTest.cc in the \\ \texttt{src/CCA/Components/Examples directory} in the  \texttt{scheduleTimeAdvance} method.  The task's callback function remains the same, a call to a CPU task.  Even though it is a GPU enabled task, a CPU task function must manually invoke a GPU kernel containing CUDA code, as seen in that UnifiedSchedulerTest.cc's \texttt{timeAdvanceUnified} method.  All GPU task code should still follow normal strategies for CUDA programming.

Uintah's runtime maintains a Task Data Warehouse for GPU tasks to enable a task developer to access simulation variables within CUDA code.  A task's Task Data Warehouses contain only entries for all the simulation variables defined in that task's declaration.  It is strongly recommended (though not required) that these data warehouses are passed into the kernel. An example of this is seen in \texttt{timeAdvanceUnified}'s method.  A task developer can retrieve a simulation variable through a simple \texttt{get()} or \texttt{getModifiable()}API call, as seen in UnifiedSchedulerTest.cu's \\ \texttt{unifiedSchedulerTestKernel()}.  An example is given below:
