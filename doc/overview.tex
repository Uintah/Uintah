...
and communication. The task graph (see Figure~\ref{fig:TaskGraph}) is an explicit representation of the
...
to perform inter-process communication via MPI. When the
scheduler is executed, the tasks will execute in their pre-determined
order.
...
component is assigned a name (in this particular example, it is
...
information (e.g. time step size, length of time for simulation, etc), and
...
read in from the \texttt{problemSetup} method and to define what variables are
