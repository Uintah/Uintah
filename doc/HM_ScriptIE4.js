       ...
       */
       ...
       function HM_f_GetItemDivStr(itemid,disptext,hasmore){
           ...
           if(hasmore && HM_CurrentMenu.showImage) {
               ...
               var ImgHSpace   = (HM_RightToLeft) ? 0 : HM_ItemPadding;
               var ImgStyle    = HM_RightToLeft ? ("left:"+ (HM_ItemPadding + HM_ImageHorizSpace) + "px;top:"+ (HM_ItemPadding + HM_ImageVertSpace) + "px;") : ("float:right;margin-right:"+ (-FullPadding) +"px;margin-top:"+ HM_ImageVertSpace + "px;width:"+ HM_ImageSize + "px;");
       ...
       function HM_f_MakeMenu(menucount) {
           ...
           while (HM_CurrentMenu.itemCount < HM_CurrentMenu.maxItems) {
               ...
               if(HM_CurrentMenu.itemElement.hasMore && (!HM_CreateTopOnly || HM_AreCreated && HM_CreateTopOnly)) {
                   ...
                   if(MenuCreated) {
                       ...
                       HM_CurrentMenu.itemElement.hasMore = false;
                   }
               }
           }
           ...
           if(HM_CurrentMenu.isHorizontal){
               var ChildHeight = HM_CurrentMenu.style.pixelHeight;
               ChildHeight -= (HM_BorderWidth * 2);
               for(i=0; i<HM_CurrentMenu.childNodes.length; i++){
                   HM_CurrentMenu.childNodes[i].style.pixelHeight = ChildHeight;
               }
           }
       ...
       function HM_f_SetItemStyle() {
           with(this.style){
               ...
               if(this.menu.isHorizontal){
                   ...
                   if (this.index) {
                       var SiblingHeight = this.siblingBelow.scrollHeight;
                       this.menu.style.pixelHeight = Math.max(this.menu.style.pixelHeight,SiblingHeight+(HM_BorderWidth * 2));
                   }
                      else{
                       var SiblingHeight = this.scrollHeight;
                       this.menu.style.pixelHeight = SiblingHeight + (HM_BorderWidth * 2);
                   }
               }
               else {
                   pixelLeft = 0;
                   if (this.index) {
                       var SiblingHeight =(this.siblingBelow.scrollHeight + HM_SeparatorSize);
                       pixelTop = this.siblingBelow.style.pixelTop + SiblingHeight;
                   }
                   else pixelTop = 0;
                   this.menu.style.pixelHeight = pixelTop + this.scrollHeight + (HM_BorderWidth * 2);
               }
           }
       }
