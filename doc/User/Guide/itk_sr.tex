...
LevelSet segmentation filters---the user can visualize the
segmentation process in real-time.

Usually data are sent only when a calculation is complete.  When
allowed, an output port will send data every $n_th$ iteration of a
calculation.
...
description \xmlstarttag{output} element.  The value of the respective
\xmlattrname{name} attributes must match.  The value of an
\xmlstarttag{output} element's \xmlattrname{send_intermediate}
