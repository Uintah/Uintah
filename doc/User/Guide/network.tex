...
\descitem{Pop-up Menu} Clicking Button3 while the mouse
...
Some operations (moving, destroying, and blocking) act on a group of
modules (see \secref{Moving Module(s)}{sec:movemod},
\secref{Destroying Module(s)}{sec:destroymod}, and \secref{Module 
Blocking}{sec:modblocking}).  A group of modules can be created in
two ways:

\begin{enumerate}
\item Perform Button1-Drag (press and hold the left mouse button and drag the mouse)
  while the pointer is in the NetEdit frame, but not over a module or
  a connection. This starts a selection box, allowing the user to
  select multiple modules. Only modules overlapping the selection box
  are selected. Release Button1 to complete the selection.
  
\item Select the first module by clicking Button1 while the pointer
  is over a module.  Then add modules to the group by clicking C-Button1
  (press and hold the \keyboard{control} key while clicking the left
  mouse button) on additional modules.   All previously selected modules remain selected.
\end{enumerate}

Selected modules are drawn in a darker shade of grey.
...
methods.  If the \keyboard{control} key is not pressed, the previous group is
...
the module menu and select \menuitem{Disable}. Note that disabling
incoming and outgoing connections seperately automatically disables
the module (see \secref{Disabling a Connection}{sec:disableconnect}).
...
module menu and select \menuitem{Enable}.
...
To delete a module, click Button3 on the module to view the module menu
and select \menuitem{Destroy}.

Multiple modules can be deleted at one time
(see \secref{Selecting Modules}{sec:movemod}).
Select one or more modules, then click Button3 on a module to view the
module menu and select \menuitem{Destroy Selected}. 

\section{Creating a Subnet}
\label{sec:subnet}

Select more than one module using Module Selection. Click Button3
anywhere in the NetEdit window and select \menuitem{Create Subnet}.
...
% Need short explanation of what blocking means.
...
select \menuitem{Block Module}.

To unblock modules select \menuitem{Unblock Module} from the
...
module menu and select \menuitem{Notes}. Then enter new notes or edit
existing notes in note editor dialog .

To display module notes in the NetEdit window, select one radio
...
view the module menu and select \menuitem{Notes}. Hiding notes does
not delete notes.
...
\menuitem{Show Log}. The module writes error messages or
...
\section{Displaying Connection Notes}
...
view the connection menu and select \menuitem{Notes}. Then enter new
...
middle mouse button) on displayed notes. To view again, click Button3 to
view the connection menu and select \menuitem{Notes}. Hiding notes does
not delete notes.
...
connection menu and select \menuitem{Delete}. A connection can also be
