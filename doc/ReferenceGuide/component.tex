...
implementation, and testing of a \sr\ component.  It also serves as data for
applications and databases.  For example:

\begin{itemize}
\item Documention (html and pdf forms) for component users and developers
...
\Note{Components that are not accomponied by a valid and complete
...
\begin{description}
  \elementitem{component name="component-name" package="package-name"
  ...
  \icode{name}, \icode{package}, and \icode{category} attributes.  These
  attributes name your component, the package the component belongs to, and
...
book (but is not itself a hyperlink).
...
\begin{description}
  ...
  \begin{description}
    ...
    \begin{description}
      ...
      publication.  Is not itself a hyperlink but it may be used within the
...
\item Select the \menuitem{Parse DTD} item from the \menu{DTD} menu
  which can be found in the window's main menu bar.  Or type \keyboard{C-c
    C-p}.
\item Insert the top level \element{component} element using \emacs\ popup
  ...
  \menuitem{component} from the popup menu.  After doing this you will be
  prompted for the \element{component} element's \attribute{name},
  \attribute{package}, and \attribute{category} attributes.
  attribute.  After typing in each of the attributes \Emacs\ 
  will then insert a number of other required elements.
...
to the orginal before using emacs.
