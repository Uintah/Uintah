...
itcl_class Packages/Phil_Tbon_TbonCL {
	...
	label $w.title -text "Curvilinear TBON Parameters"
	...
	pack $w.top.radio1 $w.top.radio2 -in $w.top \
		-side left -fill both
	...
	pack $w.bot.s1 $w.bot.s2 -in $w.bot \
		-side top -fill both
	pack $w.title $w.bmeta $w.top $w.mid $w.bot -in $w -side top -fill both
    }
