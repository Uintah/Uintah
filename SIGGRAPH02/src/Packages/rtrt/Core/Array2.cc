...
template<int> void Pio(SCIRun::Piostream&, rtrt::Array2<int>& data)
{
  ...
  if (str.supports_block_io()) {
    str.block_io(data.objs, sizeof(int), data.dm1*data.dm1);
  } else {
...
template<float> void Pio(SCIRun::Piostream&, rtrt::Array2<float>& data)
{
  ...
  if (str.supports_block_io()) {
    str.block_io(data.objs, sizeof(float), data.dm1*data.dm1);
  } else {
    for(int i=0;i<data.dm1;i++){
      for(int j=0;j<data.dm2;j++){
	SCIRun::Pio(stream, data.objs[i][j]);
...
template<double> void Pio(SCIRun::Piostream&, rtrt::Array2<double>& data)
{
  ...
  if (str.supports_block_io()) {
    str.block_io(data.objs, sizeof(double), data.dm1*data.dm1);
  } else {
    for(int i=0;i<data.dm1;i++){
      for(int j=0;j<data.dm2;j++){
	SCIRun::Pio(stream, data.objs[i][j]);
