! TOKEN from netlib (www.netlib.com)
! I checked out, it worked well, there is it still a little problem detected by some 
! compilers: 
!-----------------------------------------------------------------
!f90: Warning: eigen.f90, line 203: Variable IEXEC is used before its value has b      een defined
!     else if (iexec.eq.2) then
!--------------^
!*** Exit 3
!------------------------------------------
...
! module with subroutine to eval eigen values and eigen vectors of a general complex matrix 
! use the subroutine cg() for the general case

! these subroutines were token from so-called netlib (distribution free , open source.
! the obsolete statements has been eliminated. still lots of f77-like sintax.
