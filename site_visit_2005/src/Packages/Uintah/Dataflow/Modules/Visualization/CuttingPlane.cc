...
class CuttingPlane : public Module {
  ...
  GuiDouble yt;
  GuiDouble zt;
  GuiInt gui_plane_idx;
  GuiInt gui_use_plane_idx;
  GuiString gui_which_plane;
  int find;
  int grid_id;
  string msg;
public:
...
CuttingPlane::CuttingPlane(GuiContext* ctx) :
  ...
  gui_plane_idx(ctx->subVar("plane_index")),
  gui_use_plane_idx(ctx->subVar("use_plane_index")), 
  gui_which_plane(ctx->subVar("which_plane")), find(-1),
...
void
CuttingPlane::get_minmax(FieldHandle f)
{
  double mn, mx;
      ...
      if (need_find.get() == 1)
        {   // Find the field and put in optimal place
          ...
          if(gui_use_plane_idx.get() == 1){
            ...
            int plane_idx = (int)((c_min.z()*wh+max.z()*(1-wh) - c_min.z())/diag.z());
        ...
        {   // Find the field and put in optimal place
          ...
          if(gui_use_plane_idx.get() == 1){
            ...
            int plane_idx = (int)((c_min.x()*wh+max.x()*(1-wh) - c_min.x())/diag.x());
        ...
        {   // Find the field and put in optimal place
          ...
          if(gui_use_plane_idx.get() == 1){
            ...
            int plane_idx = (int)((c_min.y()*wh+max.y()*(1-wh) - c_min.y())/diag.y());
  ...
  if (cptype != CP_CONTOUR)
    {
      ...
      int haveval=0;
      double minval = DBL_MAX, maxval = DBL_MIN, invrange = 0;
...
void
CuttingPlane::widget_moved(bool last, BaseWidget*)
{
  if(last && !abort_flag)
    {
      abort_flag=1;
...
void
CuttingPlane::tcl_command(GuiArgs& args, void* userdata)
{
  if(args.count() < 2)
    {
  ...
  if(args[1] == "findxy")
    {
      need_find.set(1);
      gui_which_plane.set(string("Z index:  "));
      want_to_execute();
    }
  else if(args[1] == "findyz")
    {
      need_find.set(2);
      gui_which_plane.set(string("X index:  "));
      want_to_execute();
    }
  else if(args[1] == "findxz")
    {
      need_find.set(3);
      gui_which_plane.set(string("Y index:  "));
      want_to_execute();
    }
  else if(args[1] == "use_idx")
    {
      find = -1;
      want_to_execute();
    }
