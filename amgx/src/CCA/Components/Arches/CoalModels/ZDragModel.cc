...
ZDragModelBuilder::ZDragModelBuilder( const std::string         & modelName, 
...
ZDragModelBuilder::~ZDragModelBuilder(){}

ModelBase* ZDragModelBuilder::build(){
  return scinew ZDragModel( d_modelName, d_sharedState, d_fieldLabels, d_icLabels, d_scalarLabels, d_quadNode );
}

// End Builder
//---------------------------------------------------------------------------

ZDragModel::ZDragModel( std::string           modelName, 
                              SimulationStateP    & sharedState,
                              ArchesLabel   * fieldLabels,
...
ZDragModel::problemSetup(const ProblemSpecP& params, int qn)
{
  ...
  if (params_root->findBlock("CFD")->findBlock("ARCHES")->findBlock("Coal_Properties")) {
    ...
    throw InvalidValue("ERROR: ZDragmodel: problemSetup(): Missing <Coal_Properties> section in input file!",__FILE__,__LINE__);
  }
  ...
  for (ProblemSpecP variable = db_icvars->findBlock("variable"); variable != 0; variable = variable->findNextBlock("variable") ) {
    ...
    if (role_name == "particle_length") {
    ...
    } else if (role_name == "particle_zvel") {
      LabelToRoleMap[temp_label_name] = role_name;
      ...
      std::string errmsg;
      errmsg = "Invalid variable role for ZDrag model: must be \"particle_length\" or \"particle_zvel\" or \"raw_coal_mass\" or \"char_mass\", you specified \"" + role_name + "\".";
...
//---------------------------------------------------------------------------
void 
ZDragModel::sched_initVars( const LevelP& level, SchedulerP& sched )
{
  std::string taskname = "ZDragModel::initVars";
  Task* tsk = scinew Task(taskname, this, &ZDragModel::initVars);
...
//-------------------------------------------------------------------------
void
ZDragModel::initVars( const ProcessorGroup * pc, 
...
//---------------------------------------------------------------------------
void 
ZDragModel::sched_computeModel( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  std::string taskname = "ZDragModel::computeModel";
  Task* tsk = scinew Task(taskname, this, &ZDragModel::computeModel);
  ...
  for (vector<std::string>::iterator iter = d_icLabels.begin();
    ...
    if ( iMap != LabelToRoleMap.end() ) {
      ...
      } else if ( iMap->second == "particle_zvel" ) {
        if (dqmom_eqn_factory.find_scalar_eqn(*iter) ) {
          EqnBase& t_current_eqn = dqmom_eqn_factory.retrieve_scalar_eqn(*iter);
          DQMOMEqn& current_eqn = dynamic_cast<DQMOMEqn&>(t_current_eqn);
          d_zvel_scaling_factor = current_eqn.getScalingConstant();
        } else {
          std::string errmsg = "ARCHES: ZDragModel: Invalid variable given in <variable> tag for Drag model";
          errmsg += "\nCould not find given particle Z-velocity variable \"";
          errmsg += *iter;
          errmsg += "\" in EqnFactory or in DQMOMEqnFactory.";
          throw InvalidValue(errmsg,__FILE__,__LINE__);
        }
      ...
      std::string errmsg = "ARCHES: ZDragModel: You specified that the variable \"" + *iter +
...
//---------------------------------------------------------------------------
void
ZDragModel::computeModel( const ProcessorGroup* pc, 
                   const PatchSubset* patches, 
                   const MaterialSubset* matls, 
                   DataWarehouse* old_dw, 
                   DataWarehouse* new_dw )
    ...
    for (CellIterator iter=patch->getCellIterator(); !iter.done(); iter++){
      ...
      if( weight[c] < d_w_small && !d_unweighted) {
        ...
        if(!(rho_factor>=0 && rho_factor<=1.0)){
          ...
          //          cout <<"Z_rho_factor =" <<rho_factor <<", c"<<c<<" charmass="<<w_char_mass[c]<<" ,"<<char_mass<<" rcmass="<<w_raw_coal_mass[c]<<" ,"<<rc_mass<<" ,"<<rc_mass_init[d_quadNode]<<" weights="<<weight[c]<<", lenght="<<length<<endl;
          rho_factor =1;
        }
