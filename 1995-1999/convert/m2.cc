...
void dump_frame(double* data, int n, int frame)
{
	...
	for(int j=0;j<n;j++){
	    int idx=i*n+j;
	    if(data[idx] > 0){
		out[j*3]=out[j*3+1]=out[j*3+2]=0;
	    } else {
		out[j*3]=out[j*3+1]=out[j*3+2]=255;
	    }
	}
...
#if 0
	    if(x>0)
		from_obj(i,j)=Sqrt((x-0.5)*(x-0.5)+y*y)-0.3;
	    else
		from_obj(i,j)=Sqrt((x+0.5)*(x+0.5)+y*y)-0.3;
#else
	    from_obj(i,j)= Max(Abs(x),Abs(y))-0.7;
#endif
#endif
	}
    }    
