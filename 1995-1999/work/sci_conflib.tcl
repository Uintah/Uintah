...
itcl_class ConfigDir {
	...
	if {$rebuild_this_symlinks || ($value == "lib" && $rebuild_all_symlinks)} {
	    ...
	    puts "root is $root"
	    if {[catch {set files [glob $root/Imakefile $root/*.icc $root/*.c $root/*.cc $root/*.h $root/*.tcl $root/lib*.o $root/lib*.a]}]} {
		set files ""
	    }
	    foreach t $files {
		...
		set rootsub $root/$subdir
		    ...
		    exec mkdir $dir/$subdir
		}
	    	foreach t $files {
			exec ln -s $t $dir/$subdir/[file tail $t]
	    	}
	    }
	}
	set orig_value $value
