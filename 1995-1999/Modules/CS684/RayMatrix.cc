...
public:
    ...
    RayMatrix(const RayMatrix&, int deep);
    virtual ~RayMatrix();
...
RayMatrix::RayMatrix(const RayMatrix& copy, int deep)
: Module(copy, deep), tclFName("tclFname", id, this),
  tclSpectrum("tclSpectrum", id, this), tclLType("tclLType", id, this),
  tclDiff("tclDiff", id, this), tclSpec("tclSpec", id, this),
  tclMin("tclMin", id, this), tclMax("tclMax", id, this),
  tclNum("tclNum", id, this), scale("scale", id, this)
{
    NOT_FINISHED("RayMatrix::RayMatrix");
}
...
Module* RayMatrix::clone(int deep)
{
    return scinew RayMatrix(*this, deep);
}
...
void RayMatrix::execute()
{
	...
	cerr << "Here's RM...\n";
	cerr << "-- ks -- : ";
	for (int ii=0; ii<rm->ks.size(); ii++) cerr <<rm->ks[ii]<<" ";
	cerr << "\n";
	cerr << "-- kd --: ";
	...
	for (ii=0; ii<rm->pix.dim1(); ii++) {
		...
		for (int kk=0; kk<ne; kk++) cerr << rm->pix(ii,jj).E[kk] <<" ";
		cerr << "\n";
//		cerr << "  S0: ";
...
// before...
	...
	if (tclCommand == "changefile") {
		...
		for (int i=0; i<rm->lightName.size(); i++) 
		...
		for (int i=0; i<rm->matlName.size(); i++) 
