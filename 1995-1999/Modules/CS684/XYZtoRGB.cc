...
public:
    ...
    XYZtoRGB(const XYZtoRGB&, int deep);
    virtual ~XYZtoRGB();
...
XYZtoRGB::XYZtoRGB(const XYZtoRGB& copy, int deep)
: Module(copy, deep),
  WX("wx", id, this), WY("wy", id, this), 
  RX("rx", id, this), RY("ry", id, this), 
  GX("gx", id, this), GY("gy", id, this), 
  ...
  MAX("max", id, this)
{
    NOT_FINISHED("XYZtoRGB::XYZtoRGB");
}
...
Module* XYZtoRGB::clone(int deep)
{
    return scinew XYZtoRGB(*this, deep);
}
