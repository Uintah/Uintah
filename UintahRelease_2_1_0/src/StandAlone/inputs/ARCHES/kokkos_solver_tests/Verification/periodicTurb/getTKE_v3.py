...
for element in TS: 
    ...
    the_command = './lineextract -v x-mom_Filter -timestep '+str(element)+' -istart 0 0 0 -iend '+str(P0)+' '+str(P1)+' '+str(P2)+' -o ufvelTKE.'+str(count)+' -uda '+uda_name              
    os.system(the_command)
    ...
    os.system(the_command)
    the_command = './lineextract -v y-mom_Filter -timestep '+str(element)+' -istart 0 0 0 -iend '+str(P0)+' '+str(P1)+' '+str(P2)+' -o vfvelTKE.'+str(count)+' -uda '+uda_name              
    os.system(the_command)
    ...
    os.system(the_command)
    the_command = './lineextract -v z-mom_Filter -timestep '+str(element)+' -istart 0 0 0 -iend '+str(P0)+' '+str(P1)+' '+str(P2)+' -o wfvelTKE.'+str(count)+' -uda '+uda_name              
    os.system(the_command)
    ...
    os.system('mv ufvelTKE.* '+this_dir+'/.')
    os.system('mv vfvelTKE.* '+this_dir+'/.')
    os.system('mv wfvelTKE.* '+this_dir+'/.')
    ...
    fu = np.loadtxt(this_dir+'/ufvelTKE.'+str(count))[:,3].reshape([P0,P1,P2])
    fv = np.loadtxt(this_dir+'/vfvelTKE.'+str(count))[:,3].reshape([P0,P1,P2])
    fw = np.loadtxt(this_dir+'/wfvelTKE.'+str(count))[:,3].reshape([P0,P1,P2])

    r = tke_spectrum(u,v,w,L)
    fr = tke_spectrum(fu,fv,fw,L)

    plt.loglog(r[1:P0-1,0],r[1:P0-1,1],'o-')
    plt.loglog(fr[1:P0-1,0],fr[1:P0-1,1],'s-')
