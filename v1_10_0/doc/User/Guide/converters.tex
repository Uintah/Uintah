...
Converters export/import \dfn{node location files}, \dfn{connectivity
  ...
  matrix files}, \dfn{sparse row matrix files} and \dfn{color map files}.
...
optional.  A converter command line option (\option{-noEdgesCount},
\option{-noTrisCount}, \option{-noQuadsCount}, \option{-noTetsCount},
  or \option{-noHexesCount}) is used to omit \verb|N|
...
mesh.  For example, a \filename{pts} file for a structured curve field
...
and a \filename{pts} file for a structured quadrilateral surface field
...
and a \filename{pts} file for a structured hexahedral volume field
...
option \option{-noHeader} must be used.  It tells the converter
(\command{TextToStructCurveField},
...
On export, option \option{-noHeader} tells
\command{DenseRowMatrixToText} to omits \verb|N| and \verb|M| from the
output file..
...
import then option \option{-noHeader \ptext{nrows} \ptext{ncols} \ptext{nentries}} must be
passed to \command{TextToSparseRowMatrix}.  
...
(\filename{pts}) file, and \ptext{edges}, \ptext{hexes}, \ptext{quads},
\ptext{tets}, and \ptext{tris} are the names of node connectivity
text files.
