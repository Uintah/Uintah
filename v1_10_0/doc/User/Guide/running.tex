...
The \command{scirun} command may take 1 argument, which is the name of
a \dfn{\sr{} network file}.  (these files use a \filename{.net}
...
Typing \keyboard{scirun} with no arguments (argument is the name of a
\sr{}\dfn{network}\index{network} file) starts up \sr{} with a blank
...
\begin{description}
  ...
  \begin{description}
    ...
    \begin{description}
      \menuitemdesc{Save} Saves the network to a file (See \hyperref{this
  ...
  \begin{description}
    \descitem{...}  There may be other package menus if other packages
...
After starting, \sr{} will run a shell-like application in the
...
\begin{description}
  ...
  two variables specify the complete path to a \sr{} \dfn{data set}.
...
\end{verbatim}
  ...
  \envitem{LOAD\_PACKAGE} A comma seperated list of package names that
  \sr{} load.
...
\sr{} loads these packages instead of packages specified at configure time. Each package on the list must have been previously compiled.
  ...
  list.  \ptext{build\_dir} is the directory in which the
  \command{configure} and \command{make} command was run.
