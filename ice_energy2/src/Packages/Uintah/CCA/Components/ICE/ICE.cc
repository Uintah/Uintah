...
//  t->requires(Task::OldDW, lb->delTLabel);  FOR AMR
  ...
  t->requires(Task::OldDW, lb->int_eng_CCLabel,      ice_matls, gac,1);
...
//      cout << " Aggressive delT Based on currant number "<< delt_CFL << endl;
      } else if (d_delT_scheme == "conservative") {  
                                      ...
                                      DataWarehouse* new_dw)
{
  ...
  for(int p=0;p<patches->size();p++){
    ...
    constCCVariable<double> rho_CC, sp_vol_CC, int_eng;
    CCVariable<double> cv, gamma, Temp;
                                  ...
                                  DataWarehouse* new_dw)
{
  ...
  for(int p=0;p<patches->size();p++){
    ...
    for(int m = 0; m < numALLMatls; m++) {
     ...
     if(ice_matl)  {               //  I C E
      constCCVariable<double> rho_CC, int_eng_CC, cv, int_eng_source;
       ...
       new_dw->get(modelEng_src, lb->modelEng_srcLabel, indx, patch, gn, 0);
        ...
        for(CellIterator iter = patch->getExtraCellIterator(); !iter.done(); 
         ...
         IntVector c = *iter;
          ...
          double min_int_eng = min_mass * int_eng_CC[c];

          //  Glossary:
          //  int_eng    = the amount of internal energy for this
                                 ...
                                 const double AMR_subCycleProgressVar)
{
  ...
  for(int p=0;p<patches->size();p++){
    ...
    for (int m = 0; m < numMatls; m++ ) {
      ...
      int_eng.initialize(0.987654321);
