#ifndef UINTAH_COMPONENTS_SCHEDULERS_PATCHBASEDDATAWAREHOUSE3_H
#define UINTAH_COMPONENTS_SCHEDULERS_PATCHBASEDDATAWAREHOUSE3_H

#include <Core/Thread/CrowdMonitor.h>

#include <Packages/Uintah/CCA/Ports/DataWarehouse.h>
#include <Packages/Uintah/CCA/Components/Scheduler3/PatchBasedDataWarehouse3P.h>
#include <Packages/Uintah/CCA/Components/Scheduler3/DWDatabase3.h>
...
class PatchBasedDataWarehouse3 : public DataWarehouse {
public:
   PatchBasedDataWarehouse3( const ProcessorGroup* myworld,
			  Scheduler* scheduler, int generation,
			  const GridP& grid,
			  bool isInitializationDW = false);
   virtual ~PatchBasedDataWarehouse3();
  ...
  virtual void get(SoleVariableBase&, const VarLabel*,
		   const Level* level = 0, int matlIndex = -1);
  ...
  virtual void override(const SoleVariableBase&, const VarLabel*,
   ...
   void sendMPI(SendState& ss, SendState& rs, DependencyBatch* batch,
		const VarLabel* pos_var, BufferInfo& buffer, 
                PatchBasedDataWarehouse3* old_dw, const DetailedDep* dep);
   ...
   void pushRunningTask(const Task* task, std::vector<PatchBasedDataWarehouse3P>* dws);
   void popRunningTask();  
...
private:
   ...
   struct RunningTaskInfo {
     RunningTaskInfo()
       : d_task(0), dws(0) {}
     RunningTaskInfo(const Task* task, std::vector<PatchBasedDataWarehouse3P>* dws)
       : d_task(task), dws(dws) {}
     ...
     const Task* d_task;
     std::vector<PatchBasedDataWarehouse3P>* dws;
...
#pragma reset woff 1424
  ...
  void checkAccesses(RunningTaskInfo* runningTaskInfo,
   ...
   DWDatabase3<Patch>  d_varDB;
   DWDatabase3<Level>  d_levelDB;
   psetDBType          d_psetDB;
