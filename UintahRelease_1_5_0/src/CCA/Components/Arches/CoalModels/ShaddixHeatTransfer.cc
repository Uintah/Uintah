    ...
    if( iMap != LabelToRoleMap.end() ) {
      if( iMap->second == <insert role name here> ) {
        if( eqn_factory.find_scalar_eqn(*iter) ) {
          ...
          std::string errmsg = "ARCHES: ShaddixHeatTransfer: Invalid variable given in <scalarVars> block for <variable> tag for ShaddixHeatTransfer model.";
      ...
      // can't find this required variable in the labels-to-roles map!
      std::string errmsg = "ARCHES: ShaddixHeatTransfer: You specified that the variable \"" + *iter + 
                           "\" was required, but you did not specify a role for it!\n";
      throw InvalidValue( errmsg, __FILE__, __LINE__);
    }
    */
