...
XDragModel::problemSetup(const ProblemSpecP& params, int qn)
{
  ...
  for (ProblemSpecP variable = db_icvars->findBlock("variable"); variable != 0; variable = variable->findNextBlock("variable") ) {
    ...
    if (role_name == "particle_length") {
      ...
      std::string errmsg;
      errmsg = "Invalid variable role for XDrag model: must be \"particle_length\" or \"particle_xvel\", you specified \"" + role_name + "\".";
 ...
 */
void
XDragModel::dummyInit( const ProcessorGroup* pc,
  ...
  for (int p=0; p < patches->size(); p++){
    ...
    for (CellIterator iter=patch->getCellIterator(); !iter.done(); iter++){
      ...
      if( weight[c] < d_w_small ) {
        ...
        //if (cartPart.x() > 1) {
        //cout << "quad_node " << node  << " cartgasx " << cartGas.x() << " " << "catrpartx " << cartPart.x() << endl;
