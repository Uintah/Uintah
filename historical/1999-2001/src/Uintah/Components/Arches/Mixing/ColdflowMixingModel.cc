...
#include <Uintah/Components/Arches/ColdflowMixingModel.h>
...
//****************************************************************************
void 
ColdflowMixingModel::computeProps(const ProcessorGroup*,
				  ...
				  Stream& outStream)
{
  ...
  for (int ii = 0; ii < d_numMixingVars; ii++ ) {
    local_den += inStream.mixVars[ii]/d_streams[ii].d_density; 
    mixFracSum += inStream.mixVars[ii];
  }
