				    ...
				    Array1<float>&  values)
{
  ...
  index = ComputeAdjustedIndex( variableId, fluidId, isVector);
...
int 
TecplotReader::ComputeAdjustedIndex(int variableId, int fluidId, bool isVector)
{
  int v, rs, rv, fluidIndex = 1;
  int readVectors[20];
  int readScalars[20];
  ...
  for( v = 0; v < variables.size(); v++)
    {
      var = variables[v];
      char *p = var();
      stripVar( var, stripped, fluidIndex );
      if( *p == 'P'){
	...
	readScalars[rs++] = v;
      ...
      if( isVector && rv > variableId && fluidId == fluidIndex)
	return readVectors[ variableId ];
      
      if( !isVector && rs > variableId && fluidId == fluidIndex)
