  ...
  class CoalHelper { 
    ...
    public: 
      ...
      void parse_for_coal_info( ProblemSpecP& db ){ 
        ...
        //We aren't going to throw an error here is Coal properties aren't found 
        if ( db_root->findBlock("CFD")->findBlock("ARCHES")->findBlock("Coal") ){ 
          if ( db_root->findBlock("CFD")->findBlock("ARCHES")->findBlock("Coal")->findBlock("Properties") ){ 
            ...
            throw ProblemSetupException("Error: <Coal> is turned on but the coal <Properties> node is not found.", __FILE__, __LINE__); 
          }
        }
      }
