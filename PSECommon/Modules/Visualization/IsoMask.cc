...
//#include <PSECore/CommonDatatypes/ScalarFieldRGchar.h>
...
IsoMask::IsoMask(const clString& id)
...
//	return;
    ...
    mask->get_bounds(bmmin, bmmax);
    if(bmmin != bmin || bmmax != bmax){
	cerr << "Error -- mask doesn't match field.\n";
	return;
    }
...
//	widget_sphere->rad=1*widget_scale;
	...
	if(have_seedpoint.get()){
	    iso_reg_grid(regular_grid, sp, group, mask);
	} else {
	    iso_reg_grid(regular_grid, iv, group, mask);
	}
	tgroup->add(group);
...
//	    iso_tetrahedra_strip(unstructured_grid,iv,tgroup);
	...
	error("I can't IsoMask this type of field...");
    }
	...
	IsoMask_id=ogeom->addObj(topobj, surface_name);
	if (emit_surface.get()) {
...
void IsoMask::order_and_add_points(const Point &p1, const Point &p2, 
...
//The IsoMask code needs to be able to create clockwise ordered triangles for it to be useful to me.  Clearly we have the information to judge which side of any triangle we generate is inside -- the side which has positive values.   
...
int IsoMask::iso_cube(int i, int j, int k, double isoval,
...
int IsoMask::iso_tetra(Element* element, Mesh* mesh,
...
void IsoMask::iso_tetrahedra(ScalarFieldUG* field, double isoval,
				GeomTrianglesP* group)
{
...
void IsoMask::iso_tetrahedra_strip(ScalarFieldUG* field, double isoval,
				      GeomGroup* group)
{
    Mesh* mesh=field->mesh.get_rep();
    int nelems=mesh->elems.size();
...
int IsoMask::iso_strip_enter(int inc, 
...
void IsoMask::remap_element(int& rval, Element *src, Element *dst)
{
...
int IsoMask::iso_tetra_s(int nbr_status,Element *element, Mesh* mesh, 
			    ScalarFieldUG* field, double isoval, 
			    GeomTriStripList* group)
{
    double v1=field->data[element->n[0]]-isoval;
...
void IsoMask::iso_tetra_strip(int ix, Mesh* mesh, 
...
void IsoMask::iso_tetrahedra_strip(ScalarFieldUG* field, const Point& p,
				      GeomGroup* group)
{
    Mesh* mesh=field->mesh.get_rep();
    int nelems=mesh->elems.size();
...
void IsoMask::iso_tetrahedra(ScalarFieldUG* field, const Point& p,
				GeomTrianglesP* group)
{
    Mesh* mesh=field->mesh.get_rep();
    int nelems=mesh->elems.size();
...
void IsoMask::find_seed_from_value(const ScalarFieldHandle& /*field*/)
{
    NOT_FINISHED("IsoMask::find_seed_from_value");
#if 0
    int nx=field->get_nx();
    int ny=field->get_ny();
    int nz=field->get_nz();
...
void IsoMask::widget_moved(int last)
{
    if(last && !abort_flag){
	abort_flag=1;
	want_to_execute();
