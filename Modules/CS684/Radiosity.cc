...
public:
    ...
    Radiosity(const Radiosity&, int deep);
    virtual ~Radiosity();
...
Radiosity::Radiosity(const Radiosity& copy, int deep)
: Module(copy, deep), ns("ns", id, this), nl("nl", id, this), 
  ni("ni", id, this), raderr("raderr", id, this),
  cscale("cscale", id, this), nrmls("nrmls", id, this), lnx("lnx", id, this),
  vissamp("vissamp", id, this), ffsamp("ffsamp", id, this)
{
    NOT_FINISHED("Radiosity::Radiosity");
}
...
Module* Radiosity::clone(int deep)
{
    return scinew Radiosity(*this, deep);
}
