...
void DRaytracer::preRayTrace() {
    ...
    for (int ii=0; ii<scene.obj.size(); ii++)
	...
	if (mtlh->emitting) {	
	    for (int j=0; j<irm->lightName.size(); j++) {
	...
	for (int j=0; j<irm->matlName.size(); j++) {
   ...
   // diagonalize!!!
	    for (int kk=0; kk<nm; kk++) mm[kk][kk]=1;
...
void bldAndShuffle(Array1<int>& shuffle, MusilRNG& mr) {
    for (int smpl=0; smpl<shuffle.size(); smpl++) shuffle[smpl]=smpl;
