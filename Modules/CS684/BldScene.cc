...
public:
    ...
    BldScene(const BldScene&, int deep);
    virtual ~BldScene();
...
BldScene::BldScene(const BldScene& copy, int deep)
: Module(copy, deep),
  atten("atten", id, this), material("material", id, this), nb("nb", id, this),
  tcl_exec(0), widgetMoved(0), matlChanged(0)
{
    NOT_FINISHED("BldScene::BldScene");
}
...
Module* BldScene::clone(int deep)
{
    return scinew BldScene(*this, deep);
}

void BldScene::removeWidgets() {
    for (int i=0; i<sphere_widget_id.size(); i++) {
...
void BldScene::removeGeom() {
    for (int i=0; i<sphere_geom_id.size(); i++) {
