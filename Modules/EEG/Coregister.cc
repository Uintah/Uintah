...
void Coregister::makeBaseTransSurf(SurfaceHandle s) {
    PointsSurface *ps=s->getPointsSurface();
    TriSurface *ts=s->getTriSurface();
    SurfTree *st=s->getSurfTree();
    ScalarTriSurface *ss=s->getScalarTriSurface();
    if (!ps && !ts && !st && !ss) {
...
void Coregister::setTransSurfPts(void) {
    PointsSurface *ps=osh->getPointsSurface();
    TriSurface *ts=osh->getTriSurface();
    SurfTree *st=osh->getSurfTree();
    ScalarTriSurface *ss=osh->getScalarTriSurface();
    if (!ps && !ts && !st && !ss) {
	cerr << "Coregister: unknown surface type.\n";
	return;
    }
