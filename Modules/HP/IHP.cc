...
class IHP : public Module {
    ...
    int np;
    void iso_reg_grid();
    void iso_hp(GeomGroup* group, ScalarFieldHP* hp, double value);
    void iso_cube3(double* oval, Point* ov, int mask, GeomTrianglesP*);
    
public:
    void parallel_reg_grid(int proc);
    void iso_hp(ScalarFieldHP*, double value);
    ...
    void iso_hexahedra(ScalarFieldHUG*, double, GeomTrianglesP*);
    void iso_hex(Hexahedron* h, double isoval,
		 GeomTrianglesP* group, ScalarFieldHUG* field);
    ...
    int init;
public:
    IHP(const clString& id);
    IHP(const IHP&, int deep);
    virtual ~IHP();
...
extern "C" {
Module* make_IHP(const clString& id)
{
    return scinew IHP(id);
}
}

//static clString module_name("IHP");
static clString surface_name("IHP");
static clString widget_name("IHP widget");

IHP::IHP(const clString& id)
: Module("IHP", id, Filter), seed_point("seed_point", id, this),
  ...
  show_progress("show_progress", id, this)
{
    ...
    matl=scinew Material(Color(0,.3,0), Color(0,.6,0),
		      Color(.7,.7,.7), 50);
    IHP_id=0;
...
IHP::IHP(const IHP& copy, int deep)
: Module(copy, deep), seed_point("seed_point", id, this),
  have_seedpoint("have_seedpoint", id, this), isoval("isoval", id, this),
  do_3dwidget("do_3dwidget", id, this), emit_surface("emit_surface", id, this),
  show_progress("show_progress", id, this)
{
    NOT_FINISHED("IHP::IHP");
}
...
Module* IHP::clone(int deep)
{
    return scinew IHP(*this, deep);
}
...
//	widget_sphere->rad=1*widget_scale;
    ...
    ScalarFieldHUG* hex_grid=field->getHUG();

    ScalarFieldHP* hp=dynamic_cast<ScalarFieldHP*>(field.get_rep());
    if(hp){
	iso_hp(tgroup, hp, iv); 
    }
...
//	    iso_tetrahedra_strip(unstructured_grid,iv,tgroup);
	...
	if (emit_surface.get()) {
	    cerr << "You suck, this will never work\n";
	    return;
	}
	if(have_seedpoint.get()){
	    cerr << "You suck, this will never work\n";
	    ...
	    GeomTrianglesP* group = scinew GeomTrianglesP;
	    iso_hexahedra(hex_grid, iv, group);
	    tgroup->add(group);
	}

    } else {
	//error("I can't IHP this type of field...");
    }
	...
	IHP_id=ogeom->addObj(topobj, surface_name);
	if (emit_surface.get()) {
...
void IHP::order_and_add_points(const Point &p1, const Point &p2, 
...
//The IHP code needs to be able to create clockwise ordered triangles for it to be useful to me.  Clearly we have the information to judge which side of any triangle we generate is inside -- the side which has positive values.   
...
int IHP::iso_cube(int i, int j, int k, double isoval,
...
void IHP::iso_cube2(int i, int j, int k, double isoval,
			   GeomTrianglesP* group, ScalarFieldRG* field)
{
    double oval[9];
    oval[1]=field->grid(i, j, k)-isoval;
...
void IHP::iso_cube3(double* oval, Point* ov, int mask, GeomTrianglesP* group)
{
    MCubeTable* tab=&mcube_table[mask];
    double val[9];
    Point v[9];
...
void IHP::iso_hex(Hexahedron* h, double isoval,
			 GeomTrianglesP* group, ScalarFieldHUG* field)
{
    double oval[9];
    oval[1]=field->data[h->node_index(0)]-isoval;
    oval[2]=field->data[h->node_index(1)]-isoval;
    oval[3]=field->data[h->node_index(2)]-isoval;
    oval[4]=field->data[h->node_index(3)]-isoval;
    oval[5]=field->data[h->node_index(4)]-isoval;
    oval[6]=field->data[h->node_index(5)]-isoval;
    oval[7]=field->data[h->node_index(6)]-isoval;
    oval[8]=field->data[h->node_index(7)]-isoval;
    ...
    if(mask == 0 || mask == 255)
      return;
    Point ov[9];
    HexMesh* mesh=field->mesh;
    ov[1]=*mesh->find_node(h->node_index(0));
    ov[2]=*mesh->find_node(h->node_index(1));
    ov[3]=*mesh->find_node(h->node_index(2));
    ov[4]=*mesh->find_node(h->node_index(3));
    ov[5]=*mesh->find_node(h->node_index(4));
    ov[6]=*mesh->find_node(h->node_index(5));
    ov[7]=*mesh->find_node(h->node_index(6));
    ov[8]=*mesh->find_node(h->node_index(7));
...
void IHP::parallel_reg_grid(int proc)
{
  int nx=rgfield->nx;
  int ny=rgfield->ny;
  int nz=rgfield->nz;
...
void IHP::iso_reg_grid()
{
    np=Task::nprocessors();
    Task::multiprocess(np, do_parallel_reg_grid, this);
}

void IHP::iso_reg_grid(ScalarFieldRG* field, const Point& p,
...
int IHP::iso_tetra(Element* element, Mesh* mesh,
...
void IHP::iso_tetrahedra(ScalarFieldUG* field, double isoval,
				GeomTrianglesP* group)
{
...
void IHP::iso_hexahedra(ScalarFieldHUG* field, double isoval,
			       GeomTrianglesP* group)
{
    HexMesh* mesh=field->mesh;
    int elem=1;
    Hexahedron* h;
    while((h=mesh->find_element(elem++)))
	iso_hex(h, isoval, group, field);
}

// build tri-strips from iso-value


void IHP::iso_tetrahedra_strip(ScalarFieldUG* field, double isoval,
				      GeomGroup* group)
{
    Mesh* mesh=field->mesh.get_rep();
    int nelems=mesh->elems.size();
...
int IHP::iso_strip_enter(int inc, 
...
void IHP::remap_element(int& rval, Element *src, Element *dst)
{
...
int IHP::iso_tetra_s(int nbr_status,Element *element, Mesh* mesh, 
			    ScalarFieldUG* field, double isoval, 
			    GeomTriStripList* group)
{
    double v1=field->data[element->n[0]]-isoval;
...
void IHP::iso_tetra_strip(int ix, Mesh* mesh, 
...
void IHP::iso_tetrahedra_strip(ScalarFieldUG* field, const Point& p,
				      GeomGroup* group)
{
    Mesh* mesh=field->mesh.get_rep();
    int nelems=mesh->elems.size();
...
void IHP::iso_tetrahedra(ScalarFieldUG* field, const Point& p,
				GeomTrianglesP* group)
{
    Mesh* mesh=field->mesh.get_rep();
    int nelems=mesh->elems.size();
...
void IHP::find_seed_from_value(const ScalarFieldHandle& /*field*/)
{
    NOT_FINISHED("IHP::find_seed_from_value");
#if 0
    int nx=field->get_nx();
    int ny=field->get_ny();
    int nz=field->get_nz();
...
void IHP::widget_moved(int last)
{
    if(last && !abort_flag){
	abort_flag=1;
	want_to_execute();
...
void IHP::iso_hp(GeomGroup* group, ScalarFieldHP* hp, double iv)
{
  int nr=hp->images.size()-1;
  int ny=hp->height;
  int nx=hp->width;
  ...
  for(int r=0;r<nr-1;r++){
      cerr << "r=" << r << '\n';
      unsigned char* buf0=hp->images[r];
      unsigned char* buf1=hp->images[r+1];
      double theta0=(double)r*M_PI/(nr-1);
      double theta1=(double)(r+1)*M_PI/(nr-1);
      double xr0=cos(theta0);
      double yr0=sin(theta0);
      double xr1=cos(theta1);
      double yr1=sin(theta1);
      for(int y=0;y<ny-1;y++){
	  int idx0=y*nx;
	  int idx1=idx0+nx;
	  double z0=(double)y/(double)(ny-1);
	  double z1=(double)(y+1)/(double)(ny-1);
	  for(int x=0;x<nx-1;x++){
	      double r0=(double)x/(double)(nx-1);
	      double r1=(double)(x+1)/(double)(nx-1);
	      r0=r0*2-1;
	      r1=r1*2-1;
	      ov[1]=Point(xr0*r0, yr0*r0, z0);
	      ov[2]=Point(xr1*r0, yr1*r0, z0);
	      ov[3]=Point(xr1*r0, yr1*r0, z1);
	      ov[4]=Point(xr0*r0, yr0*r0, z1);
	      ov[5]=Point(xr0*r1, yr0*r1, z0);
	      ov[6]=Point(xr1*r1, yr1*r1, z0);
	      ov[7]=Point(xr1*r1, yr1*r1, z1);
	      ov[8]=Point(xr0*r1, yr0*r1, z1);

	      oval[1]=buf0[idx0+x];
	      oval[2]=buf1[idx0+x];
	      oval[3]=buf1[idx1+x];
	      oval[4]=buf0[idx1+x];
	      oval[5]=buf0[idx0+x+1];
	      oval[6]=buf1[idx0+x+1];
	      oval[7]=buf1[idx1+x+1];
	      oval[8]=buf0[idx1+x+1];
	      int mask=0;
	      for(int idx=1;idx<=8;idx++){
		  if(oval[idx] > 199.99){
		      mask=0;
