...
double GeomObj::depth(DrawInfo*)
{
    cerr << "Error: depth called on an object which isn't a primitive!\n";
    return 0;
}

void GeomObj::objdraw_X11(DrawInfo*)
{
    cerr << "Error: objdraw_X11 called on an object which isn't a primitive!\n";
}

void GeomObj::draw_X11(DrawInfo* di)
{
    unsigned long save_pixel;
    if(matl){
	di->stack.push(matl);
	save_pixel=di->current_pixel;
	di->current_pixel=WhitePixelOfScreen(DefaultScreenOfDisplay(di->dpy));
//	NOT_FINISHED("Color calculation");
	XSetForeground(di->dpy, di->gc, di->current_pixel);
    }
    objdraw_X11(di);
    if(matl){
	di->stack.pop();
	if(di->stack.size()>0){
	    di->current_pixel=save_pixel;
	    XSetForeground(di->dpy, di->gc, di->current_pixel);
	}
    }
}
...
void ObjTransform::make_prims(Array1<GeomObj*>& free,
			      Array1<GeomObj*>& dontfree)
{
    NOT_FINISHED("ObjTransform::make_prims");
...
void ObjGroup::make_prims(Array1<GeomObj*>& free,
			 Array1<GeomObj*>& dontfree)
{
    for(int i=0;i<objs.size();i++)
...
void Triangle::make_prims(Array1<GeomObj*>&,
			  Array1<GeomObj*>& dontfree)
{
    Triangle* tri=this;
    dontfree.add(tri);
...
double Triangle::depth(DrawInfo* di)
{
...
void Triangle::objdraw_X11(DrawInfo* di)
{
...
void Tetra::make_prims(Array1<GeomObj*>& free,
		       Array1<GeomObj*>&)
{
			    ...
			    Array1<GeomObj*>&)
{
	...
	for(int j=0;j<nv-1;j++){
	    ...
	    if(j>0){
		Triangle* t1=new Triangle(l1, l2, p1);
		t1->set_matl(matl);
		free.add(t1);
		Triangle* t2=new Triangle(l2, p1, p2);
		t2->set_matl(matl);
		free.add(t2);
	    }
	    l1=p1;
...
void GeomPt::make_prims(Array1<GeomObj*>&,
			Array1<GeomObj*>& dontfree)
{
    GeomPt* point;
    dontfree.add(point);
...
double GeomPt::depth(DrawInfo* di)
{
    return di->transform->project(p1).z();
}

void GeomPt::objdraw_X11(DrawInfo* di)
{
    Point t1(di->transform->project(p1));
    int x=Round(t1.x());
    int y=Round(t1.y());
    XFillRectangle(di->dpy, di->win, di->gc, x-1, y-1, 3, 3);
}
...
double GeomLine::depth(DrawInfo* di)
{
    double d1=di->transform->project(p1).z();
    double d2=di->transform->project(p2).z();
    return Min(d1, d2);
...
void GeomLine::objdraw_X11(DrawInfo* di)
{
    Point t1(di->transform->project(p1));
    Point t2(di->transform->project(p2));
    int x1=Round(t1.x());
			      ...
			      Array1<GeomObj*>&)
{
    ...
    int n=pts.size()-2;
    for(int i=0;i<n;i++){
	Triangle* tri=new Triangle(pts[i], pts[i+1], pts[i+2]);
	tri->set_matl(matl);
	free.add(tri);
    }
}
