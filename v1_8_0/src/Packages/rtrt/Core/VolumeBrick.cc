...
#include <Packages/rtrt/Core/VolumeBrick.h>
...
VolumeBrick::VolumeBrick(Material* matl, VolumeDpy* dpy, char* filebase)
  : VolumeBase(matl, dpy)
{
  char buf[200];
  ...
  read(fd, indata, sizeof(float)*nx*ny*nz);
...
void VolumeBrick::preprocess(double, int&, int&)
{
    int nx1=nx-1;
    int ny1=ny-1;
    int nz1=nz-1;
...
#define L2 6
    ...
    blockdata=new float[totalx*totaly*totalz*L2*L2*L2*L1*L1*L1];
    for(int x=0;x<nx;x++){
	int m1x=x%L1;
	int xx=x/L1;
	int m2x=xx%L2;
	...
	for(int y=0;y<ny1;y++){
	    int idx=x*nynz+y*nz;
	    for(int z=0;z<nz1;z++){
		float p000=indata[idx];
		float p001=indata[idx+1];
		float p010=indata[idx+nz];
		float p011=indata[idx+nz+1];
		float p100=indata[idx+nynz];
		float p101=indata[idx+nynz+1];
		float p110=indata[idx+nynz+nz];
		float p111=indata[idx+nynz+nz+1];
...
void VolumeBrick::compute_bounds(BBox& bbox, double offset)
{
    bbox.extend(min-Vector(offset,offset,offset));
    bbox.extend(min+diag+Vector(offset,offset,offset));
}

Vector VolumeBrick::normal(const Point&, const HitInfo& hit)
{
...
void VolumeBrick::compute_hist(int nhist, int* hist,
...
void VolumeBrick::get_minmax(float& min, float& max)
{
    min=datamin;
    max=datamax;
}
