...
void
DataArchive::queryLifetime( double& /*min*/, double& /*max*/, const Patch* /*patch*/)
{
   cerr << "DataArchive::lifetime not finished\n";
}
...
void
DataArchive::PatchHashMaps::init(XMLURL tsUrl, DOM_Node tsTopNode,
				 int processor, int numProcessors)
{
   d_isParsed = false;
...
int
DataArchive::queryNumMaterials( const string& name,
				const Patch* patch,
				double time)
{
   double start = Time::currentSeconds();
