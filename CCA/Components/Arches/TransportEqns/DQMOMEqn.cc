...
/*
  if( !d_weight ) {
    if (timeSubStep == 0) {
      tsk->requires(Task::OldDW, d_weightLabel, Ghost::None, 0);
    } else {
...
/*  
    ...
    if(!d_weight){
      if (new_dw->exists(d_weightLabel, matlIndex, patch)) {
        new_dw->get(w, d_weightLabel, matlIndex, patch, gn, 0); // only get new_dw value on rkstep > 0
      } else {
        old_dw->get(w, d_weightLabel, matlIndex, patch, gn, 0);
      }
    }
...
//---------------------------------------------------------------------------
void
DQMOMEqn::sched_TransportEqnBC( const LevelP& level, SchedulerP& sched, int timeSubStep )
{
  string taskname = "DQMOMEqn::TransportEqnBC";

  Task* tsk = scinew Task(taskname, this, &DQMOMEqn::TransportEqnBC, timeSubStep);
...
//---------------------------------------------------------------------------
// Method: Actually the transport equation BC. 
//---------------------------------------------------------------------------
void
DQMOMEqn::TransportEqnBC( const ProcessorGroup* pc,
                             const PatchSubset* patches,
                             const MaterialSubset* matls,
                             DataWarehouse* old_dw,
                             DataWarehouse* new_dw,
