 ...
 *         given a velocity field \f$\mathbf{u}\f$. We write the
 *         convective flux in conservation form as \f$ J_i = \rho
 *         \varphi u_i = \phi u_i \f$ where \f$i=1,2,3\f$ is the
 *         coordinate direction. This requires knowledge of the
 *         velocity field.
 *
 ...
 *           \f$\frac{\partial \phi}{\partial x}\f$.  Moves the
 *           volume-centered field \f$\phi\f$ to a face centered
 *           field.
...
void ConvectiveFlux<PhiInterpT, VelInterpT>::evaluate()
{
  ...
  // set the advective velocity in the interpolant operator.
  ...
  // calculate the convective flux: multiply phi by the velocity
  result *= *velInterp;
}
