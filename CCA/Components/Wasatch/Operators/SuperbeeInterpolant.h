#ifndef SuperbeeInterpolant_h
#define SuperbeeInterpolant_h
...
/**
 *  \class     SuperbeeInterpolant
 *  \author    Tony Saad
 *  \date      January 2011
 *
 *  \brief     Calculates convective flux using Superbee interpolation.
 *  
 *  This class is a lightweight operator, i.e. it does NOT implement a
 *  matvec operation. The SuperbeeInterpolant will interpolate the
 ...
 *  field. The SuperbeeInterpolant will interpolate the volume field to
 *  its corresponding face field. Thus, if \phi denotes a scalar
 *  field, then the SuperbeeInterpolant will produce a scalar face
 *  field. Similarly, if \phi was an X-Staggered volume field, then
 *  the SuperbeeInterpolant will produce an X-Staggered face field.
 * 
 *  Based on this convention, the SuperbeeInterpolant class is templated
...
private:
  ...
  //
  std::vector<size_t> xyzCountBnd_; // counter for faces at the boundary  
  std::vector<size_t> xyzVolIncrBnd_;
  std::vector<size_t> xyzFaceIncrBnd_;
...
public:
  ...
  /**
   *  \brief Constructor for superbee interpolant.
  ...
  /**
   *  \brief Destructor for superbee interpolant.
   */  
  ~SuperbeeInterpolant();
  
  /**
   *  \brief Sets the advective velocity field for the superbee interpolator.
   *
  ...
  /**
   *  \brief Applies the Superbee interpolation to the source field.
   *
