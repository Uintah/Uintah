/*
 * File:          whoc_UIPort_Stub.c
 * Symbol:        whoc.UIPort-v1.0
 * Symbol Type:   class
 * Babel Version: 0.7.0
 * SIDL Created:  20020731 15:22:36 MST
...
static const struct whoc_UIPort__external* _getIOR(void)
{
  static const struct whoc_UIPort__external *_ior = NULL;
  if (!_ior) {
#ifdef SIDL_STATIC_LIBRARY
    _ior = whoc_UIPort__externals();
#else
    const struct whoc_UIPort__external*(*dll_f)(void) =
      (const struct whoc_UIPort__external*(*)(void)) SIDL_Loader_lookupSymbol(
        "whoc_UIPort__externals");
    _ior = (dll_f ? (*dll_f)() : NULL);
    if (!_ior) {
      fputs("Unable to find the implementation for whoc.UIPort; please set SIDL_DLL_PATH\n", stderr);
      exit(-1);
    }
#endif
  }
...
whoc_UIPort__cast(
  void* obj)
{
  whoc_UIPort cast = NULL;

  if (obj != NULL) {
    SIDL_BaseInterface base = (SIDL_BaseInterface) obj;
    cast = (whoc_UIPort) (*base->d_epv->f__cast)(
      base->d_object,
      "whoc.UIPort");
  }
...
whoc_UIPort__array_createCol(int32_t        dimen,
                             ...
                             const int32_t upper[])
{
  return (struct whoc_UIPort__array*)SIDL_interface__array_createCol(dimen,
    lower, upper);
}
...
whoc_UIPort__array_createRow(int32_t        dimen,
                             ...
                             const int32_t upper[])
{
  return (struct whoc_UIPort__array*)SIDL_interface__array_createRow(dimen,
    lower, upper);
}
...
{
  return (struct whoc_UIPort__array*)SIDL_interface__array_create1d(len);
}
...
{
  return (struct whoc_UIPort__array*)SIDL_interface__array_create2dCol(m, n);
}
...
{
  return (struct whoc_UIPort__array*)SIDL_interface__array_create2dRow(m, n);
}
...
{
  return (struct whoc_UIPort__array*)SIDL_interface__array_borrow(
...
{
  return (struct whoc_UIPort__array*)
