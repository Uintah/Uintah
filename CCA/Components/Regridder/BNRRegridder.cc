  ...
  for(int l=0; l < oldGrid->numLevels() && l < d_maxLevels-1;l++)
  {
    ...
    //if flags are tighltly bound by old grid level...
      ...
      //continue
		...
		IntVector min_patch_size(4,4,1);		//Needs to be implemented 

    //Coarsen Flags
    set<IntVector> coarse_flag_set;
    for(unsigned int f=0;f<flaglist.size();f++)
    {
      coarse_flag_set.insert(flaglist[f]/min_patch_size);
    }
    ...
    //Uncoarsen
		IntVector mult=min_patch_size*d_cellRefinementRatio[l];
