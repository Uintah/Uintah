...
MPI_Request* BNRTask::getRequest()
{
  ...
  if(controller_->free_requests_.empty())
  {
    ...
    //cout << "rank:" << p_group_[p_rank_] << ": pid:" << tag_ << ": allocating new request\n";
  ...
  {
    //cout << "rank:" << p_group_[p_rank_] << ": pid:" << tag_ << ": reusing request\n";
    //get a free request
...
//    cout << "rank:" << p_group_[p_rank_] << ": " << "pid:" << tag_  << ": clearing flagscount_\n";
  ...
  if(p_group_.size()>1)
  {
    ...
    //cout << "rank:" << p_group_[p_rank_] << ": " << "pid:" << tag_  << ": communicating signatures\n";
    ...
    if(stage_<d_)
    {
      ...
      if(p_rank_<stride)
      {
        ...
        if(partner<p_group_.size())
        {
          ...
          //cout << "rank:" << p_group_[p_rank_] << ": pid:" << tag_ << ": summing count\n";      
          for(int d=0;d<3;d++)
          {
...
*/
  ...
  else
  {
    ...
    /*
    cout << "rank:" << p_group_[p_rank_] << ": Child patches:\n";
...
void BNRTask::continueTaskSerial()
{
  ...
  {
    //cout << "rank:" << p_group_[p_rank_] << ": " << "pid:" << tag_  << ": activating parent_ on tag_:" << parent_->tag_ << endl;
    //place parent_ on immediate queue 
    controller_->immediate_q_.push(parent_);
  }
