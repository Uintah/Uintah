function Alist = distributeFineFluxes(grid,k,q,ilower,iupper)
%DISTRIBUTEFINEFLUXES  Distribute C/F interface fine fluxes among coarse
%   equations.
...
for d = 1:grid.dim,                                             % Loop over dimensions of patch
    ...
    for side = 1:2                                              % side=1 (left) and side=2 (right) in dimension d
        ...
        if (edgePatch{side}(d) == edgeDomain{side}(d))
        ...
        % Convert ghost indices to level-based
        for dim = 1:grid.dim
            ghost{dim} = ghost{dim} - boxOffset(dim);
        end
        [matGhost{:}]   = ndgrid(ghost{:});

        % Create linear interpolation stencil for each ghost point
        % Checked for 2-D; check for other dims as well later on.
        r = grid.level{k}.refRatio;
        subGhost = reshape([matGhost{:}],[length(matGhost{1}) grid.dim]);   % Ghost point level-based subscripts
        cGhost = subGhost./repmat(r,size(subGhost)./size(r));
        cNodesLeft = floor(cGhost);                                         % Coarse nbhrs on the left and down of ghost points = base of interpolation stencil
        x = cGhost + 0.5./repmat(r,size(cGhost)./size(r));     % Ghost points coarse indices relative to cNodesLeft
        weight = ones(size(x,1),1);
        for dim = setdiff(grid.dim:-1:1,d)
            cNodesRight = cNodesLeft;
            cNodesRight(:,dim) = cNodesRight(:,dim)+1;
            xLeft = cNodesLeft(:,dim);
            xRight = cNodesRight(:,dim);
            xGhost = x(:,dim);
            newWeight = zeros(size(weight,1),2*size(weight,2));
            for m = 1:size(weight,2)
                newWeight(:,2*m-1) = weight(:,m);
                newWeight(:,2*m) = weight(:,m);
            end
            weight = newWeight.* ...
        ...
        % the same order as the one obtained from dec2bin(0:2^nDims-1).
        b = dec2bin(0:2^(grid.dim-1)-1,grid.dim-1);
        for m = 1:size(b,1),
            cNodesRight = cNodesLeft;
            %cNodesRight
            for dim = 1:grid.dim
                %dim
                if (dim < d)
                    cNodesRight(:,dim) = cNodesRight(:,dim) + b(m,dim) - '0';
                end
                if (dim > d)
                    %b(m,dim-1)
                    cNodesRight(:,dim) = cNodesRight(:,dim) + b(m,dim-1) - '0';
                end
                %cNodesRight
            end
            if (direction > 0)
                cNodesRight(:,d) = cNodesRight(:,d) + 1;                % If this is a right face, add one to the left coarse nbhrs so that they will be outside the fine patch
            end
            cNbhr    = cell(grid.dim,1);
            for dim = 1:grid.dim
                cNbhr{dim} = cNodesRight(:,dim) + coarseOffset(dim);  % Patch-based cell indices including ghosts
            end
            pindexCNbhr   = sub2ind(Q.size,cNbhr{:});                      % Patch-based cell indices - list
            mapCNbhr      = coarseMap(pindexCNbhr);
            
            Alist = [Alist; ...
                [mapCNbhr mapGhost  weight(:,m).*f]; ...
                [mapCNbhr mapNbhr  -weight(:,m).*f] ...
                ];
            %[mapCNbhr mapGhost  weight(:,m).*f]
        end
