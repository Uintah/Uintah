...
\section{\underline{Overview of implicit ICE\hspace{ 0.5in}10/04/04}  }
...
\WHILE{ $| \text{outer residual} | <$ convergence criteria}
    ...
    \STATE --Form RHS $B = -\beta (P^{ \iter} - P^{old}) - \SUM{m}$ \text{Advection} $(\theta,\velFC)$,\\ \quad
...
\STATE --Compute $\velFC$ add exchange contribution, set boundary conditions on $\velFC$
\end{algorithmic}
%______________________________________________________________________
\newpage
