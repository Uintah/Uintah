...
\section{\underline{Overview of implicit ICE\hspace{ 0.5in}07/7/05}  }

\begin{algorithmic}
\STATE --Compute $ \velFC$ (add Exchange contribution, set boundary
             conditions on $\velFC$ )
             
\STATE --Form RHS $B = -\SUM{m} \theta_m \kappa_m (P^{ \iter} - P^{old}) - \SUM{m}$ \text{Advection} $(\theta_m \velFC)$
...
\WHILE{ $| \text{outer residual} | <$ convergence criteria}
    ...
    \STATE --Compute $A$ matrix \\ \quad
      ...
      $A_{p} =\SUM{m} \theta_m \kappa_m +( A_r + A_l + A_t + A_b + A_f + A_ {bk})$
    ...
    \STATE --Form RHS $B = -\SUM{m} \theta_m \kappa_m (P^{ \iter} - P^{old}) - \SUM{m}$ \text{Advection} $(\theta_m \velFC)$,\\ \quad
...
%
$\SUM{m} \theta_m \kappa_m - \frac{ \delt^2}{V} \B{[} A_R +A_L+ A_T + A_B + A_F + A_{BK} \B{]} \delta{p}
- 
