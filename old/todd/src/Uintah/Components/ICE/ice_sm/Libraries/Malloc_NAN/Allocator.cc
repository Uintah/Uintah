...
#define MEDIUM_THRESHOLD (256*1024)
...
#define SENT1_VAL_FREE 0xdeadbeef
#define SENT2_VAL_FREE 0xcafebeef
#define SENT1_VAL_INUSE 0xfeedface
#define SENT2_VAL_INUSE 0xfeed2dad

#define NORMAL_OS_ALLOC_SIZE (16*1024*1024)
...
void* Allocator::alloc(size_t size, char* tag)
{
    ...
    sent1->first_word=sent1->second_word=SENT1_VAL_INUSE;
    sent2->first_word=sent2->second_word=SENT2_VAL_INUSE;
...
void* Allocator::alloc_big(size_t size, char* tag)
{
	...
	sent1->first_word=sent1->second_word=SENT1_VAL_FREE;
	sent2->first_word=sent2->second_word=SENT2_VAL_FREE;
	if(strict){
...
//    fprintf(stderr, "sentinels: %x %x %x %x\n", sent1->first_word, sent1->second_word, sent2->first_word, sent2->second_word);
    ...
    if(what == OBJFREE){
	if(sent1->first_word != SENT1_VAL_FREE || sent1->second_word != SENT1_VAL_FREE){
	    if(sent1->first_word == SENT1_VAL_INUSE){
	...
	if(sent2->first_word != SENT2_VAL_FREE || sent2->second_word != SENT2_VAL_FREE){
	    if(sent2->first_word == SENT2_VAL_INUSE){
	...
	if(sent1->first_word != SENT1_VAL_INUSE || sent1->second_word != SENT1_VAL_INUSE){
	    if(sent1->first_word == SENT1_VAL_FREE){
		if(what == OBJFREEING){
	...
	if(sent2->first_word != SENT2_VAL_INUSE || sent2->second_word != SENT2_VAL_INUSE){
	    if(sent2->first_word == SENT2_VAL_FREE){
		if(what == OBJFREEING){
...
void PrintTag(void* dobj)
{
    ...
    } else if(sent1->first_word == SENT1_VAL_INUSE && sent1->second_word == SENT1_VAL_INUSE){
