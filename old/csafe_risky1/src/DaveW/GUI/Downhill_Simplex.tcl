catch {rename DaveW_ISL_Downhill_Simplex ""}
...
itcl_class DaveW_ISL_Downhill_Simplex {
	...
	pack $w.f.label $w.f.t  -side left -pady 2m         
        ...
        button $w.p -text "Pause" -relief raised -command "$this-c pause"
        button $w.np -text "Unpause" -relief raised -command "$this-c unpause"
	
	pack $w.f1 $w.f $w.go $w.p $w.np -fill x 
