...
subroutine cut_and_shift_nonbonded
 use sys_data
 use paralel_env_data
 use boundaries, only : periodic_images
 use ALL_atoms_data, only : xxx,yyy,zzz,Natoms,i_Style_atom, all_charges,i_type_atom,fxx,fyy,fzz
 ...
 use cut_off_data, only : cut_off, cut_off_sq, displacement
   ...
   af_i_1_x=0.0d0; af_i_1_y=0.0d0; af_i_1_z=0.0d0 
    do k =  1, neightot
      ...
      if ( r2 < cut_off_sq ) then
         ...
         if (a0 > 0.0d0) then
                g = (r-(cut_off-displacement))*i_displacement
...
!STOP
 ...
 deallocate(in_list)
 end subroutine cut_and_shift_nonbonded
