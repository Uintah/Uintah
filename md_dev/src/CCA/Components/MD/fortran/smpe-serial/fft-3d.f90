...
module fft_3D
 ...
 contains
      ...
      subroutine dlpfft3(ind,isw,ndiv1,ndiv2,ndiv3,key1,key2,key3,ww1,ww2,ww3,aaa)
...
!     restore exponentials to unconjugated values if necessary
  ...
  subroutine dlpfft3_MOD(ind,isw,ndiv1,ndiv2,ndiv3,key1,key2,key3,ww1,ww2,ww3,aaa)
...
! MODIFIED TO USE 1D ARRAY instead of 3Ds
        ...
        do while(kkk.lt.ndiv1)
          do i=1,num
            ...
            do j=1,ndiv2
              ...
              do k=1,ndiv3
                k1 = k -1
                ndx = (j1+k1*ndiv2)*ndiv1 + k12
                ndx1 = (j1+k1*ndiv2)*ndiv1 + kk1
                ttt=aaa(ndx)*ww1(iii)
                aaa(ndx)=aaa(ndx1)-ttt
                aaa(ndx1)=aaa(ndx1)+ttt
              enddo  !  k=1,ndiv3
            enddo    ! j=1,ndiv2
            kkk=kkk+1
          enddo  !   i=1,num
      ...
      do kkk=1,ndiv1
        ...
        if(iii.gt.kkk)then
          do j=1,ndiv2
            ...
            do k=1,ndiv3
              k1 = k - 1
               ndx = (j1+k1*ndiv2)*ndiv1 + kkk
                ndx1 = (j1+k1*ndiv2)*ndiv1 + iii
               ttt=aaa(ndx)
              aaa(ndx)=aaa(ndx1)
              aaa(ndx1)=ttt
            enddo !k=1,ndiv3
        ...
        do while(kkk.lt.ndiv2)
          do i=1,num
            ...
            do j=1,ndiv1
              ...
              do k=1,ndiv3
                k1 = k - 1
                ndx = (k12-1+k1*ndiv2)*ndiv1 + j     ! aaa(j,k12,k)
                ndx1 = (kk1-1+k1*ndiv2)*ndiv1 + j
                ttt=aaa(ndx)*ww2(iii)
                aaa(ndx)=aaa(ndx1)-ttt
                aaa(ndx1)=aaa(ndx1)+ttt
              enddo
      ...
      do kkk=1,ndiv2
        ...
        if(iii.gt.kkk)then
          do j=1,ndiv1
            ...
            do k=1,ndiv3
              k1  = k - 1
              ndx = (kkk-1+k1*ndiv2)*ndiv1 + j
                ndx1 = (iii-1+k1*ndiv2)*ndiv1 + j
              ttt=aaa(ndx)
              aaa(ndx)=aaa(ndx1)
              aaa(ndx1)=ttt
            enddo
        ...
        do while(kkk.lt.ndiv3)
          do i=1,num
            ...
            do j=1,ndiv1
              ...
              do k=1,ndiv2
                k1 = k - 1
                ndx = (k1+(k12-1)*ndiv2)*ndiv1 + j
                ndx1 = (k1+(kk1-1)*ndiv2)*ndiv1 + j
                ttt=aaa(ndx)*ww3(iii)
                aaa(ndx)=aaa(ndx1)-ttt
                aaa(ndx1)=aaa(ndx1)+ttt
              enddo
     ...
     do kkk=1,ndiv3
        ...
        if(iii.gt.kkk)then
          do j=1,ndiv1
            ...
            do k=1,ndiv2
              k1 = k - 1
               ndx = (k1+(kkk-1)*ndiv2)*ndiv1 + j
                ndx1 = (k1+(iii-1)*ndiv2)*ndiv1 + j
              ttt=aaa(ndx)
              aaa(ndx)=aaa(ndx1)
              aaa(ndx1)=ttt
            enddo
