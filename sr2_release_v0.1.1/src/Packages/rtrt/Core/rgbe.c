...
texture *ReadRGBETexture( char *infilename )
{
  ...
  if (!t) FatalError( "LIBGFX: Unable to allocate memory!" );
  ...
  if (RGBE_ReadHeader( infile, &t->img_width, &t->img_height, NULL )!=
      RGBE_RETURN_SUCCESS)
    {
      Error("LIBGFX: Unable to read header for RGBE file!");
      free( t );
      return NULL;
    }
  
  img_size = 3*t->img_height*t->img_width;
  t->texImagef = (float *)malloc( img_size * sizeof( float ) );
  if (!t->texImagef)
    FatalError("LIBGFX: Unable to allocate memory!");

  if (RGBE_ReadPixels_RLE(infile,t->texImagef,t->img_width,t->img_height)!=
      RGBE_RETURN_SUCCESS)
    {
      Error("LIBGFX: Unable to read pixel data for RGBE file!");
      free( t->texImagef );
      free( t );
      return NULL;
    }
...
static int rgbe_error(int rgbe_error_code, char * msg)
{
  ...
  case rgbe_write_error:
    Error("LIBGFX: Write error on RGBE file!");
    break;
  case rgbe_format_error:
    Error("LIBGFX: Possible bad RGBE format?");
    break;
  default:
  case rgbe_memory_error:
    Error("LIBGFX: Memory error in RGBE code!");
  }
