...
#if 0
static void dump_mesh(Mesh* mesh)
{
...
int MeshOctree::locate(Mesh* mesh, const Point& p, double epsilon1)
{
  for (int i=0;i<elems.size();i++) {
    Element* elem=mesh->element(elems[i]);
...
int MeshGrid::locate(Mesh* mesh, const Point& p, double)
{
  ...
  for (int i=0;i<elems(ii,jj,kk).size();i++) {
    Element* elem = mesh->element(elems(ii,jj,kk)[i]);
    if(mesh->inside(p, elem)) {
...
Mesh::insert_delaunay( int nn )
{
  ...
  for (fiter.first();fiter.ok();++fiter) {
    ...
    Element* ne=new Element(this, nn, f->n[0], f->n[1], f->n[2]);
