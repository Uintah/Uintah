...
\Beq
  \Bsig = \left[p(J) - 3~\Deriv{p(J)}{J}~\alpha~(T-T_0)\right]~\Bone + \Bs ~;~~ 
  J = \det(\BF)~.
\Eeq
...
\Beq
  \dot{\hat{\Bbeta}} = \dot{\gamma}~\Bh^{\beta}
\Eeq
where $\hat{\Bbeta}$ is the back stress.
...
Then
\Beq
  \Bxi_{n+1} = \Bxi^{\Trial} - \Delta\gamma_{n+1}(2~\mu~\Dev(\Br_{n+1}) + \Bh^{\beta}_{n+1}) ~.
\Eeq
...
\Beq
  \Bal
  \Ba(\Delta\gamma) = 0 &= -\Bxi + \Bxi^{\Trial} - \Delta\gamma(2~\mu~\Dev(\Br) + \Bh^{\beta})\\
...
\Beq
  ...
  \Deriv{\Ba}{\Delta\gamma} & = 
   -\Partial{\Bxi}{\Delta\gamma}  - (2~\mu~\Dev(\Br) + \Bh^{\beta})
   - \Delta\gamma~\left(2~\mu~\Partial{\Dev(\Br)}{\Delta\gamma} + 
        \Partial{\Bh^{\beta}}{\Delta\gamma}\right) \\
   & =
   -\Partial{\Bxi}{\Delta\gamma}  - (2~\mu~\Dev(\Br) + \Bh^{\beta})
   - \Delta\gamma~\left(
   ...
   & \qquad \qquad
      \left.
      \Partial{\Bh^{\beta}}{\Bxi}:\Partial{\Bxi}{\Delta\gamma} + 
      \Partial{\Bh^{\beta}}{\Ve^p}~\Partial{\Ve^p}{\Delta\gamma} +
      \Partial{\Bh^{\beta}}{\phi}~\Partial{\phi}{\Delta\gamma} 
      \right) \\
...
\Beq
  \Bal
  \Ba^{(k)} - \Delta\Bxi - [2~\mu~\Dev(\Br^{(k)}) + \Bh^{\beta (k)}]~\delta\gamma
   ...
   \qquad \qquad
   - \Delta\gamma~\left(
      \Partial{\Bh^{\beta (k)}}{\Bxi}:\Delta\Bxi + 
      \Partial{\Bh^{\beta (k)}}{\Ve^p}~\Delta\Ve^p +
      \Partial{\Bh^{\beta (k)}}{\phi}~\Delta\phi
      \right)  & = 0\\
...
\Beq
  \Bal
  \Ba^{(k)} - \Delta\Bxi - [2~\mu~\Dev(\Br_n) + \Bh^{\beta}_n]~\delta\gamma & = 0\\
...
\Beq
  \Bal
  \Delta\Bxi & = - [2~\mu~\Dev(\Br_n) + \Bh^{\beta}_n]~\delta\gamma \\
...
\Beq
  f^{(k)} -\Partial{f^{(k)}}{\Bxi}:[2~\mu~\Dev(\Br_n) + \Bh^{\beta}_n]~\delta\gamma +
...
\Beq
  \Delta\gamma^{(k+1)} - \Delta\gamma^{(k)} = \delta\gamma = 
   \cfrac{f^{(k)}}
   {\Partial{f^{(k)}}{\Bxi}:[2~\mu~\Dev(\Br_n) + \Bh^{\beta}_n] - 
...
\begin{enumerate}
  ...
  \Beq
  \Bal
    \Delta\Bxi^{(k)} & = -[2~\mu~\Dev(\Br_n) + \Bh^{\beta}_n]~\delta\gamma^{(k)} \\
  ...
  \Beq
    ...
    T_{n+1} & = T_n + 
     \cfrac{\chi_{n+1}~\Delta t}{\rho_{n+1}~C_p}~\sigma^{n+1}_y~\dot{\Ve^p}_{n+1} \\
    p_{n+1} & = p(J_{n+1}) \\ 
    \kappa_{n+1} & = \left[\Deriv{p(J)}{J}\right]_{n+1} \\
