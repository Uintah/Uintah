...
#include <Packages/Uintah/CCA/Components/MPM/ConstitutiveModel/PlasticityModels/KinematicaHardeningModelFactory.h>
...
void SmallStrainPlastic::outputProblemSpec(ProblemSpecP& ps,bool output_cm_tag)
{
  ...
  if (output_cm_tag) {
    ...
    cm_ps->setAttribute("type","small_strain_large_rotation_plastic");
  }
                                         ...
                                         DataWarehouse* new_dw)
{
  ...
  for(int patchIndex=0; patchIndex<patches->size(); patchIndex++){
    ...
    for( ; iter != pset->end(); iter++){
      ...
      backStress_old = d_kinematic->getBackStress(idx);
      ...
      if (temperature > Tm_cur || flowStress <= 0.0) {
        ...
        if (f_0 < 0.0) {
          ...
          do {
            ...
            // ep_k, phi_k, beta_k
            double f_k = d_yield->evalYieldCondition(xi_trial, state);
            if (f_k < tolerance) break;
    ...
    if (isnan(g) || isnan(deltaGamma)) {
      ...
      throw ParameterNotFound("**ERROR**:SmallStrainPlastic: Found nan.", __FILE__, __LINE__);
    }
    ...
    if (fabs(deltaGamma-deltaGammaOld) < tolerance || count > 100) break;
          ...
          computeStilde(sigma_dev_trial, delT, matl, idx, Stilde, state, delGamma);

          // Do radial return adjustment
          double stst = sqrtThreeTwo*Stilde.Norm();
          ASSERT(stst != 0.0);
          sigma_dev_old = Stilde*(state->yieldStress/stst);
      ...
      sigma_old = sigma_dev_old + tensorHy;

      // If the particle has already failed, apply various erosion algorithms
      if (flag->d_doErosion) {
        if (pLocalized[idx]) {
          if (d_allowNoTension) {
            if (p > 0.0) sigma_old = zero;
            else sigma_old = tensorHy;
          }
          else if (d_setStressToZero) sigma_old = zero;
        }
      }
      ...
      if (!plastic) {
        ...
        */
        ...
        // Update the porosity
        if (d_evolvePorosity) 
          pPorosity_new[idx] = updatePorosity(rateOfDef_new, delT, pPorosity[idx], 
                                              state->plasticStrain);
        else
          pPorosity_new[idx] = pPorosity[idx];
        ...
        if (d_evolveDamage) 
          pDamage_new[idx] = 
            d_damage->computeScalarDamage(state->plasticStrainRate, sigma_old, 
        ...
        // Check 2 and 3: Look at TEPLA and stability
          ...
          if (d_stable->doIt() && !isLocalized) {
            ...
            if (!(sigY > 0.0)) isLocalized = true;
              ...
              d_yield->computeElasPlasTangentModulus(Ce, sigma_old, sigY, 
                                                     dsigYdep, 
                                                     pPorosity_new[idx],
                                                     A, Cep);
              ...
              isLocalized = d_stable->checkStability(sigma_old, rateOfDef_new, Cep, 
                                                     direction);
            }
          }
        } 
      ...
      sigma_old = (rotation*sigma_old)*(rotation.Transpose());

      // Save the new data
      pStress_new[idx] = sigma_old;
...
////////////////////////////////////////////////////////////////////////
void 
SmallStrainPlastic::computeStilde(const Matrix3& sigma_dev_trial,
                              const double& delT,
                              const MPMMaterial* matl,
                              ...
                              double& delGamma)
{
  double normTrialS = sigma_dev_trial.Norm();
  Matrix3 nn = sigma_dev_trial/normTrialS;
  ...
  delGamma = computeDeltaGamma(delT, tolerance, normTrialS, matl,
                               idx, state);
  Stilde = sigma_dev_trial - nn*(2.0*state->shearModulus*delGamma);
...
////////////////////////////////////////////////////////////////////////
double 
SmallStrainPlastic::computeDeltaGamma(const double& delT,
                                            ...
                                            DataWarehouse* new_dw)
{
  ...
  Matrix3 oldStress(0.0), sigma_dev_trialtress(0.0), devTrialStress(0.0);
  ...
  for(int p=0;p<patches->size();p++){
    ...
    // Get the plastic strain and back stress
    d_plastic->getInternalVars(pset, old_dw);
    d_plastic->allocateAndPutInternalVars(pset, new_dw);
    d_kinematic->getBackStress(pset, old_dw);
    d_kinematic->allocateAndPutBackStress(pset, new_dw);
    ...
    for( ; iter != pset->end(); iter++){
      ...
      if (!(J > 0.0)) {
        cerr << getpid() 
             << "**ERROR** Negative Jacobian of deformation gradient" << endl;
        throw ParameterNotFound("**ERROR**:SmallStrainPlastic:Implicit", __FILE__, __LINE__);
      }
      ...
      double temperature = pTemp_old[idx];
      double incT = temperature - pTempPrev[idx];
      incThermalStrain = One*(CTE*incT);
      ...
      sigma_dev_trialtress = oldStress + One*(lambda*incStrain.Trace()) + incStrain*(2.0*mu_cur);
      devTrialStress = sigma_dev_trialtress - One*(sigma_dev_trialtress.Trace()/3.0);
      ...
      double traceOfTrialStress = sigma_dev_trialtress.Trace();
      ...
      if (Phi < 0.0) {
        ...
        // Save the updated data
        pStress_new[idx] = sigma_dev_trialtress;
        pPlasticStrain_new[idx] = pPlasticStrain[idx];
        pPlasticStrainRate_new[idx] = 0.0;
        pDamage_new[idx] = pDamage[idx];
        pPorosity_new[idx] = pPorosity[idx];
        ...
        pStress_new[idx] = sigma_dev_trialtress - nn*(2.0*state->shearModulus*delGamma);
        pPlasticStrain_new[idx] = state->plasticStrain;
        pPlasticStrainRate_new[idx] = state->plasticStrainRate;

        // Update the porosity
        if (d_evolvePorosity) {
          Matrix3 rateOfDef_new = incStrain/delT;
          double ep = state->plasticStrain;
          pPorosity_new[idx] = updatePorosity(rateOfDef_new, delT, porosity, ep);
        } else {
          pPorosity_new[idx] = pPorosity[idx];
        }
                                    ...
                                    const bool )
{
  ...
  for(int p=0;p<patches->size();p++){
    ...
    parent_old_dw->get(pTemp_old, lb->pTemperatureLabel,        pset);
    ...
    for( ; iter != pset->end(); iter++){
      ...
      computeStiffnessMatrix(B, Bnl, D, pStress_old[idx], volold,
                             pVol_new[idx], Kmatrix);
...
*/
void 
SmallStrainPlastic::computeElasticTangentModulus(const double& K,
                                             const double& mu,
                                             double Ce[6][6])
{
...
void 
SmallStrainPlastic::computeElasticTangentModulus(double bulk,
                                             double shear,
                                             TangentModulusTensor& Ce)
{
  // Form the elastic tangent modulus tensor
...
double 
SmallStrainPlastic::updatePorosity(const Matrix3& D,
                               double delT, 
                               double f,
                               double ep)
{
...
/*
double 
SmallStrainPlastic::computeSpecificHeat(double T)
{
