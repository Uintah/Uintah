...
static void fort_radcoef(IntVector& idxlo,
IntVector& idxhi,
...
bool& lradcal)
{
  ...
  IntVector co2_low = co2.getWindow()->getOffset();
  IntVector co2_high = co2.getWindow()->getData()->size() + co2_low - IntVector(1, 1, 1);
  ...
  IntVector h2o_low = h2o.getWindow()->getOffset();
  IntVector h2o_high = h2o.getWindow()->getData()->size() + h2o_low - IntVector(1, 1, 1);
  ...
  IntVector sfv_low = sfv.getWindow()->getOffset();
  IntVector sfv_high = sfv.getWindow()->getData()->size() + sfv_low - IntVector(1, 1, 1);
...
C Assuming this is fortran code
     ...
     & abskg_high_y, abskg_high_z, abskg, esrcg_low_x, esrcg_low_y, 
     & esrcg_low_z, esrcg_high_x, esrcg_high_y, esrcg_high_z, esrcg, 
     & shgamma_low_x, shgamma_low_y, shgamma_low_z, shgamma_high_x, 
     & shgamma_high_y, shgamma_high_z, shgamma, xx_low, xx_high, xx, 
     & yy_low, yy_high, yy, zz_low, zz_high, zz, fraction_low, 
     & fraction_high, fraction, fractiontwo_low, fractiontwo_high, 
     & fractiontwo, lprobone, lprobtwo, lprobthree, lambda, lradcal)
