...
void RadWsgg::WsggkVolwEmiss(const double *CO2,
			     const double *H2O,
			     const int &bands,
			     const double *T_Vol,
			     const double *SFV,
  ...
  // for Pwater/Pco2 = 2
	...
	for ( int i = 0; i < VolElementNo; i ++ ){
	  ...
	  // Use the data and model in thesis (1.17) for Const = 402.834 -- Paula
	  
	  kl_Vol[i] = a * (CO2[i] + H2O[i]) + 402.834 * SFV[i] * T_Vol[i];
	  
	  wEmiss_Vol[i] = ( b[0]*1e-1) +
	    (b[1]*1e-4) * T_Vol[i] +
	    (b[2]*1e-7) * T_Vol[i] * T_Vol[i] +
	    (b[3]*1e-11) * T_Vol[i] * T_Vol[i] * T_Vol[i];

	  if (bands == 4) {
	    wEmiss_Vol[i] = 1.0 - wEmiss_Vol[i];
	  }
...
void RadWsgg::WsggwEmissSurface(const int &surfaceElementNo,
  ...
  // for Pwater/Pco2 = 2
	...
	for ( int i = 0; i < surfaceElementNo; i ++ ){
	  ...
	  wEmiss_surface[i] = ( b[0]*1e-1) +
	    (b[1]*1e-4) * T_surface[i] +
	    (b[2]*1e-7) * T_surface[i] * T_surface[i] +
	    (b[3]*1e-11) * T_surface[i] * T_surface[i] * T_surface[i];
	  
	  // band 4 is the band 0 in Smith's paper
	  
	  if (bands == 4)
	    wEmiss_surface[i] = 1 - wEmiss_surface[i];
	}
...
// i compared the wEmiss_surface calculated this way and the way shown previous,
