...
SimulationController::printSimulationStats ( int timestep, double delt, double time )
{
  ...
  if (0 && highwater == max_highwater && timestep > 0 && d_myworld->myrank() == 5) {
    int nbins = GetNbins(DefaultAllocator());
    dbg << d_myworld->myrank() << " timestep " << timestep << " UA: " << nalloc - nfree << "(" << sizealloc - sizefree << ")"
        << " nfillbin: " << nfillbin << " HWA: " << highwater_alloc << " HWM: " << highwater_mmap
        << " OH: " << bytes_overhead << " free: " << bytes_free << " frag: " << bytes_fragmented
        << " use: " << bytes_inuse << " hunks: " << bytes_inhunks << endl;
    for (int i = 0; i < nbins; i++) {
      size_t min, max, alloc, free, inlist;
      GetBinStats(DefaultAllocator(), i, min, max, alloc, free, inlist);
      dbg << d_myworld->myrank() << "     Bin " << i << " Min: " << min << " Max: " << max 
          << " alloc " << alloc << " free " << free << " inlist " << inlist << endl;
    }
  }
}
