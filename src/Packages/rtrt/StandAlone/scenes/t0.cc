...
static Object* make_obj(int size)
{
  ...
  TimeObj* to = new TimeObj(1);
  for(int i=0;i<size;i++){
    Group* sphereflake = new Group();
    Vector objset[9];
    create_dirs(objset);
    create_objs(sphereflake, Point(0,0,.5), BV_RADIUS/2.0, Vector(0,0,1),
		i, objset, matl0);
    cerr << i << ": " << sphereflake->numObjects() << '\n';
    if(sphereflake->numObjects() == 1)
      to->add(sphereflake);
    else
      to->add(new BV1(sphereflake));
  }
