...
RTRT_ULIBS = -lPackages_rtrt_Core -lPackages_Uintah_Core_DataArchive -lPackages_Uintah_Core_Grid -lPackages_Uintah_Core_Variables -lCore_Persistent -lCore_Geometry -lCore_Containers -lCore_Exceptions -lDataflow_Comm -lDataflow_XMLUtil $(SCI_THIRDPARTY_LIBRARY) $(XML_LIBRARY) $(MPI_LIBRARY) -lCore_Malloc -lCore_Thread $(LAPACKMP_LIBRARY)

$(SRCDIR)/uintahparticle2.$(RSE): $(SRCDIR)/uintahparticle2.o
	$(CXX) -o $@ $(LDFLAGS) $(SOFLAGS) $(patsubst %.$(RSE),%.o,$(filter %.$(RSE),$@)) $(RTRT_ULIBS) $(GLUI_LIBRARY) $(GLUT_LIBRARY)
