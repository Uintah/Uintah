...
Scene* make_scene(int argc, char* argv[], int /*nworkers*/)
{
  ...
  if (!readObjFile(objname, mtlname, room_trans, mtls, group)) return 0;
...
//   group->add( hg );
  ...
  if( loadall ) { // Non-Min Living Room
...
// 	  mtls[cnt]->my_lights.add(overhead);
    ...
    mtlname = pathname + name + ".mtl";
    cerr << "Reading: " << pathname + name << "\n";
    if (!readObjFile(objname, mtlname, room_trans, mtls, group)) exit(0);
...
//     group->add( hg );
  ...
  Scene *scene = new Scene(new HierarchicalGrid(group, 8, 8, 8, minObjs1, minObjs2, 4), cam, bgcolor, cdown, cup, groundplane, 0.3, Arc_Ambient);

  scene->select_shadow_mode( Hard_Shadows );
  scene->maxdepth = 8;

  scene->add_light( overhead );
