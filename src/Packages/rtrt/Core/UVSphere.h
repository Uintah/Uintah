...
class UVSphere : public UVMapping {
   Point cen; 
   double radius;
public:
    UVSphere(Point c, double r);
    virtual ~UVSphere();
    virtual void uv(UV& uv, const Point&, const HitInfo& hit);
    virtual void get_frame(const Point &hitpos, const HitInfo&hit,const Vector &norm,  Vector &pu, Vector &pv)
  {
     UV uv_m;
     float u,v;
     double phi,theta;
