...
void PathTraceWorker::run() {
    ...
    for(int y = 0; y < height; y++)
      for(int x = 0; x < width; x++)
      {
        ...
        for(int sample = 0; sample < ptc->num_samples; sample++) {
          ...
          Point origin=sphere->cen + sphere->radius*Vector(x,y,z);
          Vector normal(x, y, z);
	    ...
	    //            if (!shadow_hit.was_hit)
              direct = ptc->color*light->color*light->area*SCIRun::Dot(normal,sr_dir)*(1./(distance*distance*M_PI));
	      //	    else
	      //	      direct = Color(0,0,0);

	    result += direct;
	    cerr << "direct color = "<<direct<<", result = "<<result<<"\n";

            if (depth==ptc->max_depth-1)
              break;

            // Determine next sphere
            ...
            Vector ray_dir=v_out.x()*v1+v_out.y()*v2+v_out.z()*normal;
  ...
  for(int sindex = 0; sindex < local_spheres->objs.size(); sindex++) {
    ...
    sprintf(buf, "%s%05d.nrrd", basename, sindex);
    ...
    for(int y = 0; y < height; y++)
      for(int x = 0; x < width; x++) {
	float data[3];
	data[0] = sphere->texture(x,y).red();
	data[1] = sphere->texture(x,y).green();
	data[2] = sphere->texture(x,y).blue();
	if (fwrite (data, sizeof(float), 3, out) != 3) {
	  cerr << "Trouble writing texel for sphere "<<sindex<<" at ["<<x<<", "<<y<<"]\n";
	  return;
	  }
      }
    fclose(out);
