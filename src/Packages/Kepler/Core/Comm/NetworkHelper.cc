...
{
  std::cerr << msg << " has occured." << std::endl;
}
...
void Inet_pton(int family, const char *strptr, void *addrptr)
{
  ...
  if ( (n = inet_pton(family, strptr, addrptr)) < 0)
    print_error("inet_pton error for " + std::string(strptr)); /* errno set */
  else if (n == 0){
    print_error("inet_pton error for " + std::string(strptr) + ", quit");  /* errno not set, was err_quit*/
    exit(1);
  }
}
...
{
  if (written(fd, ptr, nbytes) != (ssize_t)nbytes)
