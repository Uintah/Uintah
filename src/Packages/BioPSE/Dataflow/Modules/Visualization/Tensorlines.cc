/*
 *  Tensorlines.cc:  Builds the RHS of the FE matrix for current sources
 *
 *  Written by:
 *   David Weinstein
 *   University of Utah
...
class BioPSESHARE Tensorlines : public Module {
  FieldIPort *ifield_;
  FieldOPort *ofield_;
...
public:
  Tensorlines(GuiContext *context);
  virtual ~Tensorlines();
...
DECLARE_MAKER(Tensorlines)


Tensorlines::Tensorlines(GuiContext *context) :
  Module("Tensorlines", context, Filter, "Visualization", "BioPSE"),
  ...
  widget_lock_("Tensorlines widget lock")
{
  lastGen_=-1;
  lastWidgetSize_=-1;
  lastScaleMode_="";
...
Tensorlines::execute()
{
  ...
  if (!ifield_->get(fieldH) || 
    ...
    error("No vald input in Tensorlines Field port.");
    return;
  }
...
void
Tensorlines::widget_moved(bool last)
{
  if(last && !abort_flag)
  {
    abort_flag=1;
