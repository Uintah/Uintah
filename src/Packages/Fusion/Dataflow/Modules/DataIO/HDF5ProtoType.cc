...
#include <Packages/Fusion/Dataflow/Modules/DataIO/HDF5ProtoType.h>
...
DECLARE_MAKER(HDF5ProtoType)

HDF5ProtoType::HDF5ProtoType(GuiContext *context)
  : Module("HDF5ProtoType", context, Source, "DataIO", "Fusion"),
...
HDF5ProtoType::~HDF5ProtoType() {
}

void HDF5ProtoType::execute() {
  ...
  if( updateFile || updateAll )
  {
    ...
    CompileInfoHandle ci_mesh =
      HDF5ProtoTypeMeshAlgo::get_compile_info(mtd);

    Handle<HDF5ProtoTypeMeshAlgo> algo_mesh;
    ...
    CompileInfoHandle ci =
      HDF5ProtoTypeFieldAlgo::get_compile_info(ftd, rank_);

    Handle<HDF5ProtoTypeFieldAlgo> algo;
...
float*  HDF5ProtoType::readGrid( string filename ) {
#ifdef HAVE_HDF5
  herr_t  status;
...
float* HDF5ProtoType::readData( string filename ) {
#ifdef HAVE_HDF5
  herr_t  status;
...
void HDF5ProtoType::tcl_command(GuiArgs& args, void* userdata)
{
  if(args.count() < 2){
    args.error("HDF5ProtoType needs a minor command");
    return;
  }
  ...
  if (args[1] == "error") {
    ...
    int last = new_filename.find_last_of( "/" );
...
CompileInfoHandle
HDF5ProtoTypeMeshAlgo::get_compile_info(const TypeDescription *ftd)
{
  // use cc_to_h if this is in the .cc file, otherwise just __FILE__
  static const string include_path(TypeDescription::cc_to_h(__FILE__));
  static const string base_class_name("HDF5ProtoTypeMeshAlgo");
  static const string template_class_name("HDF5ProtoTypeMeshAlgoT");
...
CompileInfoHandle
HDF5ProtoTypeFieldAlgo::get_compile_info(const TypeDescription *ftd,
					       int rank)
{
  // use cc_to_h if this is in the .cc file, otherwise just __FILE__
  static const string include_path(TypeDescription::cc_to_h(__FILE__));
  static const string base_class_name("HDF5ProtoTypeFieldAlgo");
