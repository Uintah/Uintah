#!/bin/bash
#
# This script will download and build all packages for LeXoV
#
# For more info, see http://lexov.org
# 
...
try export DIR=`pwd`/lexov-build
#if [ -d $DIR ]; then
...
# Build LeXoV cmake in lexov-bin
try mkdir -p $DIR/lexov-bin
try cd $DIR/lexov-bin
try $cmakebin $DIR/SCIRunCore-src/src/StandAlone/Apps/Painter/ -DSCIRUN_DIR=$DIR/SCIRunCore-bin -DCMAKE_BUILD_TYPE=$buildtype
try make $makeflags
