...
{ 
  ...
  for (int p=0; p < patches->size(); p++){
    ...
    for (CellIterator iter = patch->getCellIterator(); !iter.done(); iter++){ 
      ...
      for (int iRay=0; iRay < _NoOfRays; iRay++){
        ...
        while (intensity > _Threshold){
          ...
          while (in_domain){
            ...
            chiSumI += chi * sigmaT4Pi[prevCell] * ( exp(-optical_thickness_prev) - exp(-optical_thickness) ) * fs;;
          ...
          if (intensity > _Threshold){
            ...
            chiSumI += chi * local_sigmaT4Pi[face] * intensity;

            // chiSumI += chi * local_sigmaT4Pi[face +sign[face]] * intensity;
      ...
      //__________________________________
      //  Compute divQ
      divQ[origin] = 4.0 * _pi * ( sigmaT4Pi[origin] * abskg[origin] - (chiSumI/_NoOfRays) );
