#include <CCA/Components/Wasatch/Expressions/VardenMassSrc.h>
...
VardenMassSrc::VardenMassSrc( const Expr::TagList& momTags,
...
VardenMassSrc::~VardenMassSrc()
{}

//------------------------------------------------------------------

void VardenMassSrc::advertise_dependents( Expr::ExprDeps& exprDeps )
{  
  if (!isConstDensity_) {
    if( doX_ )  
    {
...
void VardenMassSrc::bind_fields( const Expr::FieldManagerList& fml )
{
...
void VardenMassSrc::bind_operators( const SpatialOps::OperatorDatabase& opDB )
{
  if (!isConstDensity_) {
    if( doX_ ){
...
void VardenMassSrc::evaluate()
{
  using namespace SpatialOps;
  typedef std::vector<SVolField*> SVolFieldVec;
  SVolFieldVec& results = this->get_value_vec();
  
  SVolField& drhodt       = *results[0];
  SVolField& drhodtstar   = *results[1];
  SVolField& divmom       = *results[2];
  SVolField& divmomstar   = *results[3];
  SVolField& alpha        = *results[4];
  SVolField& beta         = *results[5];
  
  drhodtstar <<= (*dens2Star_ - *dens_)/(2.0 * *timestep_);
  ...
  beta  <<= abs(drhodtstar/divmomstar);
  
//    alpha <<= cond( beta != beta, 0.0 )
//                  ( 1.0/(beta + 1.0)  );
  
    alpha <<= cond( beta != beta, 0.0 )
                  ( exp(log(0.5)*(pow(beta,0.5))) ); // Increase the value of the exponent to get closer to a step function
...
//  alpha <<= 0.1; // use this for the moment until we figure out the proper model for alpha
  ...
  if( is3d_ ){ // for 3D cases, inline the whole thing
    ...
    divmom <<= (*gradXOp_)(*xMom_) + (*gradYOp_)(*yMom_) + (*gradZOp_)(*zMom_) ;
    
  } else {
    
    // for 1D and 2D cases, we are not as efficient - add terms as needed...
    if( doX_ ) divmom <<= (*gradXOp_)( (*xMom_) );
    else       divmom <<= 0.0;
    if( doY_ ) divmom <<= divmom + (*gradYOp_)( (*yMom_) );
    if( doZ_ ) divmom <<= divmom + (*gradZOp_)( (*zMom_) );
  } // 1D, 2D cases
}

//------------------------------------------------------------------

VardenMassSrc::Builder::Builder( const Expr::TagList& results,
...
Expr::ExpressionBase*
VardenMassSrc::Builder::build() const
{
  return new VardenMassSrc( momTs_, velStarTs_, isConstDens_, denst_, densStart_, dens2Start_, dilt_, tstpt_);
}
//------------------------------------------------------------------
