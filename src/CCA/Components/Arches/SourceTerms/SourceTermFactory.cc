  ...
  if (srcs_db) {
    for (ProblemSpecP source_db = srcs_db->findBlock("src"); source_db != 0; source_db = source_db->findNextBlock("src")){
      ...
      if ( src_type == "constant_src" ) {
        ...
        SourceTermBase::Builder* src_builder = new CoalGasDevolMom::Builder(src_name, required_varLabels, lab, shared_state);
        factory.register_source_term( src_name, src_builder );
        ...
        // Sums up the devol. model terms * weights
        SourceTermBase::Builder* src_builder = new CoalGasOxiMom::Builder(src_name, required_varLabels, lab, shared_state);
        factory.register_source_term( src_name, src_builder );
