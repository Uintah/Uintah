#ifndef Uintah_Component_Arches_DSmaMMMLv2_h
#define Uintah_Component_Arches_DSmaMMMLv2_h
...
namespace Uintah{
  ...
  class DSmaMMMLv2 : public TaskInterface {

public:

    DSmaMMMLv2( std::string task_name, int matl_index );
    ~DSmaMMMLv2();
    ...
    //Build instructions for this (DSmaMMMLv2) class.
    class Builder : public TaskInterface::TaskBuilder {
      ...
      DSmaMMMLv2* build()
      { return scinew DSmaMMMLv2<TT>( m_task_name, m_matl_index ); }
...
//--------------------------------------------------------------------------------------------------
template<typename TT>
DSmaMMMLv2<TT>::DSmaMMMLv2( std::string task_name, int matl_index ) :
TaskInterface( task_name, matl_index ) {
}

//--------------------------------------------------------------------------------------------------
template<typename TT>
DSmaMMMLv2<TT>::~DSmaMMMLv2(){}

//--------------------------------------------------------------------------------------------------
template<typename TT>
TaskAssignedExecutionSpace DSmaMMMLv2<TT>::loadTaskComputeBCsFunctionPointers()
{
  return create_portable_arches_tasks<TaskInterface::BC>( this
                                     , &DSmaMMMLv2<TT>::compute_bcs<UINTAH_CPU_TAG>     // Task supports non-Kokkos builds
                                     //, &DSmaMMMLv2<TT>::compute_bcs<KOKKOS_OPENMP_TAG>  // Task supports Kokkos::OpenMP builds
                                     //, &DSmaMMMLv2<TT>::compute_bcs<KOKKOS_CUDA_TAG>    // Task supports Kokkos::Cuda builds
                                     );
}

//--------------------------------------------------------------------------------------------------
template<typename TT>
TaskAssignedExecutionSpace DSmaMMMLv2<TT>::loadTaskInitializeFunctionPointers()
{
  return create_portable_arches_tasks<TaskInterface::INITIALIZE>( this
                                     , &DSmaMMMLv2<TT>::initialize<UINTAH_CPU_TAG>     // Task supports non-Kokkos builds
                                     //, &DSmaMMMLv2<TT>::initialize<KOKKOS_OPENMP_TAG>  // Task supports Kokkos::OpenMP builds
                                     //, &DSmaMMMLv2<TT>::initialize<KOKKOS_CUDA_TAG>    // Task supports Kokkos::Cuda builds
                                     );
}
...
TaskAssignedExecutionSpace DSmaMMMLv2<TT>::loadTaskEvalFunctionPointers()
{
  return create_portable_arches_tasks<TaskInterface::TIMESTEP_EVAL>( this
                                     , &DSmaMMMLv2<TT>::eval<UINTAH_CPU_TAG>     // Task supports non-Kokkos builds
                                     , &DSmaMMMLv2<TT>::eval<KOKKOS_OPENMP_TAG>  // Task supports Kokkos::OpenMP builds
                                     //, &DSmaMMMLv2<TT>::eval<KOKKOS_CUDA_TAG>    // Task supports Kokkos::Cuda builds
                                     );
}
...
TaskAssignedExecutionSpace DSmaMMMLv2<TT>::loadTaskTimestepInitFunctionPointers()
{
  return create_portable_arches_tasks<TaskInterface::TIMESTEP_INITIALIZE>( this
                                     , &DSmaMMMLv2<TT>::timestep_init<UINTAH_CPU_TAG>     // Task supports non-Kokkos builds
                                     , &DSmaMMMLv2<TT>::timestep_init<KOKKOS_OPENMP_TAG>  // Task supports Kokkos::OpenMP builds
                                     );
}
...
DSmaMMMLv2<TT>::problemSetup( ProblemSpecP& db ){
...
//--------------------------------------------------------------------------------------------------
template<typename TT> void
DSmaMMMLv2<TT>::register_initialize( std::vector<ArchesFieldContainer::VariableInformation>&
...
template<typename TT>
template<typename ExecutionSpace, typename MemSpace>
void DSmaMMMLv2<TT>::initialize( const Patch* patch, ArchesTaskInfoManager* tsk_info, ExecutionObject<ExecutionSpace, MemSpace>& executionObject ){

}
//--------------------------------------------------------------------------------------------------
template<typename TT> void
DSmaMMMLv2<TT>::register_timestep_init( std::vector<ArchesFieldContainer::VariableInformation>&
                                          variable_registry , const bool packed_tasks){
...
DSmaMMMLv2<TT>::timestep_init( const Patch* patch, ArchesTaskInfoManager* tsk_info, ExecutionObject<ExecutionSpace, MemSpace>& executionObject ){

}

//--------------------------------------------------------------------------------------------------
template<typename TT> void
DSmaMMMLv2<TT>::register_timestep_eval( std::vector<ArchesFieldContainer::VariableInformation>&
  ...
  register_variable( "Filterrho", ArchesFieldContainer::REQUIRES,0 , ArchesFieldContainer::NEWDW, variable_registry, time_substep, m_task_name, packed_tasks );
...
template<typename TT>
template<typename ExecutionSpace, typename MemSpace>
void DSmaMMMLv2<TT>::eval( const Patch* patch, ArchesTaskInfoManager* tsk_info, ExecutionObject<ExecutionSpace, MemSpace>& executionObject ){
...
//  Uintah::parallel_for(range1,get_fBeta23);
 ...
 // Uintah::parallel_for(range1,get_fs11);
  ...
  m_Filter.applyFilter<TT>((*IsI),filterIsI,vol_fraction,range1);
  m_Filter.applyFilter<TT>((*s11),filters11,vol_fraction,range1);
  m_Filter.applyFilter<TT>((*s22),filters22,vol_fraction,range1);
  m_Filter.applyFilter<TT>((*s33),filters33,vol_fraction,range1);
  m_Filter.applyFilter<TT>((*s12),filters12,vol_fraction,range1);
  m_Filter.applyFilter<TT>((*s13),filters13,vol_fraction,range1);
  m_Filter.applyFilter<TT>((*s23),filters23,vol_fraction,range1);
