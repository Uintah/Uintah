...
//--------------------------------------------------------------------------------------------------
template<typename TT>
TaskAssignedExecutionSpace DSmaCsv2<TT>::loadTaskComputeBCsFunctionPointers()
{
  return create_portable_arches_tasks<TaskInterface::BC>( this
                                     , &DSmaCsv2<TT>::compute_bcs<UINTAH_CPU_TAG>     // Task supports non-Kokkos builds
                                     //, &DSmaCsv2<TT>::compute_bcs<KOKKOS_OPENMP_TAG>  // Task supports Kokkos::OpenMP builds
                                     //, &DSmaCsv2<TT>::compute_bcs<KOKKOS_CUDA_TAG>    // Task supports Kokkos::Cuda builds
                                     );
}

//--------------------------------------------------------------------------------------------------
template<typename TT>
TaskAssignedExecutionSpace DSmaCsv2<TT>::loadTaskInitializeFunctionPointers()
{
  return create_portable_arches_tasks<TaskInterface::INITIALIZE>( this
                                     , &DSmaCsv2<TT>::initialize<UINTAH_CPU_TAG>     // Task supports non-Kokkos builds
                                     //, &DSmaCsv2<TT>::initialize<KOKKOS_OPENMP_TAG>  // Task supports Kokkos::OpenMP builds
                                     //, &DSmaCsv2<TT>::initialize<KOKKOS_CUDA_TAG>    // Task supports Kokkos::Cuda builds
                                     );
}
...
//--------------------------------------------------------------------------------------------------
template<typename TT>
TaskAssignedExecutionSpace DSmaCsv2<TT>::loadTaskTimestepInitFunctionPointers()
{
  return create_portable_arches_tasks<TaskInterface::TIMESTEP_INITIALIZE>( this
                                     , &DSmaCsv2<TT>::timestep_init<UINTAH_CPU_TAG>     // Task supports non-Kokkos builds
                                     , &DSmaCsv2<TT>::timestep_init<KOKKOS_OPENMP_TAG>  // Task supports Kokkos::OpenMP builds
                                     //, &DSmaCsv2<TT>::timestep_init<KOKKOS_CUDA_TAG>    // Task supports Kokkos::Cuda builds
                                     );
}

//--------------------------------------------------------------------------------------------------
template<typename TT>
TaskAssignedExecutionSpace DSmaCsv2<TT>::loadTaskRestartInitFunctionPointers()
{
...
template<typename TT>
template <typename ExecSpace, typename MemSpace>
void DSmaCsv2<TT>::initialize( const Patch* patch, ArchesTaskInfoManager* tsk_info, ExecutionObject<ExecSpace, MemSpace>& execObj ){
  if (m_create_labels_IsI_t_viscosity) {
...
DSmaCsv2<TT>::timestep_init( const Patch* patch, ArchesTaskInfoManager* tsk_info, ExecutionObject<ExecSpace, MemSpace>& execObj ){
  //CCVariable<double>& mu_sgc = *(tsk_info->get_uintah_field<CCVariable<double> >(m_t_vis_name));
  //CCVariable<double>& Cs = *(tsk_info->get_uintah_field<CCVariable<double> >(m_Cs_name));
}
...
template<typename TT>
template <typename ExecSpace, typename MemSpace>
void DSmaCsv2<TT>::eval( const Patch* patch, ArchesTaskInfoManager* tsk_info, ExecutionObject<ExecSpace, MemSpace>& execObj ){
