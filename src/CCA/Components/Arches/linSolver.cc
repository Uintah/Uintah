...
//}
  ...
  tsk->needsLabel(Task::NewDW, ALabel, Ghost::AroundCells, 1);   // ghosts needed fro jjacobiblock, not jacobi
  ...
  for (unsigned int i=0;i<d_precMLabel.size();i++){
    ...
    tskc->needsLabel( Task::NewDW ,d_precMLabel[i], 0, Task::FineLevel, 0, Task::NormalDomain, Ghost::None, 0 );
 ...
 //This allows us to populate ghosts into the "static" a_matrix, and is otherwise unnecessary, this could have been donw ith more complex upstream logic, but this is much more simple.
  ...
  for (unsigned int i=0;i<d_precMLabel.size();i++){
    tsk_hack->needsLabel( Task::NewDW ,d_precMLabel[i], Ghost::AroundCells, cg_ghost+1 ); // +1 not needed for i=0;
                                        ...
                                        const MaterialSet* matls)
{
  ...
  tsk->needsLabel(Task::NewDW, d_cellTypeLabel,       gac, 1);
...
//sched_PreconditionerConstruction( sched,  matls, patches,nullptr);  
  ...
  if(rk_step==0){
    ...
    for (int l = 0; l < maxLevels; ++l) {
      ...
      for (unsigned int i=0;i<d_precMLabel.size();i++){
        task_u->needsLabel(Task::OldDW,d_precMLabel[i],Ghost::AroundCells,cg_ghost+1);
...
////------------------ set up CG solver---------------------//
  ...
  task_i1->needsLabel(Task::NewDW, guess, Ghost::AroundCells, 1);
  task_i1->needsLabel(Task::NewDW, bLabel, Ghost::None, 0);
  task_i1->needsLabel(Task::NewDW, ALabel, Ghost::AroundCells, 1);
...
if (rb_iter == final_iter){
  ...
  task_i2->needsLabel(Task::NewDW, d_residualLabel, Ghost::AroundCells, cg_ghost); 

    for (unsigned int i=0;i<d_precMLabel.size();i++){
      task_i2->needsLabel(Task::NewDW,d_precMLabel[i],Ghost::AroundCells,cg_ghost+1); //  possibly not needed...
  }
  ...
  task1->needsLabel(Task::NewDW,ALabel,Ghost::None, 0 );
  task1->needsLabel(Task::NewDW,d_smallPLabel, Ghost::AroundCells, 1);
  ...
  task2->needsLabel(Task::NewDW,d_corrSumLabel[cg_iter]);
  task2->needsLabel(Task::NewDW,d_resSumLabel[cg_iter]);
  task2->needsLabel(Task::NewDW,d_littleQLabel, Ghost::None, 0);
  task2->needsLabel(Task::NewDW,d_smallPLabel, Ghost::None, 0);
  ...
  for (int l = maxLevels-2; l > -1; l--) { // Coarsen fine to coarse
    ...
    task_multigrid_up->needsLabel( Task::NewDW ,d_residualLabel, 0, Task::FineLevel, 0, Task::NormalDomain, Ghost::None, 0 );
    if ( cg_iter==0){
  ...
  for (int l = 0; l < maxLevels; ++l) {
    ...
    if (l>0){
        //int offset = 1;
        task_multigrid_down->needsLabel( Task::NewDW ,d_bigZLabel, 0, Task::CoarseLevel, 0, Task::NormalDomain, Ghost::None, 0 );
    }
    ...
    for (int red_black_switch=smoother_iter-1; red_black_switch > -1; red_black_switch--){
      ...
      task_multigrid_smooth->needsLabel(Task::NewDW,d_residualLabel, Ghost::AroundCells, cg_ghost);
      task_multigrid_smooth->needsLabel(Task::NewDW,d_bigZLabel, Ghost::AroundCells,cg_ghost);
      task_multigrid_smooth->modifies(d_bigZLabel);

      for (unsigned int i=0;i<d_precMLabel.size();i++){
        task_multigrid_smooth->needsLabel(Task::NewDW,d_precMLabel[i], Ghost::AroundCells,cg_ghost+1); // NOT REALLY 0 ghost cells, we are taking shortcuts upstream
      }
  ...
  task4->needsLabel(Task::NewDW,d_convMaxLabel[cg_iter]);
  task4->needsLabel(Task::NewDW,d_resSumLabel[cg_iter]);
  task4->needsLabel(Task::NewDW,d_resSumLabel[cg_iter+1]);
  task4->modifies(d_smallPLabel);
  task4->needsLabel(Task::NewDW,d_bigZLabel,Ghost::None, 0);
