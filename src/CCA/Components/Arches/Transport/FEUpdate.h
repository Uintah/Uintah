...
private:
  ...
  template <typename T> 
  void FEUpdate<T>::register_all_variables( std::vector<VariableInformation>& variable_registry, const int time_substep ){
  ...
  void FEUpdate<T>::eval( const Patch* patch, FieldCollector* field_collector, 
    ...
    const SVol* const rho = field_collector->get_so_field<SVol>( "density" ); 
    typedef std::vector<std::string> SV;
    for ( SV::iterator i = _eqn_names.begin(); i != _eqn_names.end(); i++){ 

      T* const phi = field_collector->get_so_field<T>( *i );
      T* const rhs = field_collector->get_so_field<T>( *i+"_RHS" ); 
