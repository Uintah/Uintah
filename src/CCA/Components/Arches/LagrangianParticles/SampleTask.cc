...
SampleTask::register_initialize( std::vector<ArchesFieldContainer::VariableInformation>& variable_registry ){
  ...
  register_variable( "a_sample_variable", ArchesFieldContainer::REQUIRES, 0, ArchesFieldContainer::NEWDW,  variable_registry );
  register_variable( "a_result_variable", ArchesFieldContainer::COMPUTES, 0, ArchesFieldContainer::NEWDW,  variable_registry );
...
SampleTask::initialize( const Patch* patch, ArchesTaskInfoManager* tsk_info,
  ...
  SVolFP field  = tsk_info->get_so_field_new<SVolF>( "a_sample_variable" );
  SVolFP result = tsk_info->get_so_field_new<SVolF>( "a_result_variable" );
...
SampleTask::register_timestep_eval( std::vector<ArchesFieldContainer::VariableInformation>& variable_registry, const int time_substep ){
  ...
  register_variable( "a_result_variable", ArchesFieldContainer::COMPUTES,       0, ArchesFieldContainer::NEWDW,  variable_registry, time_substep );
