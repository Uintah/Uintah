...
namespace Uintah{
  ...
  public:
    ...
    public:
      ...
      { return scinew ShaddixEnthalpy<T>( m_task_name, m_matl_index, _base_var_name, _Nenv ); }
  ...
  TaskAssignedExecutionSpace ShaddixEnthalpy<T>::loadTaskInitializeFunctionPointers()
  {
    return create_portable_arches_tasks<TaskInterface::INITIALIZE>( this
                                       ...
                                       //, &ShaddixEnthalpy<T>::initialize<KOKKOS_DEFAULT_HOST_TAG>    // Task supports Kokkos::DefaultHostExecutionSpace builds
                                       //, &ShaddixEnthalpy<T>::initialize<KOKKOS_DEFAULT_DEVICE_TAG>  // Task supports Kokkos::DefaultExecutionSpace builds
                                       //, &ShaddixEnthalpy<T>::initialize<KOKKOS_DEFAULT_DEVICE_TAG>            // Task supports Kokkos builds
                                       );
  }
  ...
  TaskAssignedExecutionSpace ShaddixEnthalpy<T>::loadTaskEvalFunctionPointers()
  {
    return create_portable_arches_tasks<TaskInterface::TIMESTEP_EVAL>( this
                                       ...
                                       //, &ShaddixEnthalpy<T>::eval<KOKKOS_DEFAULT_HOST_TAG>    // Task supports Kokkos::DefaultHostExecutionSpace builds
                                       //, &ShaddixEnthalpy<T>::eval<KOKKOS_DEFAULT_DEVICE_TAG>  // Task supports Kokkos::DefaultExecutionSpace builds
                                       //, &ShaddixEnthalpy<T>::eval<KOKKOS_DEFAULT_DEVICE_TAG>            // Task supports Kokkos builds
                                       );
  }
...
//--------------------------------------------------------------------------------------------------
  template <typename T>
  template <typename ExecSpace, typename MemSpace>
  void ShaddixEnthalpy<T>::initialize( const Patch* patch, ArchesTaskInfoManager* tsk_info, ExecutionObject<ExecSpace, MemSpace>& execObj ){
...
//--------------------------------------------------------------------------------------------------
  ...
  void ShaddixEnthalpy<T>::register_timestep_eval( std::vector<ArchesFieldContainer::VariableInformation>& variable_registry, const int time_substep , const bool packed_tasks){
    ...
    for ( int i = 0; i < _Nenv; i++ ){
      ...
      register_variable( char_oxi_temp_name, ArchesFieldContainer::NEEDSLABEL, 0, ArchesFieldContainer::NEWDW, variable_registry, time_substep );
      register_variable( surf_rate_name, ArchesFieldContainer::NEEDSLABEL, 0, ArchesFieldContainer::NEWDW, variable_registry, time_substep );
      register_variable( char_gas_name, ArchesFieldContainer::NEEDSLABEL, 0, ArchesFieldContainer::NEWDW, variable_registry, time_substep );
      register_variable( devol_gas_name, ArchesFieldContainer::NEEDSLABEL, 0, ArchesFieldContainer::NEWDW, variable_registry, time_substep );
      ...
      if ( d_radiation ) {
        ...
        register_variable( abskp_name, ArchesFieldContainer::NEEDSLABEL, 0, ArchesFieldContainer::LATEST, variable_registry, time_substep );
      }
    }
    ...
    register_variable( _gas_cp_name, ArchesFieldContainer::NEEDSLABEL, 0, ArchesFieldContainer::LATEST, variable_registry, time_substep );
    ...
    if ( d_radiation ) {
      register_variable( _volq_name, ArchesFieldContainer::NEEDSLABEL, 0, ArchesFieldContainer::LATEST, variable_registry, time_substep );
    }
  }
...
//--------------------------------------------------------------------------------------------------
  template <typename T>
  template <typename ExecSpace, typename MemSpace>
  void ShaddixEnthalpy<T>::eval( const Patch* patch, ArchesTaskInfoManager* tsk_info, ExecutionObject<ExecSpace, MemSpace>& execObj ){
