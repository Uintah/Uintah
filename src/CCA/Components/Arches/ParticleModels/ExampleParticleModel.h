...
private:
  ...
  void ExampleParticleModel<IT, DT>::register_timestep_eval( std::vector<ArchesFieldContainer::VariableInformation>& variable_registry, const int time_substep , const bool packed_tasks){
    ...
    register_variable( _temperature_var_name, ArchesFieldContainer::NEEDSLABEL, 0, ArchesFieldContainer::NEWDW, variable_registry, time_substep );
    register_variable( _conc_var_name, ArchesFieldContainer::NEEDSLABEL, 0, ArchesFieldContainer::NEWDW, variable_registry, time_substep );
  ...
  void ExampleParticleModel<IT,DT>::eval( const Patch* patch, ArchesTaskInfoManager* tsk_info, ExecutionObject<ExecSpace, MemSpace>& execObj ){
    ...
    for ( int ienv = 0; ienv < _N; ienv++ ){
      ...
      CIT& temperature = tsk_info->get_field<CIT>(_temperature_var_name);
      CIT& conc = tsk_info->get_field<CIT>(_conc_var_name);
