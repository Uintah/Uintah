...
namespace Uintah{
  ...
  TaskAssignedExecutionSpace ShaddixOxidation<T>::loadTaskComputeBCsFunctionPointers()
  {
    return create_portable_arches_tasks<TaskInterface::BC>( this
                                       , &ShaddixOxidation<T>::compute_bcs<UINTAH_CPU_TAG>     // Task supports non-Kokkos builds
                                       //, &ShaddixOxidation<T>::compute_bcs<KOKKOS_OPENMP_TAG>  // Task supports Kokkos::OpenMP builds
                                       //, &ShaddixOxidation<T>::compute_bcs<KOKKOS_CUDA_TAG>    // Task supports Kokkos::Cuda builds
                                       );
  }
  ...
  TaskAssignedExecutionSpace ShaddixOxidation<T>::loadTaskTimestepInitFunctionPointers()
  {
    return create_portable_arches_tasks<TaskInterface::TIMESTEP_INITIALIZE>( this
                                       , &ShaddixOxidation<T>::timestep_init<UINTAH_CPU_TAG>     // Task supports non-Kokkos builds
                                       , &ShaddixOxidation<T>::timestep_init<KOKKOS_OPENMP_TAG>  // Task supports Kokkos::OpenMP builds
                                       );
  }
  ...
  template <typename T>
  template<typename ExecutionSpace, typename MemSpace>
  void ShaddixOxidation<T>::initialize( const Patch* patch, ArchesTaskInfoManager* tsk_info, ExecutionObject<ExecutionSpace, MemSpace>& executionObject ){
  ...
  template <typename T>
  template<typename ExecutionSpace, typename MemSpace>
  void ShaddixOxidation<T>::eval( const Patch* patch, ArchesTaskInfoManager* tsk_info, ExecutionObject<ExecutionSpace, MemSpace>& executionObject ){
