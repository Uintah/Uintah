...
//--------------------------------------------------------------------------------------------------
template<typename T> void
CharOxidationps<T>::register_initialize( std::vector<ArchesFieldContainer::VariableInformation>&
...
//--------------------------------------------------------------------------------------------------
template<typename T> void
CharOxidationps<T>::register_timestep_init( std::vector<ArchesFieldContainer::VariableInformation>&
                                          variable_registry , const bool packed_tasks){
}

//--------------------------------------------------------------------------------------------------
template<typename T> void
CharOxidationps<T>::timestep_init( const Patch* patch, ArchesTaskInfoManager* tsk_info ){
}

//--------------------------------------------------------------------------------------------------
template<typename T> void
CharOxidationps<T>::register_timestep_eval( std::vector<ArchesFieldContainer::VariableInformation>&
...
//--------------------------------------------------------------------------------------------------
template<typename T> void
CharOxidationps<T>::eval( const Patch* patch, ArchesTaskInfoManager* tsk_info ){
 // gas variables
  ...
  // reaction rate 
   ...
   Uintah::parallel_for(range,  [&]( int i,  int j, int k){
     ...
     if (weight(i,j,k)/m_weight_scaling_constant[l] < _weight_small) {
       ...
       dfdrh->zero();// [-]
       for (int r=0; r<_NUM_reactions; r++) {
         rh_l[r]     = (*old_reaction_rate[m2 + r])(i,j,k);// [kg/m^3/s]
         rh_l_new[r] = (*old_reaction_rate[m2 + r])(i,j,k);// [kg/m^3/s]
       }

       for (int r=0; r<_NUM_reactions; r++) { // check this 
         oxid_mass_frac[l]=(*species[_oxidizer_indices[r]])(i,j,k);// [mass fraction]
       }
       ...
       double p_area   = M_PI * p_diam*p_diam; // particle surface area [m^2]
       ...
       for (int r=0; r<_NUM_reactions; r++) {
         ...
         rh_l_i               = std::min(rh_l_new[r] * p_area * x_org * (1.-p_void), oxi_lim);// [kg/s/#] 
       ...
       double Size_rate = (x_org < 1e-8) ? 0.0 : w/m_weight_scaling_constant[l] * 2.*x_org * surface_rate_factor * char_mass_rate / m_rho_org_bulk[l] / p_area / x_org / (1.-p_void) /m_length_scaling_constant[l]; // [m/s] 
