...
TaskAssignedExecutionSpace VarInterpolation<T, IT>::loadTaskFunctionPointers(){
  ...
  LOAD_ARCHES_EVAL_TASK_3TAGS(UINTAH_CPU_TAG, KOKKOS_OPENMP_TAG, void, assignedTag, VarInterpolation<T COMMA IT>::eval);
  return assignedTag;
...
void VarInterpolation<T,IT>::eval( const Patch* patch, ArchesTaskInfoManager* tsk_info, void* stream ){
