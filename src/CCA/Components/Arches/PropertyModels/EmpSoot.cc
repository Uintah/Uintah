                          ...
                          int time_substep )
{
  ...
  for (int p=0; p < patches->size(); p++){
    ...
    if ( new_dw->exists( _prop_label, matlIndex, patch ) ){
      ...
      new_dw->get( temperature, _T_label, archIndex, patch, Ghost::None, 0 ); 
      new_dw->get( density, _den_label, archIndex, patch, Ghost::None, 0 ); 
      ...
      old_dw->get( temperature, _T_label, archIndex, patch, Ghost::None, 0 ); 
      old_dw->get( density, _T_label, archIndex, patch, Ghost::None, 0 ); 
    ...
    for (iter.begin(); !iter.done(); iter++){
      ...
      absorp_coef[c] = 0.01 + Min( 0.5, (4.0/_opl)*log(1.0+350.0*
                       soot_vf[c] * temperature[c] * _opl));
