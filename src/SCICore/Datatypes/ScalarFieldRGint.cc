...
/*
 *  ScalarFieldRGint.cc: int Scalar Field defined on a Regular grid
 *
 *  Written by:
...
{
    return scinew ScalarFieldRGint;
}

PersistentTypeID ScalarFieldRGint::type_id("ScalarFieldRGint", "ScalarField", maker);

ScalarFieldRGint::ScalarFieldRGint()
: ScalarFieldRGBase("int")
{
}

ScalarFieldRGint::ScalarFieldRGint(const ScalarFieldRGint& copy)
...
void ScalarFieldRGint::resize(int x, int y, int z) {
    nx=x; ny=y; nz=z;
    grid.newsize(x,y,z);
}

#define ScalarFieldRGint_VERSION 3

void ScalarFieldRGint::io(Piostream& stream)
{
    using SCICore::PersistentSpace::Pio;
    using SCICore::Containers::Pio;
    using SCICore::Geometry::Pio;

    int version=stream.begin_class("ScalarFieldRGint", ScalarFieldRGint_VERSION);
    if(version == 1){
...
void ScalarFieldRGint::compute_minmax()
{
...
Vector ScalarFieldRGint::gradient(const Point& p)
{
...
int ScalarFieldRGint::interpolate(const Point& p, double& value, int&,
				   double epsilon1, double epsilon2,
				   int) {
    return interpolate(p, value, epsilon1, epsilon2);
}

int ScalarFieldRGint::interpolate(const Point& p, double& value, double,
				   double)
{
    using SCICore::Math::Interpolate;
...
ScalarField* ScalarFieldRGint::clone()
{
    return scinew ScalarFieldRGint(*this);
}

Vector
ScalarFieldRGint::get_normal( const Point& ivoxel )
{
  return get_normal( ivoxel.x(), ivoxel.y(), ivoxel.z() );
}

Vector
ScalarFieldRGint::get_normal( int x, int y, int z )
{
  Vector normal;
...
Vector 
ScalarFieldRGint::gradient(int x, int y, int z)
{
...
void ScalarFieldRGint::fill_gradmags() // these guys ignor the vf
{
...
//
// $Log$
// Revision 1.6  2000/02/04 01:25:54  dmw
...
//
// replaced ScalarFieldRGxxx with ScalarFieldRGint. it also replaces
