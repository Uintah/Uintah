...
/*
 *  ScalarFieldRGuchar.cc: uchar Scalar Field defined on a Regular grid
 *
 *  Written by:
...
{
    return scinew ScalarFieldRGuchar;
}

PersistentTypeID ScalarFieldRGuchar::type_id("ScalarFieldRGuchar", "ScalarField", maker);

ScalarFieldRGuchar::ScalarFieldRGuchar()
: ScalarFieldRGBase("uchar")
{
}

ScalarFieldRGuchar::ScalarFieldRGuchar(const ScalarFieldRGuchar& copy)
...
void ScalarFieldRGuchar::resize(int x, int y, int z) {
    nx=x; ny=y; nz=z;
    grid.newsize(x,y,z);
}

#define ScalarFieldRGuchar_VERSION 3

void ScalarFieldRGuchar::io(Piostream& stream)
{
    using SCICore::PersistentSpace::Pio;
    using SCICore::Containers::Pio;
    using SCICore::Geometry::Pio;

    int version=stream.begin_class("ScalarFieldRGuchar", ScalarFieldRGuchar_VERSION);
    if(version == 1){
...
void ScalarFieldRGuchar::compute_minmax()
{
...
Vector ScalarFieldRGuchar::gradient(const Point& p)
{
...
int ScalarFieldRGuchar::interpolate(const Point& p, double& value, int&,
				   double epsilon1, double epsilon2,
				   int) {
    return interpolate(p, value, epsilon1, epsilon2);
}

int ScalarFieldRGuchar::interpolate(const Point& p, double& value, double eps,
				   double)
{
    using SCICore::Math::Interpolate;
...
ScalarField* ScalarFieldRGuchar::clone()
{
    return scinew ScalarFieldRGuchar(*this);
}

Vector
ScalarFieldRGuchar::get_normal( const Point& ivoxel )
{
  return get_normal( ivoxel.x(), ivoxel.y(), ivoxel.z() );
}

Vector
ScalarFieldRGuchar::get_normal( int x, int y, int z )
{
  Vector normal;
...
Vector 
ScalarFieldRGuchar::gradient(int x, int y, int z)
{
...
void ScalarFieldRGuchar::fill_gradmags() // these guys ignor the vf
{
...
//
// replaced ScalarFieldRGxxx with ScalarFieldRGuchar. it also replaces
