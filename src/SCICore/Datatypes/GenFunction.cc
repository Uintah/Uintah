...
namespace Datatypes{
  ...
  GenFunction::GenFunction(const StrV& ss, int dd)
    : fns(0), fevals(0), fstr(0), dom_dims(dd), res_dims(0), fns_dims(0)
    {
      set_funcs(ss);
      vars2=new double[dom_dims];
    }
  
  GenFunction::GenFunction(const FuncPtrV& ff, int dd)
    : fns(0), fevals(0), fstr(0), dom_dims(dd), res_dims(0), fns_dims(0)
    {
      set_funcs(ff);
      vars2=new double[dom_dims];
  ...
  void GenFunction::set_funcs(const StrV& ss){
  ...
  void GenFunction::set_funcs(const FuncPtrV& ff){
  ...
  string GenFunction::get_str_f(int n) const{
    if (n<fstr.size() && n>=0)
      return fstr[n];
    else
      return "";
  ...
  void GenFunction::set_num_comp(int nc){    
    if (nc>fns.size()){
      throw DimensionMismatch(nc, fns.size());
    }
    else if (nc<=0)
      return;
    else
  ...
  void GenFunction::io(Piostream&){
    NOT_FINISHED("Implement GenFunction::io function");
  }
