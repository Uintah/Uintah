...
public:
    //////////
    // Create a thread, which will execute the <b>run()</b> method in the
    // <b>runner</b> object. The thread <b>name</b> is used for
    // identification purposes, and does not need to be unique
    // with respect to other threads.  <b>Group</b> specifies the
    // ThreadGroup that to which this thread should belong.  If no group
    // is specified (group==0), the default group is used.
    Thread(Runnable* runner, const std::string& name, ThreadGroup* group=0,
	   bool stopped=false);
    ...
    //////////
    //Flag the thread as a daemon thread.  When all non-deamon threads
    // exit, the program will exit.
    void setDaemon(bool to=true);

    //////////
    // Set the priority for the thread.  Priorities range from 1 to 10,
    // with 10 having the highest priority.  The default priority is 5.
    ...
    //////////
    // Arrange to have the thread deleted automatically at exit.  The
    // pointer to the thread should not be used by any other threads
    ...
    //////////
    // An error occured for a particular thread.  Print out the message
    // and call <b>niceAbort()</b>
    ...
    //////////
    // Blocks the calling thread until this thead has finished executing.
    // You cannot join detached threads or daemon threads.
    void join();
    ...
    //////////
    // Request that the thread migrate to processor <i>proc</i>.  If
    // <i>proc</i> is -1, then the thread is free to run anywhere.
    void migrate(int proc);

    //////////
    // Returns the private pointer - should only be used by the thread
    // implementation
    ...
    // of the threads return.  Otherwise, the call will return immediately.
    ...
    static int couldBlock(const std::string& why);

    //////////
    // Mark the end of a selection that could block.  <i>restore</i> was
    // returned from a previous invocation of the above <b>couldBlock</b>.
