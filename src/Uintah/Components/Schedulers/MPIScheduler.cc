...
MPIScheduler::MPIScheduler( int MpiRank, int MpiProcesses ) :
...
void
MPIScheduler::setupTaskConnections()
{
...
void
MPIScheduler::performTask(TaskRecord* task,
...
void
MPIScheduler::execute(const ProcessorContext * pc,
			             DataWarehouseP   & dwp )
{
    if(d_tasks.size() == 0){
    ...
    for( iter=d_tasks.begin(); iter != d_tasks.end(); iter++ ) {
	 ...
	 if( !patch || patch->getID() >= 0 && patch->getID() <= 2 )
	   ...
	   taskLocation = 1;
	    ...
	    throw InternalError("MPIScheduler: Unknown patch");		                              
#else
	 int taskLocation=0;
#endif
	...
	if(ncompleted == 0)
	    throw InternalError("MPIScheduler stalled");
...
bool
MPIScheduler::allDependenciesCompleted(TaskRecord*) const
{
    //cerr << "MPIScheduler::allDependenciesCompleted broken!\n";
    return true;
}
...
MPIScheduler::dumpDependencies()
{
    ...
    if (!depfile) {
	cerr << "MPIScheduler::dumpDependencies: unable to open output file!\n";
