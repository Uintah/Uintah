...
#	puts "r: $r  c: $c  rr: $rr  cc: $cc"
	...
	global $this-TclMat
	global $this-loading
	global .ui[modname].matrix.tab
	puts "changing the columns..."
#	set last_nc [expr [lindex [.ui[modname].matrix.tab config -cols] 4] - 1]
	trace vdelete $this-array w "$this matChanged"
#	.ui[modname].matrix.tab config -cols [expr [set $this-ncol] + 1]
	...
	set last_nr [expr [lindex [.ui[modname].matrix.tab config -rows] 4] - 1]
	trace vdelete $this-array w "$this matChanged"
	.ui[modname].matrix.tab config -rows [expr [set $this-nrow] + 1]
	# gotta go in and change $this-array to fit the new dimensions
	set oldMat [set $this-TclMat]
...
#	puts "In tcl_load"
	...
	trace vdelete $this-array w "$this matChanged"
	.ui[modname].matrix.tab flash mode off
	set cc [set $this-nncol]
	set r 0
	set c 0
...
#	}          
	...
	set $this-loading 0
	.ui[modname].matrix.tab flash mode on
        update
	...
	global $this-ncol
	global $this-array
	label $w.matrix.label -text "matrix editor"
	pack $w.matrix.label -fill x -pady 2
