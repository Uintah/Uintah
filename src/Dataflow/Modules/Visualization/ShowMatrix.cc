...
/*
 *  ShowMatrix.cc:  Visual matrix editor
 *
 *  Written by:
 *   David Weinstein
...
extern "C" Module* make_ShowMatrix(const clString& id) {
  return new ShowMatrix(id);
}

ShowMatrix::ShowMatrix(const clString& id)
: Module("ShowMatrix", id, Source), numMatrices("numMatrices", id, this),
...
void ShowMatrix::connection(ConnectionMode mode, int which_port,
				 int output)
{
    if (output) {
...
void ShowMatrix::makeRasterFont(void)
{
...
void ShowMatrix::printString(char *s)
{
...
void ShowMatrix::tcl_command(TCLArgs& args, void* userdata) {
...
void ShowMatrix::plot_snoop_rect() {
