...
/*
 *  Resample.cc:  Unfinished modules
 *
 *  Written by:
 *   David Weinstein
...
public:
    Resample(const clString& id);
    virtual ~Resample();
    virtual void execute();
};

extern "C" Module* make_Resample(const clString& id) {
  return new Resample(id);
}

static clString module_name("Resample");
Resample::Resample(const clString& id)
: Module("Resample", id, Filter), ox("ox", id, this), oy("oy", id, this),
  ...
  range_max_y("range_max_y", id, this), range_max_z("range_max_z", id, this),
    widget_lock("Resample widget lock")
{
...
void Resample::widget_moved(int last) {
    if (last && !abort_flag) {
	abort_flag=1;
	check_widget=1;
	want_to_execute();
...
void Resample::filter(ScalarFieldRGBase* in, ScalarFieldRGBase* out,
			 int min_z, int max_x) {
    if (lastFT == "Box") {
...
void Resample::execute() {
    ...
    isf=ifh->getRGBase();
    if(!isf){
	error("Resample can't deal with unstructured grids!");
	return;
    }
    if (!init) {
	init=1;
	...
	int min_ijk[3], max_ijk[3];
	Point min_point(right.x(), down.y(), in.z());
	Point max_point(center+(center-min_point));
	isf->locate(min_ijk, min_point);
	isf->locate(max_ijk, max_point);
	min_ijk[0]++; min_ijk[1]++; min_ijk[2]++;
	max_ijk[0]++; max_ijk[1]++; max_ijk[2]++;
	if (min_ijk[0]<1) min_ijk[0]=1;
	if (min_ijk[1]<1) min_ijk[1]=1;
	if (min_ijk[2]<1) min_ijk[2]=1;
	if (max_ijk[0]<2) max_ijk[0]=2;
	if (max_ijk[1]<2) max_ijk[1]=2;
	if (max_ijk[2]<2) max_ijk[2]=2;
	if (max_ijk[0]>isf->nx) max_ijk[0]=isf->nx;
	if (max_ijk[1]>isf->ny) max_ijk[1]=isf->ny;
	if (max_ijk[2]>isf->nz) max_ijk[2]=isf->nz;
	if (min_ijk[0]>isf->nx) min_ijk[0]=isf->nx-1;
	if (min_ijk[1]>isf->ny) min_ijk[1]=isf->ny-1;
	if (min_ijk[2]>isf->nz) min_ijk[2]=isf->nz-1;
	if (min_ijk[0]==max_ijk[0]) max_ijk[0]++;
	if (min_ijk[1]==max_ijk[1]) max_ijk[1]++;
	if (min_ijk[2]==max_ijk[2]) max_ijk[2]++;
	range_min_x.set(min_ijk[0]);
	range_min_y.set(min_ijk[1]);
	range_min_z.set(min_ijk[2]);
	range_max_x.set(max_ijk[0]);
	range_max_y.set(max_ijk[1]);
	range_max_z.set(max_ijk[2]);
    }
    if (newFld) {
...
void Resample::boxFilter(ScalarFieldRGBase* in, ScalarFieldRGBase* out,
				  int fZ, int lZ) {
...
void Resample::triangleFilter(ScalarFieldRGBase* in, ScalarFieldRGBase* out,
				 int fZ, int lZ) {
    ScalarFieldRGdouble *ifd, *ofd;
    ScalarFieldRGfloat *iff, *off;
    ScalarFieldRGint *ifi, *ofi;
...
void Resample::buildUndersampleTriangleTable(Array2<double> *table,
						int size, int span, 
						double ratio) {
...
void Resample::buildOversampleTriangleTable(Array2<double> *table,
...
void Resample::mitchellFilter(ScalarFieldRGBase* in, ScalarFieldRGBase* out,
				 int fZ, int lZ) {
    ScalarFieldRGdouble *ifd, *ofd;
    ScalarFieldRGfloat *iff, *off;
    ScalarFieldRGint *ifi, *ofi;
...
void Resample::buildUndersampleMitchellTable(Array2<double> *table,
						int size, int span, 
						double ratio) {
    for (int i=0; i<size; i++) {
        double total=0;
...
void Resample::buildOversampleMitchellTable(Array2<double> *table,
					       int size, double ratio) {
    double curr=1.;
    for (int i=0; i<size; i++) {
	(*table)(i,0)=curr;
