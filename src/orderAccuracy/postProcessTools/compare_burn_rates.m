...
%
%  Example usage:
%  compare_BurnRates.m -pDir 1 -mat 1 -SA 0.001 -rho_CC 1832  
%_________________________________
clear all;
close all;
format short e;
...
time_ap
%__________________________________
% Load and copy experimental data
% This data is for experiments with initial temp of 298 K
% The script is expecting this file to be in your home directory

c4 = sprintf('cp ~/ExpData.dat .');
unix(c4);
...
% move plot to uda file  
  c5 = sprintf('mv mbr_ap.ps %s.000',uda);
  unix(c5);
 
% make copy of plot with uda name for easier access   
  c6          = sprintf('cp %s.000/mbr_ap.ps %s_mbr_ap.ps',uda,uda);
  [s6, r6]    = unix(c6);

%Plot average pressure vs time 
figure(3)
 plot(time_ap, avePress, 'b:o;Computed;')
...
% move plot to uda file  
  c7 = sprintf('mv Time_ap.ps %s.000',uda);
  unix(c7);
...
fclose(fid);
 c8 = sprintf('mv AP_mbr.dat %s.000',uda);
  unix(c8);
