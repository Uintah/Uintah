...
#ifdef HAVE_PTHREAD
static struct sidl_recursive_mutex_t pde_Tri__mutex= SIDL_RECURSIVE_MUTEX_INITIALIZER;
#define LOCK_STATIC_GLOBALS sidl_recursive_mutex_lock( &pde_Tri__mutex )
#define UNLOCK_STATIC_GLOBALS sidl_recursive_mutex_unlock( &pde_Tri__mutex )
/* #define HAVE_LOCKED_STATIC_GLOBALS (sidl_recursive_mutex_trylock( &pde_Tri__mutex )==EDEADLOCK) */
#else
...
static struct pde_Tri__epv s_my_epv__pde_tri;

static struct pde_Tri__epv s_my_epv_contracts__pde_tri;

static struct pde_Tri__epv s_my_epv_hooks__pde_tri;
...
static struct pdeports_MeshPort__epv s_my_epv__pdeports_meshport;
static struct pdeports_MeshPort__epv s_my_epv_hooks__pdeports_meshport;
...
static struct pde_Tri__pre_epv s_preEPV;
static struct pde_Tri__post_epv s_postEPV;
...
extern void pde_Tri__set_epv(
  struct pde_Tri__epv* epv,
    struct pde_Tri__pre_epv* pre_epv,
    struct pde_Tri__post_epv* post_epv);

extern void pde_Tri__call_load(void);
#ifdef __cplusplus
}
#endif

static void
pde_Tri_addRef__exec(
        struct pde_Tri__object* self,
...
pde_Tri_isSame__exec(
  ...
  iobj = skel_pde_Tri_fconnect_sidl_BaseInterface(iobj_str, TRUE, 
    _ex);SIDL_CHECK(*_ex);
#else
  iobj_bc = sidl_rmi_InstanceRegistry_getInstanceByString(iobj_str, 
    _ex);SIDL_CHECK(*_ex);
  if(iobj_bc != NULL) {
    iobj= (struct pde_Tri__object*) (*iobj_bc->d_epv->f__cast)(iobj_bc, 
      "pde.Tri", _ex);
    if(iobj != NULL) {
...
pde_Tri_triangulate__exec(
  ...
  struct sidl_int__array* boundaries = NULL;
  struct sidl_int__array* triangles_data = NULL;
  struct sidl_int__array** triangles = &triangles_data;
  int32_t _retval = 0;
  sidl_BaseInterface _throwaway_exception = NULL;
  sidl_BaseInterface _ex3   = NULL;
  sidl_BaseException _SIDLex = NULL;
  ...
  sidl_rmi_Call_unpackDoubleArray( inArgs, "nodes", &nodes,0,0,FALSE, 
    _ex);SIDL_CHECK(*_ex);
  sidl_rmi_Call_unpackIntArray( inArgs, "boundaries", &boundaries,0,0,FALSE, 
    _ex);SIDL_CHECK(*_ex);

  /* make the call */
  _retval = (self->d_epv->f_triangulate)(
    self,
    nodes,
    boundaries,
    triangles,
  ...
  /* pack out and inout argments */
  sidl_rmi_Return_packIntArray( outArgs, "triangles",*triangles,0,0,0, 
    _ex);SIDL_CHECK(*_ex);
  EXIT:
  /* clean-up dangling references */
  sidl__array_deleteRef((struct sidl__array*)nodes);
  sidl__array_deleteRef((struct sidl__array*)boundaries);
  sidl__array_deleteRef((struct sidl__array*)*triangles);
  if(*_ex) { 
    _SIDLex = sidl_BaseException__cast(*_ex,&_throwaway_exception);
    sidl_rmi_Return_throwException(outArgs, _SIDLex, &_throwaway_exception); 
    if(_throwaway_exception) {
...
pde_Tri_setServices__exec(
  ...
  services = skel_pde_Tri_fconnect_gov_cca_Services(services_str, TRUE, 
    _ex);SIDL_CHECK(*_ex);
#else
  services_bc = sidl_rmi_InstanceRegistry_getInstanceByString(services_str, 
    _ex);SIDL_CHECK(*_ex);
  if(services_bc != NULL) {
    services= (struct pde_Tri__object*) (*services_bc->d_epv->f__cast)(
      services_bc, "pde.Tri", _ex);
    if(services != NULL) {
...
static void ior_pde_Tri__set_contracts(
  struct pde_Tri__object* self,
  sidl_bool   enable,
...
static void ior_pde_Tri__dump_stats(
  struct pde_Tri__object* self,
  const char* filename,
  const char* prefix,
  struct sidl_BaseInterface__object **_ex)
...
static void ior_pde_Tri__ensure_load_called(void) {
  /*
  ...
  *_ex = NULL; /* default to no exception */
  ...
  if (!cmp) {
    ...
    if (!cmp) {
      ...
      cmp = strcmp(name, "pde.Tri");
      if (!cmp) {
        (*self->d_epv->f_addRef)(self, _ex); SIDL_CHECK(*_ex);
        cast = ((struct pde_Tri__object*)self);
        return cast;
      }
    }
  }
...
static void ior_pde_Tri__set_hooks(
  struct pde_Tri__object* self,
...
static void ior_pde_Tri__delete(
  struct pde_Tri__object* self, struct sidl_BaseInterface__object **_ex)
{
  *_ex  = NULL; /* default to no exception */
  pde_Tri__fini(self, _ex);
  memset((void*)self, 0, sizeof(struct pde_Tri__object));
  free((void*) self);
}
...
static void
ior_pde_Tri__raddRef(
    struct pde_Tri__object* self, sidl_BaseInterface* _ex) {
  sidl_BaseInterface_addRef((sidl_BaseInterface)self, _ex);
}

static sidl_bool
ior_pde_Tri__isRemote(
    struct pde_Tri__object* self, sidl_BaseInterface* _ex) {
  *_ex  = NULL; /* default to no exception */
  return FALSE;
}

struct pde_Tri__method {
  const char *d_name;
  void (*d_func)(struct pde_Tri__object*,
...
ior_pde_Tri__exec(
  ...
  struct sidl_BaseInterface__object **_ex )
{
  static const struct pde_Tri__method  s_methods[] = {
    { "_cast", pde_Tri__cast__exec },
    { "addRef", pde_Tri_addRef__exec },
    { "deleteRef", pde_Tri_deleteRef__exec },
    { "getClassInfo", pde_Tri_getClassInfo__exec },
    { "isSame", pde_Tri_isSame__exec },
    { "isType", pde_Tri_isType__exec },
    { "setServices", pde_Tri_setServices__exec },
    { "triangulate", pde_Tri_triangulate__exec }
  };
  int i, cmp, l = 0;
  int u = sizeof(s_methods)/sizeof(struct pde_Tri__method);
  *_ex  = NULL; /* default to no exception */
...
static void pde_Tri__init_epv(void)
{
/*
 * assert( HAVE_LOCKED_STATIC_GLOBALS );
 */

  struct pde_Tri__epv*            epv  = &s_my_epv__pde_tri;
  struct gov_cca_Component__epv*  e0   = &s_my_epv__gov_cca_component;
  struct gov_cca_Port__epv*       e1   = &s_my_epv__gov_cca_port;
  struct pdeports_MeshPort__epv*  e2   = &s_my_epv__pdeports_meshport;
  ...
  epv->f__cast                 = ior_pde_Tri__cast;
  epv->f__delete               = ior_pde_Tri__delete;
  epv->f__exec                 = ior_pde_Tri__exec;
  epv->f__getURL               = ior_pde_Tri__getURL;
  epv->f__raddRef              = ior_pde_Tri__raddRef;
  epv->f__isRemote             = ior_pde_Tri__isRemote;
  epv->f__set_hooks            = ior_pde_Tri__set_hooks;
  epv->f__set_contracts        = ior_pde_Tri__set_contracts;
  epv->f__dump_stats           = ior_pde_Tri__dump_stats;
  epv->f__ctor                 = NULL;
  epv->f__ctor2                = NULL;
  epv->f__dtor                 = NULL;
  epv->f_addRef                = (void (*)(struct pde_Tri__object*,struct sidl_BaseInterface__object **)) s1->f_addRef;
  epv->f_deleteRef             = (void (*)(struct pde_Tri__object*,struct sidl_BaseInterface__object **)) s1->f_deleteRef;
  epv->f_isSame                = (sidl_bool (*)(struct pde_Tri__object*,struct sidl_BaseInterface__object*,struct sidl_BaseInterface__object **)) s1->f_isSame;
  epv->f_isType                = (sidl_bool (*)(struct pde_Tri__object*,const char*,struct sidl_BaseInterface__object **)) s1->f_isType;
  epv->f_getClassInfo          = (struct sidl_ClassInfo__object* (*)(struct pde_Tri__object*,struct sidl_BaseInterface__object **)) s1->f_getClassInfo;
  epv->f_triangulate           = NULL;
  epv->f_setServices           = NULL;

  pde_Tri__set_epv(epv, &s_preEPV, &s_postEPV);
  ...
  /*
   * Override function pointers for pdeports.MeshPort with mine, as needed.
   */
  ...
  e2->f_triangulate           = (int32_t (*)(void*,struct sidl_double__array*,struct sidl_int__array*,struct sidl_int__array**,struct sidl_BaseInterface__object **)) epv->f_triangulate;
  ...
  ior_pde_Tri__ensure_load_called();
}

/*
 * pde_Tri__getEPVs: Get my version of all relevant EPVs.
 */

void pde_Tri__getEPVs (
  ...
  struct pde_Tri__epv **s_arg_epv_hooks__pde_tri)
{
  ...
  *s_arg_epv__pdeports_meshport = &s_my_epv__pdeports_meshport;
  *s_arg_epv_hooks__pdeports_meshport = &s_my_epv_hooks__pdeports_meshport;
  *s_arg_epv__pde_tri = &s_my_epv__pde_tri;
  *s_arg_epv_hooks__pde_tri = &s_my_epv_hooks__pde_tri;
}
/*
 * __getSuperEPV: returns parent's non-overrided EPV
 */

static struct sidl_BaseClass__epv* pde_Tri__getSuperEPV(void) {
...
initClassInfo(sidl_ClassInfo *info, struct sidl_BaseInterface__object **_ex)
{
  ...
  *_ex  = NULL; /* default to no exception */
  ...
  if (!s_classInfo) {
    ...
    if (impl) {
      sidl_ClassInfoI_setName(impl, "pde.Tri", _ex);
...
static void
initMetadata(struct pde_Tri__object* self, sidl_BaseInterface* _ex)
{
...
/*
 * pde_Tri__createObject: Allocate the object and initialize it.
 */
...
{
  struct pde_Tri__object* self =
    (struct pde_Tri__object*) sidl_malloc(
      sizeof(struct pde_Tri__object),
      "Object allocation failed for struct pde_Tri__object",
        __FILE__, __LINE__, "pde_Tri__createObject", _ex);
  if (!self) goto EXIT;
  pde_Tri__init(self, ddata, _ex); SIDL_CHECK(*_ex);
...
void pde_Tri__init(
  ...
  struct sidl_BaseInterface__object **_ex)
{
  struct pde_Tri__object*        s0 = self;
  ...
  *_ex = 0; /* default no exception */
  ...
  s0->d_pdeports_meshport.d_epv = &s_my_epv__pdeports_meshport;
  s0->d_epv                     = &s_my_epv__pde_tri;

  s0->d_gov_cca_component.d_object = self;

  s0->d_gov_cca_port.d_object = self;

  s0->d_pdeports_meshport.d_object = self;
...
void
pde_Tri__IOR_version(int32_t *major, int32_t *minor)
{
  *major = s_IOR_MAJOR_VERSION;
  *minor = s_IOR_MINOR_VERSION;
}

static const struct pde_Tri__external
s_externalEntryPoints = {
  pde_Tri__createObject,
  pde_Tri__getSuperEPV,
  2, 
  0
};
