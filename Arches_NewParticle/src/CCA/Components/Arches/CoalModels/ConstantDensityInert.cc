...
ConstantDensityInert::problemSetup(const ProblemSpecP& params)
{
  ...
  */
  ...
  if( !d_useMass ) {
    string errmsg = "ERROR: Arches: ConstantDensityInert: You must specify a particle mass internal coordinate.\n";
    throw ProblemSetupException(errmsg,__FILE__,__LINE__);
  }

  if( !d_useLength ) {
    string errmsg = "ERROR: Arches: ConstantDensityInert: You must specify a particle size internal coordinate.\n";
    throw ProblemSetupException(errmsg,__FILE__,__LINE__);
  }
  ...
  massLabels.push_back( d_particle_mass_label );
...
{
  std::string taskname = "ConstantDensityInert::computeModel";
