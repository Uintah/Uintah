 ...
 ---------------------------------------------------------------------- */
    ...
    void readInputFile(    
           ...
           *___________________________________*/
            double  *t_inject_TE_parms, /* array holding TE injection timing info */
                                        /*  [1] = When to begin injection   */
                                        /*  [2] = When to stop injecting    */
                                        /*  [3] = delta t injection         */
           double  *Q_fire,             /* Heat Release rate of fire        */
           int     *N_inject_TE,        /* number of TE injected at any time*/
           double  *del_H_fuel,         /* heat of combustion for the fuel  */ 
           double  *rho_fuel,           /* density of fuel                  */
           int     *index_inject_TE,    /* indices where the TE are injected*/        
           int     *nMaterials         )
...
*   NIST fire varialbes
*___________________________________*/  
    t_inject_TE_parms[1]                            = readdouble(fp," t_initial_output",switchDebug_readInputFile);
    t_inject_TE_parms[2]                            = readdouble(fp," t_final_output",switchDebug_readInputFile);
    t_inject_TE_parms[3]                            = readdouble(fp," t_delta_output",switchDebug_readInputFile);
    *N_inject_TE                                    = readint(fp," N_inject_TE",switchDebug_readInputFile);
    *Q_fire                                         = readdouble(fp," Q_fire",switchDebug_readInputFile);
    *del_H_fuel                                     = readdouble(fp," del_H_fuel",switchDebug_readInputFile);                         
    *rho_fuel                                       = readdouble(fp," rho_fuel",switchDebug_readInputFile);                         
    
    index_inject_TE[1]                              = readint(fp,"  index_inject_TE[1]",switchDebug_readInputFile);              
    index_inject_TE[2]                              = readint(fp,"  index_inject_TE[2]",switchDebug_readInputFile);              
    index_inject_TE[3]                              = readint(fp,"  index_inject_TE[3]",switchDebug_readInputFile);              
    index_inject_TE[4]                              = readint(fp,"  index_inject_TE[4]",switchDebug_readInputFile);              
    index_inject_TE[5]                              = readint(fp,"  index_inject_TE[5]",switchDebug_readInputFile);              
    index_inject_TE[6]                              = readint(fp,"  index_inject_TE[6]",switchDebug_readInputFile);              
 ...
 *___________________________________*/     
    ...
    if (t_inject_TE_parms[1]< 0.0)  
        Message(1,"File: if","You've specified a negative initial ouput time",
       "Error: t_inject_TE_parms[1]< 0.0");
       
    if (t_inject_TE_parms[2]< t_inject_TE_parms[1])  
        Message(1,"File: if","The final output time is < the initial output time",
       "Error: t_inject_TE_parms[2]< t_inject_TE_parms[1]");
       
    if (t_inject_TE_parms[3]< 0.0)  
        Message(1,"File: if","You've specified a negative delta ouput time",
       "Error: t_inject_TE_parms[3]< 0.0");
       
    if ( (Q_fire<0.0) || (Q_fire > BIG_NUM) )  
        Message(1,"File: input.f","Subroutine: testInputFile",
       "Error: Q_fire<0.0) || (Q_fire > BIG_NUM)");  
       
    if ( (N_inject_TE<0) || (N_inject_TE > N_THERMAL_ELEMENTS) )  
        Message(1,"File: input.f","Subroutine: testInputFile",
       "Error: N_inject_TE<0.0) || (N_inject_TE > BIG_NUM)");  
       
    if ( (del_H_fuel<0.0) || (del_H_fuel > BIG_NUM) )  
        Message(1,"File: input.f","Subroutine: testInputFile",
       "Error: del_H_fuel<0.0) || (del_H_fuel > BIG_NUM)"); 
       
     if ( (rho_fuel<0.0) || (rho_fuel > BIG_NUM) )  
        Message(1,"File: input.f","Subroutine: testInputFile",
       "Error: rho_fuel<0.0) || (rho_fuel > BIG_NUM)"); 

        
    if ( (index_inject_TE[1]<xLoLimit) || (index_inject_TE[1] > xHiLimit) )  
        Message(1,"File: input.f","Subroutine: testInputFile",
       "Error: (index_inject_TE[1]<xLoLimit) || (index_inject_TE[1] > xHiLimit)");
       
    if ( (index_inject_TE[2]<xLoLimit) || (index_inject_TE[2] > xHiLimit) )  
        Message(1,"File: input.f","Subroutine: testInputFile",
       "Error: (index_inject_TE[2]<xLoLimit) || (index_inject_TE[2] > xHiLimit)");         
          
    if ( (index_inject_TE[3]<yLoLimit) || (index_inject_TE[3] > yHiLimit) )  
        Message(1,"File: input.f","Subroutine: testInputFile",
       "Error: (index_inject_TE[3]<yLoLimit) || (index_inject_TE[3] > yHiLimit)");
       
    if ( (index_inject_TE[4]<yLoLimit) || (index_inject_TE[4] > yHiLimit) )  
        Message(1,"File: input.f","Subroutine: testInputFile",
       "Error: (index_inject_TE[4]<yLoLimit) || (index_inject_TE[4] > yHiLimit)");          

    if ( (index_inject_TE[5]<zLoLimit) || (index_inject_TE[5] > zHiLimit) )  
        Message(1,"File: input.f","Subroutine: testInputFile",
       "Error: (index_inject_TE[5]<zLoLimit) || (index_inject_TE[5] > zHiLimit)");          
 
     if ( (index_inject_TE[6]<zLoLimit) || (index_inject_TE[6] > zHiLimit) )  
        Message(1,"File: input.f","Subroutine: testInputFile",
       "Error: (index_inject_TE[6]<zLoLimit) || (index_inject_TE[6] > zHiLimit)");          
